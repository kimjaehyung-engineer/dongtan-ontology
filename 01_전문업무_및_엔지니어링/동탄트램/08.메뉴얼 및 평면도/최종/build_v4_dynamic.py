...
if not v3_file_name:
    for f in os.listdir(base_dir):
        if f.endswith('.xlsx') and '집행단계' in f and not f.startswith('~$'):
            v3_file_name = f
            break

src_path = os.path.join(base_dir, v3_file_name)
dst_path = os.path.join(base_dir, "매뉴얼 BODY (집행단계)v4.xlsx")

print(f"Source file found: {v3_file_name}")
print(f"Copying to destination: 매뉴얼 BODY (집행단계)v4.xlsx")
