...
path_std = os.path.join(target_dir, "표준서", "[PST] 전단앵커설치 및 충전재 주입_표준서.html")
path_std_alt = os.path.join(target_dir, "표준서", "16_[PST] 전단앵커설치 및 충전재 주입_표준서.html")
...
path_chk = os.path.join(target_dir, "체크리스트", "[PST] 전단앵커설치 및 충전재 주입_체크리스트.html")
path_chk_alt = os.path.join(target_dir, "체크리스트", "16_[PST] 전단앵커설치 및 충전재 주입_체크리스트.html")
...
def force_write(path, text):
    ...
    with open(path, 'w', encoding='utf-8') as f:
        f.write(text)
    print(f"✏️ Successfully written WBS 16 file: {path}")
...
<head>
    ...
    <title>콘크리트도상 - [PST] 전단앵커설치 및 충전재 주입 기술 표준서</title>
...
<div class="max-w-5xl mx-auto bg-white shadow-2xl rounded-2xl border border-slate-200 overflow-hidden">
    ...
    <div class="bg-slate-900 text-white p-8 relative overflow-hidden">
        ...
        <div class="relative z-10">
            <div class="flex items-center gap-3 mb-2">
                <span class="bg-blue-600 text-white text-xs font-bold px-3 py-1 rounded-full uppercase tracking-wider">Dongtan Tram WBS 9000-6-16 Standard</span>
                <span class="bg-emerald-50 text-slate-950 text-xs font-bold px-3 py-1 rounded-full">전단앵커 일체화 & 그라우트 45 MPa 규격</span>
            </div>
            <h1 class="text-3xl sm:text-4xl font-black tracking-tight">[PST] 전단앵커설치 및 충전재 주입 기술 표준서</h1>
            <p class="text-slate-300 mt-2 text-sm sm:text-base">"HBS/TCL층과 일체화: 전단앵커 천공/수직도 검수, 무수축 그라우트(&ge;45 MPa) 주입, EVA Foam 오버플로우 및 경화 전 전단키 덮개 봉인 매뉴얼"</p>
        </div>
    </div>
    ...
    <div class="p-6 sm:p-10 space-y-8">
        ...
        <div class="bg-amber-50 border border-amber-200 p-4 rounded-xl text-xs sm:text-sm text-amber-900">
            <h4 class="font-bold text-amber-950 mb-1">💡 [PST] 전단앵커설치 및 충전재 주입 과업 정의</h4>
            <p class="leading-relaxed">
                본 공종은 HBS 강화노반 및 TCL 기초 콘크리트층과 상부 PST 프리캐스트 슬래브 간의 일체화 구조 확보를 위해 <strong><span class="term-highlight" onclick="openGlossary('shear_anchor')">전단앵커를 설치</span></strong>하고 몰탈 충전재를 밀실 주입하는 핵심 공종입니다. 천공 깊이 및 수직도 게이지 점검, <strong><span class="term-highlight" onclick="openGlossary('grout_strength')">무수축 그라우트 압축강도 &ge; 45 MPa(최소 30 MPa 이상)</span></strong> 확보, <strong><span class="term-highlight" onclick="openGlossary('eva_foam')">EVA Foam 구멍을 통한 몰탈 유출 오버플로우</span></strong> 확인, 그리고 경화 전 <strong><span class="term-highlight" onclick="openGlossary('key_cap')">전단키 덮개 시공</span></strong> 수칙을 준수합니다.
            </p>
        </div>
        ...
        <div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                <div class="bg-slate-50 p-4 rounded-xl border border-slate-200">
                    ...
                    <p class="font-bold text-slate-800 mt-1">콘크리트도상 / [PST] 전단앵커설치 및 충전재 주입 (현장 공사팀)</p>
                </div>
                <div class="bg-slate-50 p-4 rounded-xl border border-slate-200">
                    <span class="text-xs font-bold text-blue-700 uppercase">최종 품질 산출물</span>
                    <p class="font-bold text-slate-800 mt-1">앵커 시공인장 시험기록 | 그라우트 공시체 강도대장 | 음향탐사 검측표</p>
                </div>
            </div>
        </div>
        ...
        <div>
            <h2 class="text-xl font-bold text-slate-900 mb-4 flex items-center gap-2 border-b-2 border-blue-600 pb-2">
                <span class="text-blue-600">2.</span> 전단앵커 및 충전재 정량 기술 시방 절대 기준
            </h2>
            <div class="overflow-x-auto">
                <table class="w-full text-xs sm:text-sm text-left border-collapse border border-slate-200 rounded-lg">
                    ...
                    <tbody class="divide-y divide-slate-200 bg-white">
                        <tr>
                            <td class="p-3 font-bold bg-slate-50 border text-center">천공 깊이 & 수직도</td>
                            <td class="p-3 border text-center font-bold text-blue-700">게이지 측정 100% 합격</td>
                            <td class="p-3 border text-slate-600">• 천공 전용 깊이 게이지로 설계 깊이 수직도 점검.<br>• 홀 내 분진 및 돌가루 에어/수쇄 100% 제거</td>
                        </tr>
                        <tr>
                            <td class="p-3 font-bold bg-slate-50 border text-center">충전재 압축강도</td>
                            <td class="p-3 border text-center font-bold text-red-600">&ge; 45.0 MPa (몰탈 공시체)<br>(최소 &ge; 30 MPa 이상)</td>
                            <td class="p-3 border text-slate-600">• 무수축 모르타르 배합비 준수 및 28일 강도 시험.<br>• 현장 몰드 시편 45 MPa 미달 시 전량 교체</td>
                        </tr>
                        <tr>
                            <td class="p-3 font-bold bg-slate-50 border text-center">EVA Foam 유출 확인</td>
                            <td class="p-3 border text-center font-bold text-blue-700">구멍별 오버플로우 확인</td>
                            <td class="p-3 border text-slate-600">• EVA Foam 공기 배출구로 몰탈 유출 시까지 연속 주입.<br>• 내부 공기 공동(Air Void) 0% 밀실 충전 구현</td>
                        </tr>
                        <tr>
                            <td class="p-3 font-bold bg-slate-50 border text-center">미충전 시 재작업 수칙</td>
                            <td class="p-3 border text-center font-bold text-red-600">경화 전 100% 제거 후 세척</td>
                            <td class="p-3 border text-slate-600">• 주입 장비/재료 이상으로 미충전 발생 시 즉시 중단.<br>• 기 주입재 경화 전 완전 제거 및 물청소 후 재작업</td>
                        </tr>
                        <tr>
                            <td class="p-3 font-bold bg-slate-50 border text-center">전단키 덮개 시공</td>
                            <td class="p-3 border text-center font-bold text-blue-700">경화 전 봉인 덮개 시공</td>
                            <td class="p-3 border text-slate-600">• 충전재 주입 직후 경화 전 전단키 덮개 가설.<br>• 수분 급속 유출 및 외부 이물질 침투 완전 차단</td>
                        </tr>
                    </tbody>
                </table>
            </div>
...
# =========================================================================
# 2. WBS 16 GUIDELINE HTML (Flexible 4-Step & 4 Step-by-Step Diagrams)
# =========================================================================
...
<div class="max-w-5xl mx-auto bg-white shadow-2xl rounded-2xl border border-slate-200 overflow-hidden">
    ...
    <div class="p-6 sm:p-10 space-y-10">
        ...
        <div class="bg-amber-50 border border-amber-200 p-5 rounded-xl text-xs sm:text-sm text-amber-900 shadow-sm">
            ...
            <p class="leading-relaxed">
                HBS 강화노반 및 TCL 기초층과 상부 PST 패널을 일체화하는 <strong><span class="term-highlight" onclick="openGlossary('shear_anchor')">전단앵커 공종</span></strong>입니다. STEP 1부터 STEP 4까지 각 단계에 직관적으로 1:1 매칭되는 <strong>4개의 정밀 2D 기술 도식</strong>을 제공하며, 모든 도식은 클릭 시 화면 전체에 <strong>초대형 고화질 뷰(`openDiagramZoom`)</strong>로 확대되어 작은 글씨도 시원하게 읽힙니다.
            </p>
        </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
                ...
                <div class="flow-card bg-blue-50 p-4 rounded-xl border border-blue-200 flex flex-col justify-between space-y-3">
                    <div>
                        ...
                        <p class="text-[11px] text-slate-600 mt-1 leading-snug">
                            경화 전 <strong>전단키 덮개 시공</strong> 봉인 & 28일 몰탈 강도 <strong>&ge; 45 MPa</strong> 승인
                        </p>
                    </div>
                    ...
                    <div>
                        ...
                        <div class="clickable-diagram bg-slate-50 p-4 rounded-xl flex justify-center items-center shadow-inner border border-slate-200" onclick="openDiagramZoom('svgDiagram16_STEP3', '[도식 3] 무수축 모르타르 주입 & EVA Foam 유출 오버플로우 도면')">
                            <svg id="svgDiagram16_STEP3" viewBox="0 0 420 220" width="100%" height="220" xmlns="http://www.w3.org/2000/svg">
                                ...
                                <text x="170" y="12" font-size="10" font-weight="black" fill="#0284c7" text-anchor="middle">주입 펌프</text>

                                <!-- EVA Foam 유출 구멍 오버플로우 -->
...
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>콘크리트도상 - [PST] 전단앵커설치 및 충전재 주입 체크리스트</title>
    <style>
        :root {{
            --bg-primary: #f8fafc;
            --bg-card: #ffffff;
...
<div class="container">
    <div class="header">
        <h1 class="title">[PST] 전단앵커설치 및 충전재 주입 체크리스트</h1>
        <span class="meta">WBS Code 9000-6-16 | 콘크리트도상 품질 검측대장</span>
    </div>

    <div class="summary-box">
        <h4 style="margin: 0 0 8px 0; color: #065f46; font-size: 1.05rem; font-weight: 800;">📋 표준서 & 수행지침 연계 전단앵커 및 충전재 필수 검측 항목</h4>
        <div style="font-weight: 600; line-height: 1.7; font-size: 0.9rem;">
            본 체크리스트는 천공 깊이/수직도 검수, 앵커 인장강도 확인, 무수축 모르타르 그라우트 주입, EVA Foam 오버플로우 확인, 미충전 시 즉시 세척 재작업 수칙, 전단키 덮개 시공 및 45 MPa 몰탈 강도를 검측하기 위해 작성되었습니다.
        </div>
    </div>
    ...
    <table>
        ...
        <tbody>
            <tr>
                <td class="category" style="color:var(--accent-orange);">⚠️ 사전 준비<br>(Step 1~2 천공/앵커)</td>
                <td>
                    <div style="margin-bottom: 10px;">
                        <span class="step-tag" style="background:#fef3c7; color:#92400e;">Step 1. 천공 & 청소</span>
                        <strong>[천공 깊이 게이지 & 수직도]</strong> 천공 깊이/수직도 게이지 측정 점검 및 고압 에어 노즐 <b>분진 이물질 100% 청소</b> 여부
                    </div>
                    <div style="margin-bottom: 4px;">
                        <span class="step-tag" style="background:#e0f2fe; color:#0369a1;">Step 2. 앵커 정위치</span>
                        <strong>[전단앵커 인장시험 성적]</strong> HBS/TCL층 일체화 <span class="term-highlight" onclick="openGlossary('shear_anchor')">전단앵커 정위치 세팅</span> 및 현장 앵커 인장강도 합격 여부
                    </div>
                </td>
                <td class="check-cell">☐ 확인완료</td>
            </tr>
            <tr>
                <td class="category" style="color:var(--accent-red);">⚡ 본 주입<br>(Step 3 무수축그라우트)</td>
                <td>
                    <div style="margin-bottom: 10px;">
                        <span class="step-tag" style="background:#dcfce7; color:#166534;">Step 3. EVA Foam 오버플로우</span>
                        <strong>[EVA Foam 유출 밀실주입]</strong> 무수축 모르타르 주입 시 <span class="term-highlight" onclick="openGlossary('eva_foam')">EVA Foam 구멍으로 몰탈 오버플로우 유출</span> 확인 및 연속 주입 여부
                    </div>
                    <div style="margin-bottom: 4px;">
                        <span class="step-tag" style="background:#fef2f2; color:#991b1b;">Step 3. 미충전 세척 수칙</span>
                        <strong>[미충전 시 경화 전 제거]</strong> 장비/재료 이상으로 미충전 발생 시 <b>경화 전 기 주입재 완전 제거 & 물청소 재작업</b> 시행 여부
                    </div>
                </td>
                <td class="check-cell">☐ 확인완료</td>
            </tr>
            <tr>
                <td class="category" style="color:var(--accent-green);">✅ 봉인 마감<br>(Step 4 덮개/45MPa)</td>
                <td>
                    <div style="margin-bottom: 10px;">
                        <span class="step-tag" style="background:#f0fdf4; color:#15803d;">Step 4. 전단키 덮개</span>
                        <strong>[전단키 덮개 시공 봉인]</strong> 충전층 경화 전 <span class="term-highlight" onclick="openGlossary('key_cap')">전단키 덮개 즉시 설치</span>로 수분 유출 및 이물질 침투 차단 여부
                    </div>
                    <div style="margin-bottom: 4px;">
                        <span class="step-tag" style="background:#f0fdf4; color:#15803d;">Step 4. 몰탈 45MPa 강도</span>
                        <strong>[공시체 45 MPa & 음향탐사]</strong> 28일 몰탈 공시체 압축강도 <span class="term-highlight" onclick="openGlossary('grout_strength')">&ge; 45 MPa(최소 30 MPa)</span> & 음향탐사 공동 파악 합격 여부
                    </div>
                </td>
                <td class="check-cell">☐ 확인완료</td>
            </tr>
    ...
    <div style="text-align: center; font-size: 0.85rem; color: #94a3b8; margin-top: 30px; border-top: 1px solid #e2e8f0; padding-top: 15px;">
        동탄도시철도(트램) 건설공사 | WBS 9000-6-16 콘크리트도상 [PST] 전단앵커설치 및 충전재 주입 마스터 체크리스트
    </div>
</div>
{common_modal_html}
</body>
...
print("\n🎉 SUCCESSFULLY BUILT WBS 16 MASTER HTML FILES!")
