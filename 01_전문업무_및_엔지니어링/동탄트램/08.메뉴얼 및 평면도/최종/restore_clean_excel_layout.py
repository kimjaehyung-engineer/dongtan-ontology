...
print(f"Restoring clean formatting & row heights for '지장물이설' sheet in '{target_v3}'...")
...
for r_idx, act in enumerate(jijangmul_activities_v3, start=3):
    ...
    row_data = [
        ...
        act['act'], act.get('owner', '현장 공사팀'), act.get('purpose', ''), act.get('method', ''), act.get('result', ''),
...
wb.save(target_v3)
print(f"🎉 Fully restored clean layout, proper row heights (42pt), borders & formatting in '{target_v3}'!")
