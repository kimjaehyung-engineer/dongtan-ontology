...
orig_img = "optical_power_meter_1310_1550_1785118411776.jpg"
target_img = "optical_power_meter_1310_1550.jpg"

src_p = os.path.join(artifact_dir, orig_img)
dst_p = os.path.join(gui_folder, target_img)

if os.path.exists(src_p):
    shutil.copy(src_p, dst_p)
    print(f"   ✓ [IMAGE COPIED] {target_img} -> {gui_folder}")
...
<script>
const glossaryData = {
    "RSRP": "<b>RSRP (Reference Signal Received Power: 기준 신호 수신 파워)</b><br><br>• <b>약어 풀이:</b> Reference Signal Received Power<br>• <b>개념:</b> LTE-R 기지국 안테나에서 쏘아주는 무선 신호 중 기준이 되는 신호를 트램 무전 장치가 얼마나 세게 받고 있는지 나타내는 객관적 수치입니다.<br>• <b>안전 의미:</b> <b>-95dBm 이상</b>을 유지해야 음영지역 없이 관제실과 24시간 끊김 없는 긴급 무전 및 열차 제어 데이터를 주고받아 대형 사고를 예방합니다.",
    "전계강도": "<b>전계강도 (Field Strength)</b><br><br>• <b>개념:</b> 공기 중으로 전파되는 무선 신호의 세기(Power)를 의미합니다. 스마트폰 안테나 막대 개수와 같은 개념입니다. (단위: dBm)",
    "OTDR": "<b>OTDR (Optical Time Domain Reflectometer: 광파동 측정기)</b><br><br>• 광케이블에 파장(1310/1550nm)의 빛을 쏘아 접속 손실(≤0.05dB) 및 절단 지점을 1cm 오차 없이 정밀 측정하는 전문 테스트 장비입니다.",
    "ITP": "<b>ITP (Inspection and Test Plan: 품질검사 계획서)</b><br><br>• 발주처 및 시방서 기준에 따라 자재 입고부터 시공 완료까지 시행할 품질 검사를 정의한 표준 서류입니다.",
    "ITC": "<b>ITC (Inspection Certificate: 품질 검측 승인서)</b><br><br>• 발주처 품질감독관, 현장 총괄 감리원, 통신 책임자 3인이 합격을 최종 승인하는 결재 서류입니다.",
    "융착접속": "<b>융착 접속 (Fusion Splicing)</b><br><br>• 머리카락만큼 가느다란 유리선 두 가닥을 2,000℃ 고온 전기 불꽃으로 녹여서 하나로 완벽히 붙이는 정밀 광케이블 연결 작업입니다.",
    "접속손실": "<b>접속 손실 (Optical Loss)</b><br><br>• 유리선 접속 부위의 먼지, 불완전 절단, 미세한 중심축 불일치로 인해 빛 신호가 밖으로 새어나가는 현상입니다. <b>≤0.05dB 이하</b>가 합격 수치입니다.",
    "헷지방안": "<b>손실 초과 시 현장 헷지(Hedge / 대책) 방안</b><br><br>• <b>1단계:</b> 접속 부위 즉시 절단 후 절삭기(Cleaver)로 단면 재정리 및 자동 융착 재시행<br>• <b>2단계:</b> 72가닥 중 손실이 발생하는 코어 대신 여분의 예비 코어(Spare Core) 채널로 즉시 우회 전환<br>• <b>3단계:</b> 접속 함체 알코올 세척 및 방수 재포장 조치",
    "광파워미터": "<b>광파워미터 (Optical Power Meter: OPM)</b><br><br>• 광케이블 건너편에서 쏘아준 레이저 빛의 세기를 수신 받아 <b>빛이 줄어들지 않고 도착했는지 측정하는 광 통신 전용 계측기</b>입니다.",
    "1310nm/1550nm": "<b>1310nm / 1550nm 파장 숫자의 의미</b><br><br>• <b>nm (나노미터):</b> 광케이블 내부를 지나는 레이저 빛의 파장(Wavelength) 길이입니다.<br>• <b>1310nm (신호 변형 최소 파장):</b> 데이터가 찌그러지는 분산 현상이 적어 단거리/중거리 전송에 정밀한 파장입니다.<br>• <b>1550nm (신호 손실 최소 파장):</b> 유리관을 지날 때 빛 세기가 옅어지는 감쇄가 가장 적어 장거리 전송에 최적인 파장입니다.<br>• <b>듀얼 파장 측정 이유:</b> 케이블 꺾임은 1550nm가, 미세 오염은 1310nm가 영향을 받으므로 <b>두 파장 모두 ≤0.05dB 이하</b>여야 100% 무결점으로 통과됩니다."
};
...
gui_full_opm_html = f"""<!DOCTYPE html>
<html lang="ko">
<head>
    <meta charset="UTF-8">
...
<div class="max-w-5xl mx-auto bg-white shadow-2xl rounded-2xl border border-slate-200 overflow-hidden">
    <div class="bg-slate-900 text-white p-8 relative">
        ...
        <h1 class="text-3xl font-black mt-2">발주처 품질 요구사항 검토 엔지니어링 수행지침서</h1>
        <p class="text-blue-200 text-sm mt-1">L4 Code: 9000-2-7 | 주관: 현장 시스템팀 / 통신 감리단 | "광파워미터 & 1310/1550nm 파장 해설 수록"</p>
    </div>
    ...
    <div class="p-6 sm:p-10 space-y-10">
        ...
        <div class="bg-blue-50 border border-blue-200 p-6 rounded-2xl text-sm text-blue-950 space-y-3">
            ...
            <p class="bg-white p-4 rounded-xl border border-blue-300 font-medium text-slate-900 leading-relaxed">
                본 지침서는 발주처 품질검사 지침(<span class="term-highlight" onclick="openGlossary('ITP')">ITP</span>/<span class="term-highlight" onclick="openGlossary('ITC')">ITC</span>)에 맞춰 <strong>품질 계획 사전검토, 광 접속 정밀측정, LTE-R 전파검증, ITC 서명체결 4단계 유연 프로세스</strong>로 체계화되었습니다.
                본문 속 <span class="term-highlight" onclick="openGlossary('전계강도')">전계강도</span>, <span class="term-highlight" onclick="openGlossary('RSRP')">RSRP</span>, <span class="term-highlight" onclick="openGlossary('OTDR')">OTDR</span>, <span class="term-highlight" onclick="openGlossary('광파워미터')">광파워미터</span>, <span class="term-highlight" onclick="openGlossary('1310nm/1550nm')">1310nm/1550nm</span> 용어를 클릭하면 <strong>친절한 용어 팝업 해설 창</strong>이 뜹니다.
            </p>
        </div>
        ...
        <div class="bg-blue-50/70 border border-blue-200 p-7 rounded-2xl shadow-md space-y-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                <div class="bg-white p-5 rounded-xl border border-slate-200 shadow-sm space-y-2">
                    ...
                    <p class="text-slate-700 text-xs">발주처 품질검사 계획서(ITP) 및 공사시방서의 품질 기준 사전 정밀 대조.</p>
                </div>
                <div class="bg-white p-5 rounded-xl border border-slate-200 shadow-sm space-y-2">
                    <span class="font-bold text-blue-700 flex items-center gap-2"><span>⚡</span> 2. 광케이블 접속 손실 0.05dB 측정</span>
                    <p class="text-slate-700 text-xs">OTDR 및 광파워미터(1310/1550nm)로 72가닥 전수 광 손실 0.05dB 이하 판정.</p>
                </div>
                <div class="bg-white p-5 rounded-xl border border-slate-200 shadow-sm space-y-2">
                    <span class="font-bold text-blue-700 flex items-center gap-2"><span>📶</span> 3. LTE-R 무선국 준공 검증</span>
                    <p class="text-slate-700 text-xs">스펙트럼 분석기로 전구간 수신 전계강도(RSRP ≥ -95dBm) 정밀 측정 및 필증 확보.</p>
                </div>
                <div class="bg-white p-5 rounded-xl border border-slate-200 shadow-sm space-y-2">
                    <span class="font-bold text-blue-700 flex items-center gap-2"><span>🖊️</span> 4. 품질 승인서(ITC) 최종 체결</span>
                    <p class="text-slate-700 text-xs">발주처·감리원·시스템 책임자 3자 서명 날인으로 검측 승인서(ITC) 체결.</p>
                </div>
            </div>
        </div>
        ...
        <div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-3">
                <div class="bg-blue-50 p-4 rounded-xl border border-blue-200 flex flex-col justify-between">
                    ...
                    <p class="text-[11px] text-blue-900 mt-1 font-medium">• 품질계획서 대조<br">• 감리 입회점 지정</p>
                </div>
                <div class="bg-indigo-50 p-4 rounded-xl border border-indigo-200 flex flex-col justify-between">
                    ...
                    <p class="text-[11px] text-indigo-900 mt-1 font-medium">• 광 손실 ≤0.05dB<br">• 72Core 전수 측정</p>
                </div>
                <div class="bg-cyan-50 p-4 rounded-xl border border-cyan-200 flex flex-col justify-between">
                    ...
                    <p class="text-[11px] text-cyan-900 mt-1 font-medium">• 수신 전계 ≥-95dBm<br">• 무선국 준공검사</p>
                </div>
                <div class="bg-emerald-50 p-4 rounded-xl border border-emerald-200 flex flex-col justify-between">
                    ...
                    <p class="text-[11px] text-emerald-900 mt-1 font-medium">• ITC 3자 서명<br">• 품질대장 확정</p>
                </div>
            </div>
        </div>

        <!-- 🔥 2. 세부 실무 가이드 (STEP 1~4 1:1 직관적 2D VISUAL SVG) -->
        <div class="space-y-8">
            <h2 class="text-xl font-bold text-slate-900 mb-4 border-b-2 border-indigo-600 pb-2">
                <span class="text-indigo-600">2.</span> 엔지니어링 세부 실무 가이드 & 1:1 2D 그림
            </h2>
            ...
            <div class="space-y-8 text-sm">
                ...
                <div class="bg-white p-6 rounded-2xl border border-blue-200 shadow-sm space-y-4">
                    <div class="flex items-center gap-3">
                        ...
                        <h3 class="font-bold text-base text-slate-900">발주처 품질검사 계획서(<span class="term-highlight" onclick="openGlossary('ITP')">ITP</span>/<span class="term-highlight" onclick="openGlossary('ITC')">ITC</span>) 사전 정밀 대조 & 감리 입회점 지정</h3>
                    </div>
                    <div class="bg-slate-50 p-5 rounded-xl text-xs text-slate-700 space-y-3 leading-relaxed border border-slate-200">
                        <p><strong>💡 핵심 실무 지침:</strong></p>
                        <ul class="list-disc pl-5 space-y-1.5 text-slate-800">
                            <li><strong>품질 계획 검토:</strong> 발주처 품질검사 계획서(ITP: Inspection and Test Plan) 및 공사시방서의 품질 기준을 사전 정밀 대조합니다.</li>
                            <li><strong>자재 검수 체계:</strong> 광케이블, 4K CCTV, PIS/PA, LTE-R 안테나 등 주요 통신 자재 입고 시 공장시험 성적서(Mill Test Report)의 적합성을 승인합니다.</li>
                            <li><strong>감리 입회점(Hold Point) 지정:</strong> 감리원 현장 직접 입회 검측 항목(Hold Point / Witness Point)을 사전 분류하여 검측 누락을 원천 차단합니다.</li>
                        </ul>
                    </div>
                    ...
                    <div class="clickable-diagram bg-slate-50 p-4 rounded-xl border border-slate-200 mt-3" onclick="openDiagramZoom('svg_step1', 'STEP 1 ITP/ITC 서류 대조 2D 시공 도식')">
                ...
                <!-- STEP 2 HOW + VISUAL SVG (72-Core 광 접속, 헷지방안 & 광파워미터 1310/1550nm 해설 수록) -->
                <div class="bg-white p-6 rounded-2xl border border-indigo-200 shadow-sm space-y-4">
                    <div class="flex items-center gap-3">
                        ...
                        <h3 class="font-bold text-base text-slate-900">72-Core 메인 광케이블 <span class="term-highlight" onclick="openGlossary('융착접속')">융착 접속</span> 손실 0.05dB 이하 정밀 측정 시험</h3>
                    </div>

                    <!-- 📖 초심자를 위한 72-Core 광 접속 및 헷지 방안 해설 박스 -->
                    <div class="bg-indigo-50 border border-indigo-300 p-5 rounded-xl text-xs text-slate-800 space-y-3 shadow-inner">
                        <h4 class="font-black text-sm text-indigo-950 flex items-center gap-1.5">📖 [초심자 해설] 72-Core 광 접속의 의미, 하자 위험, 손실 원인 & 헷지(Hedge / 대책) 방안</h4>
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-3 bg-white p-4 rounded-lg border border-indigo-200">
                            <div>
                                <span class="font-bold text-indigo-900 block mb-1">⚡ 1. 72-Core & 융착 접속(Fusion Splicing)이란?</span>
                                <p class="text-slate-700 leading-relaxed">
                                    • <b>72-Core:</b> 머리카락만큼 가느다란 유리 파이버 72가닥이 들어있는 <b>트램의 메인 통신 고속도로(핏줄)</b>입니다.<br>
                                    • <b>융착 접속:</b> 두 유리 단면을 <b>2,000℃ 고온 전기 불꽃으로 녹여서 하나로 완벽히 붙이는 정밀 공정</b>입니다.
                                </p>
                            </div>
                            <div>
                                <span class="font-bold text-indigo-900 block mb-1">🚨 2. 융착 불량 시 발생하는 하자 위험</span>
                                <p class="text-slate-700 leading-relaxed">
                                    • <b>화면 끊김/멈춤:</b> 유리 연결부 불량으로 빛이 새어나가 4K CCTV 및 승강장 안내 전광판 화면이 멈춥니다.<br>
                                    • <b>통신 블랙아웃:</b> 융착이 나쁘면 열차 제어 통신이 차단되어 <b>대형 통신 장애</b>가 발생합니다.
                                </p>
                            </div>
                        </div>
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-3 bg-white p-4 rounded-lg border border-indigo-200">
                            <div>
                                <span class="font-bold text-amber-900 block mb-1">🔍 3. 1개 코어당 접속 손실(Optical Loss) 원인</span>
                                <p class="text-slate-700 leading-relaxed">
                                    ① <b>단면 오염:</b> 유리선 끝에 묻은 미세 먼지/기름때<br>
                                    ② <b>절단 각도 불량:</b> 칼날(Cleaver)로 자를 때 단면이 삐뚤어짐<br>
                                    ③ <b>중심축 불일치:</b> 두 유리선의 빛 통로 중심축이 미세 편심 발생
                                </p>
                            </div>
                            <div>
                                <span class="font-bold text-emerald-900 block mb-1">🛡️ 4. 기준 초과(>0.05dB) 시 현장 <span class="term-highlight" onclick="openGlossary('헷지방안')">헷지(Hedge)</span> 방안</span>
                                <p class="text-slate-700 leading-relaxed">
                                    • <b>1단계 (즉시 재접속):</b> 접속 부위 절단 후 정밀 절삭기 재정리 및 자동 융착 재시행<br>
                                    • <b>2단계 (예비 Core 전환):</b> 72가닥 중 손실된 코어 대신 <b>여분의 예비 코어(Spare Core)</b>로 우회 포트 교체<br>
                                    • <b>3단계 (함체 재정비):</b> 접속 함체 알코올 클리닝 및 습기 재포장
                                </p>
                            </div>
                        </div>
                    </div>

                    <!-- 📖 초심자를 위한 광파워미터 및 1310/1550nm 파장 숫자 의미 해설 박스 -->
                    <div class="bg-blue-50 border border-blue-300 p-5 rounded-xl text-xs text-slate-800 space-y-3 shadow-inner">
                        <h4 class="font-black text-sm text-blue-950 flex items-center gap-1.5">📖 [초심자 해설] 광파워미터란 무엇이며 1310nm / 1550nm 숫자가 의미하는 바는?</h4>
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-3 bg-white p-4 rounded-lg border border-blue-200">
                            <div>
                                <span class="font-bold text-blue-900 block mb-1">💡 1. 광파워미터(Optical Power Meter: OPM)란?</span>
                                <p class="text-slate-700 leading-relaxed">
                                    광케이블 반대쪽에서 쏘아준 레이저 빛의 세기를 측정하여 <b>"빛의 파워가 줄어들지 않고 도착했는지"</b> 정밀하게 계측하는 광 통신 전용 빛 측정기입니다.
                                </p>
                            </div>
                            <div>
                                <span class="font-bold text-blue-900 block mb-1">🔍 2. 1310nm / 1550nm 숫자의 의미 (파장 길이)</span>
                                <p class="text-slate-700 leading-relaxed">
                                    • <b>nm (나노미터):</b> 광케이블 속을 통과하는 <b>레이저 빛의 파장(Wavelength) 길이</b>입니다.<br>
                                    • <b>1310nm (신호 변형 최소 파장):</b> 빛의 퍼짐(분산)이 적어 데이터 전송이 깨끗합니다.<br>
                                    • <b>1550nm (신호 손실 최소 파장):</b> 유리관을 지날 때 빛 감쇄(손실)가 가장 적습니다.
                                </p>
                            </div>
                        </div>
                        <div class="bg-white p-4 rounded-lg border border-blue-200">
                            <span class="font-bold text-indigo-900 block mb-1">❓ 왜 1310nm와 1550nm 두 가지 파장을 다 측정하나요?</span>
                            <p class="text-slate-700 leading-relaxed">
                                케이블이 꺾였을 때 1550nm 빛이 손실을 크게 받고, 미세 먼지가 묻었을 때 1310nm 빛이 손실을 크게 받습니다.<br>
                                따라서 <b>두 가지 파장 모두에서 손실이 ≤0.05dB 이하</b>임을 확인해야 어떤 환경에서도 100% 완벽한 광 통신선임이 증명됩니다.
                            </p>
                        </div>
                    </div>
                    ...
                    <div class="bg-slate-50 p-5 rounded-xl text-xs text-slate-700 space-y-3 leading-relaxed border border-slate-200">
                        <p><strong>💡 핵심 실무 지침:</strong></p>
                        <ul class="list-disc pl-5 space-y-1.5 text-slate-800">
                            <li><strong>광 접속 검측:</strong> 72-Core 메인 광케이블 융착 접속 완료 후, <span class="term-highlight" onclick="openGlossary('OTDR')">OTDR</span>(광파동 측정기) 및 <span class="term-highlight" onclick="openGlossary('광파워미터')">광파워미터</span>(<span class="term-highlight" onclick="openGlossary('1310nm/1550nm')">1310/1550nm</span>)를 통해 측정합니다.</li>
                            <li><strong>품질 기준 검증:</strong> 1개 Core당 <span class="term-highlight" onclick="openGlossary('접속손실')">접속 손실</span>이 <strong>≤0.05dB(현장 관리 목표: 0.01 ~ 0.04dB)</strong>를 만족하는지 전수 검측하고 측정 결과 리포트를 제출합니다.</li>
                        </ul>
                    </div>

                    <!-- 📸 STEP 2 실제 장비 사진 (OTDR & 광파워미터 2종 갤러리) -->
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-3 mt-4">
                        <div class="bg-slate-50 p-3 rounded-xl border border-slate-200">
                            <span class="text-xs font-bold text-indigo-900 block mb-2">📸 실무 측정 장비 사진: OTDR(광파동 측정기)</span>
                            <img src="./otdr_power_meter.jpg" alt="OTDR 측정 장비 실물 사진" class="w-full h-44 object-cover rounded-lg border border-slate-300 shadow-sm cursor-pointer" onclick="openDiagramZoom('img_step2_tag', 'STEP 2 OTDR 측정 장비 현장 사진')">
                            <img id="img_step2_tag" src="./otdr_power_meter.jpg" alt="OTDR 측정 장비" class="hidden">
                        </div>
                        <div class="bg-slate-50 p-3 rounded-xl border border-slate-200">
                            <span class="text-xs font-bold text-blue-900 block mb-2">📸 실무 측정 장비 사진: 광파워미터(1310/1550nm 디지털 화면)</span>
                            <img src="./optical_power_meter_1310_1550.jpg" alt="광파워미터 실물 사진" class="w-full h-44 object-cover rounded-lg border border-slate-300 shadow-sm cursor-pointer" onclick="openDiagramZoom('img_opm_tag', 'STEP 2 광파워미터(1310/1550nm) 실물 장비 사진')">
                            <img id="img_opm_tag" src="./optical_power_meter_1310_1550.jpg" alt="광파워미터 장비" class="hidden">
                        </div>
                    </div>
                ...
                <!-- STEP 3 HOW + VISUAL SVG (RSRP & 전계강도 초심자 해설 박스 수록) -->
                <div class="bg-white p-6 rounded-2xl border border-cyan-200 shadow-sm space-y-4">
                    <div class="flex items-center gap-3">
                        ...
                        <h3 class="font-bold text-base text-slate-900">LTE-R 안테나 수신 <span class="term-highlight" onclick="openGlossary('전계강도')">전계강도</span> & 무선국 준공검사 전파 품질 측정</h3>
                    </div>
                    
                    <!-- 📖 초심자를 위한 전계강도 & RSRP 개념 및 안전적 의미 해설 박스 -->
                    <div class="bg-cyan-50 border border-cyan-300 p-5 rounded-xl text-xs text-slate-800 space-y-3 shadow-inner">
                        <h4 class="font-black text-sm text-cyan-950 flex items-center gap-1.5">📖 [초심자 해설] 전계강도 및 RSRP 개념과 열차 안전에서의 의미</h4>
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-3 bg-white p-4 rounded-lg border border-cyan-200">
                            <div>
                                <span class="font-bold text-cyan-900 block mb-1">📡 1. 전계강도(Field Strength)란?</span>
                                <p class="text-slate-700 leading-relaxed">공기 중을 날아다니는 <b>무선 전파 신호의 세기(Power)</b>입니다. 스마트폰 상단의 안테나 막대가 몇 개 채워지는지와 같은 개념입니다. (단위: dBm)</p>
                            </div>
                            <div>
                                <span class="font-bold text-cyan-900 block mb-1">🔍 2. RSRP의 의미 (약어 풀이)</span>
                                <p class="text-slate-700 leading-relaxed"><b>RSRP = Reference Signal Received Power</b> (기준 신호 수신 파워). 기지국 안테나가 쏘아주는 무선 신호를 얼마나 세게 수신하는지 나타내는 <b>객관적 정밀 수치</b>입니다.</p>
                            </div>
                        </div>
                        <div class="bg-white p-4 rounded-lg border border-cyan-200">
                            <span class="font-bold text-rose-700 block mb-1">🚨 3. 왜 필요하며 어떤 안전적 의미가 있는가?</span>
                            <p class="text-slate-700 leading-relaxed">
                                트램은 노선(정거장·교차로·터널)을 달리며 관제실과 <b>24시간 끊김 없이 긴급 무전 및 열차 제어 데이터</b>를 주고받아야 합니다.<br>
                                만약 전계강도가 <b>-95dBm 밑으로 떨어지면 전파 음영지역이 생겨 무전이 끊기고 긴급 제어 명령 수신이 불가능</b>해집니다.<br>
                                <b>RSRP ≥ -95dBm 검증</b>은 <b>"동탄트램 전구간에서 24시간 100% 무전 끊김 없이 안전하게 운행할 수 있다"</b>는 것을 입증하는 필수 검증입니다.
                            </p>
                        </div>
                    </div>
                    ...
                    <div class="bg-slate-50 p-5 rounded-xl text-xs text-slate-700 space-y-3 leading-relaxed border border-slate-200">
                        <p><strong>💡 핵심 실무 지침:</strong></p>
                        <ul class="list-disc pl-5 space-y-1.5 text-slate-800">
                            <li><strong>전파 품질 검증:</strong> LTE-R 무선망 전파 측정 장비(스펙트럼 분석기)를 활용하여 정거장, 교차로, 터널 등 전구간 수신 전계강도를 측정합니다.</li>
                            <li><strong>무선국 필증 확보:</strong> 수신 전계강도가 <span class="term-highlight" onclick="openGlossary('RSRP')">RSRP</span> <strong>≥ -95dBm</strong>을 만족하는지 검사하고, 전파법에 따른 국가 공인 무선국 준공검사 적합 필증을 확보합니다.</li>
                        </ul>
                    </div>
                ...
                <div class="bg-white p-6 rounded-2xl border border-emerald-200 shadow-sm space-y-4">
                    <div class="flex items-center gap-3">
                        ...
                        <h3 class="font-bold text-base text-slate-900">발주처·감리원 품질 검측 승인서(<span class="term-highlight" onclick="openGlossary('ITC')">ITC</span>) 최종 체결 & 대장 확정</h3>
                    </div>
                    <div class="bg-slate-50 p-5 rounded-xl text-xs text-slate-700 space-y-3 leading-relaxed border border-slate-200">
                        <p><strong>💡 핵심 실무 지침:</strong></p>
                        <ul class="list-disc pl-5 space-y-1.5 text-slate-800">
                            <li><strong>ITC 승인 체결:</strong> 모든 ITP 검측 항목 통과 시, 발주처 품질감독관, 현장 총괄 감리원, 통신 책임자 3자 서명 날인으로 최종 검측 승인서(ITC)를 체결합니다.</li>
                            <li><strong>대장 확정 관리:</strong> 지적 사항 결함 조치서를 수반하여 최종 준공 품질 관리대장에 완결 반영합니다.</li>
                        </ul>
                    </div>
...
for fn in os.listdir(gui_folder):
    if fn.endswith('.html'):
        ...
        with open(fp, 'w', encoding='utf-8') as f:
            f.write(gui_full_opm_html)
        print(f"   ✓ [OPM 1310/1550nm GLOSSARY & IMAGE ADDED] Guideline -> {fn}")

print("\n🎉 SUCCESSFULLY ADDED OPTICAL POWER METER IMAGE & 1310/1550nm WAVELENGTH EXPLANATIONS TO WBS 9000-2-7 GUIDELINE HTMLs!")
