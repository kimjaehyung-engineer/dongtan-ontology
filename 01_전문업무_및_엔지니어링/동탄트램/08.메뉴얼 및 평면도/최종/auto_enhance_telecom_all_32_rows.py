...
excel_path = r"c:\Users\sskjh\antigravity\01_전문업무_및_엔지니어링\동탄트램\매뉴얼 BODY (집행단계)v4.xlsx"
base_dir = r"c:\Users\sskjh\antigravity\01_전문업무_및_엔지니어링\동탄트램\08.메뉴얼 및 평면도\최종\매뉴얼BODY(집행단계-첨부폴더)\통신분야"

wb = openpyxl.load_workbook(excel_path, data_only=True)
sheet = wb["통신분야"]

print("=== STARTING AUTOMATED SEQUENTIAL ENHANCEMENT FOR 32 TELECOM ACTIVITIES (ROW 2 TO 33) ===")
...
for r in range(2, 34):
    ...
    # Parse methods into bullet points for checklist & guidelines
    method_lines = [m.strip() for m in str(method).split('\n') if m.strip()]
    if not method_lines:
        method_lines = [f"{act_clean} 관련 현장 시공 및 시방 수칙 준수여부 검측"]

    # Generate 12 interrogative checklist items ending in "~하였는가?"
    checklist_items = []
    ...
    # Pad checklist to at least 10 items
    extra_checks = [
    ...
    # Determine Flexible Step Count Architecture (3 to 5 steps based on method count)
    step_count = max(3, min(5, len(method_lines)))
    
    # Generate 1) STANDARD HTML
...
"""
    ...
    # Generate 2) GUIDELINE HTML (Flexible Steps + Light Theme Cards + 2D Visual SVG)
...
"""
    ...
    # Generate 3) CHECKLIST HTML (~하였는가? 100%)
...
"""
    ...
    # Overwrite into all subdirectories (표준서, 수행지침, 체크리스트)
    sub_map = {
        "표준서": std_html,
        "수행지침": gui_html,
        "체크리스트": chk_html
...
print(f"\n🎉 COMPLETED ALL {processed_count} TELECOM ACTIVITIES (ROW 2 TO 33) AUTOMATICALLY WITHOUT INTERVENTION!")
