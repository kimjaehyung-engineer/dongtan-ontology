...
xlsx_files = [os.path.join(base_dir, f) for f in os.listdir(base_dir) if f.endswith('.xlsx') and not f.startswith('~$')]

v3_path = None
for f_path in xlsx_files:
    if 'v3' in os.path.basename(f_path):
        v3_path = f_path
        break

if not v3_path and xlsx_files:
    v3_path = xlsx_files[0]

v4_path = os.path.join(base_dir, "매뉴얼 BODY (집행단계)v4.xlsx")

print(f"Source V3 File: {v3_path}")
print(f"Target V4 File: {v4_path}")

shutil.copy2(v3_path, v4_path)
print(f"Copied base file to '{v4_path}'")
