...
rich_kom_gui_html = f"""<!DOCTYPE html>
<html lang="ko">
<head>
    <meta charset="UTF-8">
...
<div class="max-w-5xl mx-auto bg-white shadow-2xl rounded-2xl border border-slate-200 overflow-hidden">
    ...
    <div class="p-6 sm:p-10 space-y-10">
        ...
        <!-- 🔥 8대 현장조건 상세 친절 해설 특화 섹션 (신설) -->
        <div class="bg-slate-900 text-white p-7 rounded-2xl shadow-xl space-y-6">
            <div class="border-b border-slate-700 pb-4">
                <span class="bg-sky-500 text-slate-950 text-xs font-black px-3 py-1 rounded-full uppercase">Special Focus</span>
                <h3 class="text-xl font-black text-white mt-2 flex items-center gap-2">
                    <span>📋</span> 동탄트램 통신공사 필수 반영 『8대 현장조건』 정밀 해설
                </h3>
                <p class="text-slate-400 text-xs sm:text-sm mt-1">발주 착수 전 예산 누락 및 분쟁을 사전에 100% 방지하기 위해 정밀 계상해야 하는 8가지 실무 수칙</p>
            </div>

            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 text-xs sm:text-sm">
                <!-- 1. 자재 제작/수급 기간 -->
                <div class="bg-slate-800 p-4 rounded-xl border border-slate-700 space-y-1.5">
            ...
            for f in os.listdir(gui_p):
                if f.endswith('.html'):
                    with open(os.path.join(gui_p, f), 'w', encoding='utf-8') as out:
                        out.write(rich_kom_gui_html)
                    print("OVERWROTE ENRICHED 8-CONDITIONS HTML FOR:", f)

print("\n🎉 SUCCESSFULLY REFLECTED ALL 8 SITE CONDITIONS WITH DETAILED EXPLANATIONS INTO TARGET GUIDELINE HTML FILES!")
