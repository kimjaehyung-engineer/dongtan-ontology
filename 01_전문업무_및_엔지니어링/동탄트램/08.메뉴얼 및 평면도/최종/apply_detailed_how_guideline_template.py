...
sname = [s for s in wb.sheetnames if '통신' in s][0]
sheet = wb[sname]
...
for r in range(2, 34):
    ...
    if act_name:
        clean_act = str(act_name).strip()
        excel_data[r - 1] = {
            "row": r,
            "l4_code": str(l4_code).strip(),
            "act_name": clean_act,
            "ju_gwan": str(ju_gwan).strip(),
...
const glossaryData = {
    'sleeve_penetration': {
        title: '🧱 콘크리트 타설 전 관통 슬리브 매설 수칙',
        desc: '동탄트램 본선 및 정거장 골조 타설 전 72-Core 광케이블 및 전원선 관입용 슬리브(ø100~ø150mm)를 철근 지지대에 용접 고정하여 오차 ±10mm 이내로 정밀 매설하는 수칙입니다.'
    },
    'equipment_room_height': {
        title: '🏢 통신기계실·관제실·TPS실 마감 층고 실측',
        desc: '통신 랙(Height 2200mm) 및 상부 트레이 적재 공간 확보를 위해 레이저 측정기로 천장 마감 층고 최소 3.0m 이상 확보 및 이중바닥(Access Floor H=300mm) 규격을 검증합니다.'
    },
    'fire_stop_seal': {
        title: '🔥 방화 구속(Fire-Stop) 밀폐 씰링',
        desc: '벽체 및 슬라브 관통 부위에 내화 2시간 이상의 인증 방화재료(Fire-Stop Calking)를 치밀하게 관입하고 수밀/가스밀 패드로 밀폐 시공하는 필수 안전 수칙입니다.'
    }
};
...
def build_guideline_html(act_idx, item):
    act_name = item["act_name"]
    l4_code = item["l4_code"]
    ju_gwan = item["ju_gwan"]
    purpose = item["purpose"]
    method_str = item["method"]
    deliverable = item["deliverable"]

    m_lines = [line.strip() for line in method_str.split('\n') if line.strip()]
    m1 = m_lines[0] if len(m_lines) > 0 else f"{act_name} 시방 기준"
    m2 = m_lines[1] if len(m_lines) > 1 else "현장 실측 및 인터페이스 검증"
...
<div class="max-w-5xl mx-auto bg-white shadow-2xl rounded-2xl border border-slate-200 overflow-hidden">
    ...
    <div class="bg-slate-900 text-white p-8 relative">
        ...
        <h1 class="text-3xl font-black mt-2">{act_name} 초정밀 실무 수행지침서</h1>
        <p class="text-blue-200 text-sm mt-1">L4 Code: {l4_code} | 주관: {ju_gwan} | "어떻게(HOW) 수행하는가 초정밀 가이드"</p>
    </div>
    ...
    <div class="p-6 sm:p-10 space-y-10">
        ...
        <div class="bg-blue-50 border border-blue-200 p-6 rounded-2xl text-sm text-blue-950 space-y-3">
            <h4 class="font-bold text-base flex items-center gap-2">💡 {act_name} 현장 실무 개요 및 수칙</h4>
            <p class="bg-white p-4 rounded-xl border border-blue-300 font-medium text-slate-900 leading-relaxed">
                {purpose}. 본 지침서는 현장 엔지니어가 작업 단계별로 <strong>어떻게(HOW) 사전 준비, 실측, 시공 매설, 씰링 및 최종 서명</strong>을 수행해야 하는지 구체적 기술 수치와 행동 수칙을 정밀하게 제공합니다. 
        ...
        <div class="bg-blue-50/70 border border-blue-200 p-7 rounded-2xl shadow-md space-y-6">
            <div class="border-b border-blue-200 pb-4">
                ...
                <h3 class="text-xl font-black text-blue-950 mt-2">📋 {act_name} 4대 핵심 실무 검토 가이드</h3>
            </div>
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                <div class="bg-white p-5 rounded-xl border border-slate-200 shadow-sm space-y-2">
                    <span class="font-bold text-blue-700 flex items-center gap-2"><span>🧱</span> 1. {m1[:25]}</span>
                    <p class="text-slate-700 text-xs">콘크리트 타설 전 통신 관통 관로 및 방수 슬리브 위치 1:1 대조 확정.</p>
                </div>
                <div class="bg-white p-5 rounded-xl border border-slate-200 shadow-sm space-y-2">
                    <span class="font-bold text-blue-700 flex items-center gap-2"><span>🏢</span> 2. {m2[:25]}</span>
        ...
        <div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-5 gap-2">
                ...
                <div class="bg-emerald-50 p-3 rounded-xl border border-emerald-200 flex flex-col justify-between">
                    ...
                    <p class="text-[10px] text-emerald-900 mt-1 font-medium">• 회의록 서명<br">• {deliverable[:12]}</p>
                </div>
            </div>
        </div>

        <!-- 🔥 2. 초정밀 HOW(어떻게 수행하는가) 세부 실무 가이드 (NEW EXPANDED SECTION) -->
        <div class="space-y-6">
        ...
        <div class="space-y-6">
            ...
            <div class="clickable-diagram bg-slate-50 p-5 rounded-2xl border border-slate-200 shadow-inner" onclick="openDiagramZoom('svg_r3_{act_idx}', '[WBS {l4_code}] {act_name} 2D Visual 정밀 도식')">
                <svg id="svg_r3_{act_idx}" viewBox="0 0 550 180" width="100%" height="180" xmlns="http://www.w3.org/2000/svg">
                    ...
                    <text x="140" y="45" font-size="13" font-weight="black" fill="#1d4ed8" text-anchor="middle">🧱 사전 도면대조 & 슬리브 매설</text>
                    <text x="50" y="75" font-size="11" font-weight="bold" fill="#334155">• 타설 전 통신관 매설 위치 1:1</text>
                    <text x="50" y="98" font-size="11" font-weight="bold" fill="#334155">• 오차 ±10mm 이내 레이저 검측</text>
                    ...
                    <text x="412" y="45" font-size="13" font-weight="black" fill="#047857" text-anchor="middle">🏢 현장 층고 실측 & 3자 서명</text>
                    <text x="325" y="75" font-size="11" font-weight="bold" fill="#334155">• 기계실/관제실 층고 ≥ 3.0m</text>
                    <text x="325" y="98" font-size="11" font-weight="bold" fill="#334155">• Fire-Stop 2시간 씰링 체결</text>
                    <text x="275" y="162" font-size="13" font-weight="black" fill="#0f172a" text-anchor="middle">{act_name} 회의록 및 1,2공구 종합 관리대장 등재 완료</text>
                </svg>
            </div>
        </div>
    </div>
...
all_act_folders = sorted([f for f in os.listdir(base_dir) if os.path.isdir(os.path.join(base_dir, f))])

updated_count = 0

for act_folder_name in all_act_folders:
    prefix_str = act_folder_name.split('_')[0]
    if prefix_str.isdigit():
        act_idx = int(prefix_str)
    else:
        continue
        
    item = excel_data.get(act_idx)
    if not item:
        continue

    full_act_path = os.path.join(base_dir, act_folder_name)
    gui_dir = os.path.join(full_act_path, "수행지침")
    
    if os.path.exists(gui_dir):
        html_content = build_guideline_html(act_idx, item)
        for fn in os.listdir(gui_dir):
            if fn.endswith('.html'):
                ...
                updated_count += 1
                print(f"✓ [{act_idx:02d}] Updated Guideline HTML (HOW Expanded) -> {fn}")

print(f"\n🎉 SUCCESSFULLY UPDATED ALL {updated_count} TELECOM GUIDELINE HTML FILES WITH DETAILED HOW EXECUTION GUIDES!")
