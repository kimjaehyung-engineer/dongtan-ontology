...
# GUIDELINE HTML (ELP ø150mm 33% Fill Ratio Visualized)
gui_html = f"""<!DOCTYPE html>
<html lang="ko">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>동탄트램 통신분야 - 전기 / 신호 / 기계 / PSD / 차량 인터페이스 협의 수행지침서 (KEC 33% 점유율 visual 반영)</title>
...
<div class="max-w-5xl mx-auto bg-white shadow-2xl rounded-2xl border border-slate-200 overflow-hidden">
    <div class="bg-slate-900 text-white p-8 relative">
        <span class="bg-blue-600 text-white text-xs font-bold px-3 py-1 rounded-full uppercase">Dongtan Tram Detailed Guideline (KEC 33% 점유율 visual 반영)</span>
    ...
    <div class="p-6 sm:p-10 space-y-10">
        ...
        <!-- 🔥 2. 초정밀 HOW 세부 실무 가이드 (KEC 33% 점유율 visual 반영) -->
        <div class="space-y-8">
            ...
            <div class="space-y-8 text-sm">
                <!-- STEP 1 HOW + KEC 33% Fill Ratio Visualized SVG -->
                <div class="bg-white p-6 rounded-2xl border border-blue-200 shadow-sm space-y-4">
                    ...
                    <!-- STEP 1 33% Fill Ratio Visualized Diagram -->
                    <div class="clickable-diagram bg-slate-50 p-4 rounded-xl border border-slate-200 mt-3" onclick="openDiagramZoom('svg_step1', 'STEP 1 KEC 33% 점유율 시각화 & 96Core 광케이블/ELP ø150mm 2D 시공 도식')">
                        <svg id="svg_step1" viewBox="0 0 520 200" width="100%" height="200" xmlns="http://www.w3.org/2000/svg">
                            ...
                            <!-- ELP ø150mm 배관 6개 + 내부 KEC 33% 점유율(약 1/3 파랑 채움) 시각화 -->
                            ...
                            <!-- Fixed 96Core 광케이블 박스 (height 165, text padded) -->
...
# CHECKLIST HTML (Fill Ratio Visualized SVG)
...
<div class="max-w-5xl mx-auto space-y-6">
    ...
    <!-- 숨겨진 현장 점검용 Step 1~3 2D Visual SVG 소스 (KEC 33% Fill Ratio Visualized) -->
        ...
        for fn in os.listdir(sp):
            if fn.endswith('.html'):
                with open(os.path.join(sp, fn), 'w', encoding='utf-8') as f_out:
                    f_out.write(content)
                print(f"   ✓ [VISUALIZED KEC 33% FILL RATIO] {s_n} -> {fn}")

print("\n🎉 SUCCESSFULLY VISUALIZED KEC 33% CONDUIT FILL RATIO IN WBS 9000-2-4 SVG DIAGRAMS!")
