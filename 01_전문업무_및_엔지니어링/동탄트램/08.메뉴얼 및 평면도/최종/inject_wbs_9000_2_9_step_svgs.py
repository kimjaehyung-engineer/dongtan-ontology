...
target_dir = r"c:\Users\sskjh\antigravity\01_전문업무_및_엔지니어링\동탄트램\08.메뉴얼 및 평면도\최종\매뉴얼BODY(집행단계-첨부폴더)\통신분야\9_착수 전 Big Room 회의\수행지침"
...
gui_wbs_9000_2_9_html = f"""<!DOCTYPE html>
<html lang="ko">
<head>
    <meta charset="UTF-8">
...
<div class="max-w-5xl mx-auto bg-white shadow-2xl rounded-2xl border border-slate-200 overflow-hidden">
    ...
    <div class="bg-slate-900 text-white p-8 relative">
        ...
        <p class="text-blue-200 text-sm mt-1">L4 Code: 9000-2-9 | 주관: 현장소장 | "1:1 STEP별 직관적 2D 그림 수록 가이드"</p>
    </div>
    ...
    <div class="p-6 sm:p-10 space-y-10">
        ...
        <div class="bg-blue-50 border border-blue-200 p-6 rounded-2xl text-sm text-blue-950 space-y-3">
            <h4 class="font-bold text-base flex items-center gap-2">💡 착수 전 Big Room 회의 현장 실무 개요 및 수칙</h4>
            <p class="bg-white p-4 rounded-xl border border-blue-300 font-medium text-slate-900 leading-relaxed">
                통신공사 수행 시, 선행공정 간섭사항 및 각종 Risk에 대한 대책 수립. 본 지침서는 현장 엔지니어가 작업 단계별로 <strong>어떻게(HOW) 사전 준비, 실측, 시공 매설, 씰링 및 최종 서명</strong>을 수행해야 하는지 구체적 기술 수치와 행동 수칙을 정밀하게 제공합니다. 
                특히 <strong><span class="term-highlight" onclick="openGlossary('sleeve_penetration')">관통 슬리브 매설</span></strong>, <strong><span class="term-highlight" onclick="openGlossary('equipment_room_height')">기능실 층고 실측</span></strong> 및 <strong><span class="term-highlight" onclick="openGlossary('fire_stop_seal')">방화 구속(Fire-Stop)</span></strong>을 완벽히 수록하였습니다.
            </p>
        </div>
        ...
        <div class="bg-blue-50/70 border border-blue-200 p-7 rounded-2xl shadow-md space-y-6">
            <div class="border-b border-blue-200 pb-4">
                ...
                <h3 class="text-xl font-black text-blue-950 mt-2">📋 착수 전 Big Room 회의 4대 핵심 실무 검토 가이드</h3>
            </div>
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                <div class="bg-white p-5 rounded-xl border border-slate-200 shadow-sm space-y-2">
                    <span class="font-bold text-blue-700 flex items-center gap-2"><span>🧱</span> 1. 본사/현장/협력사 협의체 구축 후 상세검토</span>
                    <p class="text-slate-700 text-xs">콘크리트 타설 전 통신 관통 관로 및 방수 슬리브 위치 1:1 대조 확정.</p>
                </div>
                <div class="bg-white p-5 rounded-xl border border-slate-200 shadow-sm space-y-2">
                    <span class="font-bold text-blue-700 flex items-center gap-2"><span>🏢</span> 2. 기능실 마감 층고 & 타공 검측</span>
                    <p class="text-slate-700 text-xs">기계실, 관제실, 앰프실, TPS실 천장 층고(≥3.0m) 및 타공 규격 확인.</p>
                </div>
                <div class="bg-white p-5 rounded-xl border border-slate-200 shadow-sm space-y-2">
                    <span class="font-bold text-blue-700 flex items-center gap-2"><span>🚛</span> 3. 자재 반입 동선 & 공용구</span>
        ...
        <!-- 1. FLEXIBLE 5-STEP ARCHITECTURE SLOTS -->
        <div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-5 gap-2">
                <div class="bg-blue-50 p-3 rounded-xl border border-blue-200 flex flex-col justify-between">
                    ...
                    <p class="text-[10px] text-blue-900 mt-1 font-medium">• 1,2공구 도서 대조<br">• 슬리브 위치 대조</p>
                </div>
                ...
                <div class="bg-cyan-50 p-3 rounded-xl border border-cyan-200 flex flex-col justify-between">
                    ...
                    <h4 class="font-bold text-slate-900 text-[11px] mt-1">시공 매설</h4>
                ...
                <div class="bg-emerald-50 p-3 rounded-xl border border-emerald-200 flex flex-col justify-between">
                    ...
                    <p class="text-[10px] text-emerald-900 mt-1 font-medium">• 회의록 서명<br">• Big Room 회의록</p>
                </div>
            </div>
        </div>

        <!-- 🔥 2. 초정밀 HOW(어떻게 수행하는가) 세부 실무 가이드 (1:1 2D Visual SVG 수록) -->
        <div class="space-y-6">
            <h2 class="text-xl font-bold text-slate-900 mb-4 border-b-2 border-indigo-600 pb-2">
                <span class="text-indigo-600">2.</span> 초정밀 HOW(어떻게 수행하는가) 세부 실무 가이드 & 1:1 직관적 2D 그림
            </h2>

            <div class="space-y-6 text-sm">
                
                <!-- STEP 1 HOW Card with 2D SVG -->
                <div class="bg-white p-6 rounded-2xl border border-blue-200 shadow-sm space-y-4">
                    <div class="flex items-center gap-3">
                        ...
                        <h3 class="font-bold text-base text-slate-900">도면 대조 및 선제 정밀 검토 방법 (HOW TO PREPARE)</h3>
                    </div>
                    <ul class="list-disc pl-5 text-slate-700 space-y-1.5 text-xs font-medium leading-relaxed">
                        <li><strong>3D BIM & CAD 오버레이 대조:</strong> 동탄트램 1,2공구 본선 궤도 및 정거장 토목 CAD 도면과 통신 설비 배치도를 오버레이하여 관통 지점 오차를 사전 검출합니다.</li>
                        <li><strong>슬리브 규격 및 좌표 검증:</strong> 광케이블(72-Core) 및 전원선 관입용 슬리브(ø100~ø150mm) 매설 위치의 세계측지계(GRS80) 좌표 오차를 ±10mm 이내로 도면 대조합니다.</li>
                        <li><strong>자재 보관 창고 검토:</strong> 현장 자재 보관 전용 창고(임대 비용 내역서 반영) 및 반입동선 내 장애 요소(층고, 가설 도로)를 사전 점검합니다.</li>
                    </ul>

                    <!-- 1:1 STEP 1 HOW 2D SVG -->
                    <div class="clickable-diagram bg-slate-50 p-4 rounded-xl border border-slate-200 mt-3" onclick="openDiagramZoom('svg_step1_how', 'STEP 1 HOW 3D BIM/CAD 도면 대조 및 관통 슬리브 좌표 오차 검측 2D 도식')">
                        <svg id="svg_step1_how" viewBox="0 0 520 180" width="100%" height="180" xmlns="http://www.w3.org/2000/svg">
                            ...
                            <text x="130" y="50" font-size="13" font-weight="black" fill="#1d4ed8" text-anchor="middle">📐 3D BIM & CAD 도면 오버레이</text>
                            <text x="35" y="78" font-size="11" font-weight="bold" fill="#334155">• 토목/건축 vs 통신배치도 대조</text>
                            <text x="35" y="100" font-size="11" font-weight="bold" fill="#334155">• 72-Core 광/전원 슬리브 위치</text>
                            ...
                            <text x="400" y="50" font-size="13" font-weight="black" fill="#047857" text-anchor="middle">🎯 좌표 오차 검측 (GRS80)</text>
                            <text x="315" y="78" font-size="11" font-weight="bold" fill="#334155">• 슬리브 ø100~ø150mm 매설</text>
                            <text x="315" y="100" font-size="11" font-weight="bold" fill="#334155">• 허용 오차 ±10mm 이내 확정</text>
                            <text x="400" y="132" font-size="11" font-weight="black" fill="#047857" text-anchor="middle">✔ 선제 정밀 검토 통과</text>
                        </svg>
                    </div>
                </div>

                <!-- STEP 2 HOW Card with 2D SVG -->
                <div class="bg-white p-6 rounded-2xl border border-indigo-200 shadow-sm space-y-4">
                    <div class="flex items-center gap-3">
                        ...
                        <h3 class="font-bold text-base text-slate-900">현장 실측 및 공간 여건 검증 방법 (HOW TO MEASURE)</h3>
                    </div>
                    <ul class="list-disc pl-5 text-slate-700 space-y-1.5 text-xs font-medium leading-relaxed">
                        <li><strong>마감 층고 레이저 측량:</strong> 레이저 거리 측정기를 사용하여 통신기계실, 관제실, 앰프실, TPS실 천장 마감 층고(최소 3.0m 이상) 및 이중바닥(Access Floor H=300mm) 간격을 1:1 실측합니다.</li>
                        <li><strong>반입 엘리베이터 및 개구부 검측:</strong> 고가 통신 랙(Height 2200mm) 및 케이블 드럼 반입을 위한 엘리베이터 유효 너비(최소 1.5m×2.1m 이상)와 건축 반입구 통과 유효 치수를 검측합니다.</li>
                        <li><strong>관로 인입 및 타공 마감 확인:</strong> 케이블 인입 관로와 건축 벽체 타공 부위의 위치를 현장에 먹줄 표기하여 토목/건축 감리원과 사전 현장 확인합니다.</li>
                    </ul>

                    <!-- 1:1 STEP 2 HOW 2D SVG -->
                    <div class="clickable-diagram bg-slate-50 p-4 rounded-xl border border-slate-200 mt-3" onclick="openDiagramZoom('svg_step2_how', 'STEP 2 HOW 레이저 층고 측량 및 반입 엘리베이터 치수 검측 2D 도식')">
                        <svg id="svg_step2_how" viewBox="0 0 520 180" width="100%" height="180" xmlns="http://www.w3.org/2000/svg">
                            <rect x="0" y="0" width="520" height="180" fill="#f8fafc"/>
                            ...
                            <text x="35" y="78" font-size="11" font-weight="bold" fill="#334155">• 기능실 천장 층고 ≥ 3.0m 이상</text>
                            ...
                            <text x="315" y="100" font-size="11" font-weight="bold" fill="#334155">• EV 유효너비 ≥ 1.5m×2.1m 검측</text>
                            <text x="400" y="132" font-size="11" font-weight="black" fill="#047857" text-anchor="middle">✔ 반입동선 간섭 Zero 승인</text>
                        </svg>
                    </div>
                </div>

                <!-- STEP 3 HOW Card with 2D SVG -->
                <div class="bg-white p-6 rounded-2xl border border-cyan-200 shadow-sm space-y-4">
                    <div class="flex items-center gap-3">
                        ...
                        <h3 class="font-bold text-base text-slate-900">콘크리트 타설 전 슬리브 매설 방법 (HOW TO INSTALL)</h3>
                    </div>
                    <ul class="list-disc pl-5 text-slate-700 space-y-1.5 text-xs font-medium leading-relaxed">
                        <li><strong>철근 용접 및 거푸집 고정:</strong> 콘크리트 거푸집 가설 시, 통신 전용 강관/PVC 슬리브를 철근 지지대(Support Bracket)에 용접 및 결속선으로 3중 고정하여 타설 시 이탈을 방지합니다.</li>
                        <li><strong>수밀 캡 착용 및 유입 차단:</strong> 콘크리트 페이스트 유입을 차단하기 위해 슬리브 양단에 고무 수밀 캡을 강하게 마감 착용합니다.</li>
                        <li><strong>타설 후 레이저 오차 검측:</strong> 콘크리트 경화 후 슬리브 관통 도통 시험을 실시하고 오차(±10mm 이내)를 현장 사진 찍어 검측대장에 수록합니다.</li>
                    </ul>

                    <!-- 1:1 STEP 3 HOW 2D SVG -->
                    <div class="clickable-diagram bg-slate-50 p-4 rounded-xl border border-slate-200 mt-3" onclick="openDiagramZoom('svg_step3_how', 'STEP 3 HOW 콘크리트 타설 전 슬리브 용접 3중 고정 및 고무 수밀 캡 2D 도식')">
                        <svg id="svg_step3_how" viewBox="0 0 520 180" width="100%" height="180" xmlns="http://www.w3.org/2000/svg">
                            <rect x="0" y="0" width="520" height="180" fill="#f8fafc"/>
                            <rect x="20" y="25" width="220" height="130" fill="#ffffff" stroke="#0891b2" stroke-width="2" rx="8"/>
                            <text x="130" y="50" font-size="13" font-weight="black" fill="#0e7490" text-anchor="middle">🏗️ 철근 지지대 3중 고정</text>
                            <text x="35" y="78" font-size="11" font-weight="bold" fill="#334155">• 강관/PVC 슬리브 용접 고정</text>
                            <text x="35" y="100" font-size="11" font-weight="bold" fill="#334155">• 거푸집 타설 시 위치 이탈 방지</text>
                            <text x="130" y="132" font-size="11" font-weight="black" fill="#15803d" text-anchor="middle">✔ 매설 위치 3중 고정 완수</text>

                            <path d="M 245 90 L 285 90" stroke="#0891b2" stroke-width="3"/>
                            <polygon points="285,85 295,90 285,95" fill="#0891b2"/>

                            <rect x="300" y="25" width="200" height="130" fill="#ffffff" stroke="#059669" stroke-width="2" rx="8"/>
                            <text x="400" y="50" font-size="13" font-weight="black" fill="#047857" text-anchor="middle">🔌 고무 수밀 캡 & 도통 시험</text>
                            <text x="315" y="78" font-size="11" font-weight="bold" fill="#334155">• 콘크리트 시멘트 유입 차단</text>
                            <text x="315" y="100" font-size="11" font-weight="bold" fill="#334155">• 타설 후 관통 도통시험 ±10mm</text>
                            <text x="400" y="132" font-size="11" font-weight="black" fill="#047857" text-anchor="middle">✔ 관로 도통 검측 통과</text>
                        </svg>
                    </div>
                </div>

                <!-- STEP 4 HOW Card with 2D SVG -->
                <div class="bg-white p-6 rounded-2xl border border-teal-200 shadow-sm space-y-4">
                    <div class="flex items-center gap-3">
                        ...
                        <h3 class="font-bold text-base text-slate-900">방화 구속 & 관로 밀폐 시공 방법 (HOW TO SEAL)</h3>
                    </div>
                    <ul class="list-disc pl-5 text-slate-700 space-y-1.5 text-xs font-medium leading-relaxed">
                        <li><strong>Fire-Stop 내화 씰링 관입:</strong> 벽체 및 슬라브 관통 부위에 내화 2시간 이상 공인 인증을 받은 방화 충전재(Fire-Stop Calking)를 틈새 없이 치밀하게 관입 시공합니다.</li>
                        <li><strong>수밀/가스밀 패드 마감:</strong> 침수 및 유해가스 유입을 방지하기 위해 케이블 인입 관로 입구에 수밀/가스밀 밀폐형 코킹 및 방화 패드를 2중 씰링 마감합니다.</li>
                        <li><strong>방화 검측 사진 채증:</strong> 방화 충전재 시공 전/중/후 상태를 각 3도 각도에서 사진 촬영하여 감리원 서류로 제출합니다.</li>
                    </ul>

                    <!-- 1:1 STEP 4 HOW 2D SVG -->
                    <div class="clickable-diagram bg-slate-50 p-4 rounded-xl border border-slate-200 mt-3" onclick="openDiagramZoom('svg_step4_how', 'STEP 4 HOW Fire-Stop 내화 2시간 씰링 및 수밀/가스밀 2중 관로 밀폐 2D 도식')">
                        <svg id="svg_step4_how" viewBox="0 0 520 180" width="100%" height="180" xmlns="http://www.w3.org/2000/svg">
                            <rect x="0" y="0" width="520" height="180" fill="#f8fafc"/>
                            ...
                            <rect x="300" y="25" width="200" height="130" fill="#ffffff" stroke="#059669" stroke-width="2" rx="8"/>
                            <text x="400" y="50" font-size="13" font-weight="black" fill="#047857" text-anchor="middle">🛡️ 수밀 & 가스밀 2중 밀폐</text>
                            <text x="315" y="78" font-size="11" font-weight="bold" fill="#334155">• 케이블 인입구 2중 방화 패드</text>
                            <text x="315" y="100" font-size="11" font-weight="bold" fill="#334155">• 시공 전/중/후 3각도 사진 채증</text>
                            <text x="400" y="132" font-size="11" font-weight="black" fill="#047857" text-anchor="middle">✔ 방화 밀폐 검측 완수</text>
                        </svg>
                    </div>
                </div>

                <!-- STEP 5 HOW Card with 2D SVG -->
                <div class="bg-white p-6 rounded-2xl border border-emerald-200 shadow-sm space-y-4">
                    <div class="flex items-center gap-3">
                        ...
                        <h3 class="font-bold text-base text-slate-900">3자 합동 검측 및 결과서 체결 방법 (HOW TO SIGN & SIGN-OFF)</h3>
                    </div>
                    <ul class="list-disc pl-5 text-slate-700 space-y-1.5 text-xs font-medium leading-relaxed">
                        <li><strong>3자 합동 현장 검측:</strong> 통신/토목/건축 3자 책임 엔지니어 및 감리원이 현장에 동시 입석하여 체크리스트 12대 항목을 1:1 대조 점검합니다.</li>
                        <li><strong>인터페이스 회의록 서명:</strong> 이견 사항 조율 후 최신화된 회의록에 현장대리인, 토목/건축 엔지니어 및 책임감리원의 서명/날인을 체결합니다.</li>
                        <li><strong>종합 관리대장 실시간 연동:</strong> 체결된 서류 및 증빙 사진을 동탄트램 1,2공구 통합 관리대장에 실시간 등재하여 최종 승인을 완수합니다.</li>
                    </ul>

                    <!-- 1:1 STEP 5 HOW 2D SVG -->
                    <div class="clickable-diagram bg-slate-50 p-4 rounded-xl border border-slate-200 mt-3" onclick="openDiagramZoom('svg_step5_how', 'STEP 5 HOW 통신/토목/건축 3자 합동 검측 및 Big Room 회의록 서명 체결 2D 도식')">
                        <svg id="svg_step5_how" viewBox="0 0 520 180" width="100%" height="180" xmlns="http://www.w3.org/2000/svg">
                            <rect x="0" y="0" width="520" height="180" fill="#f8fafc"/>
                            <rect x="20" y="25" width="220" height="130" fill="#ffffff" stroke="#059669" stroke-width="2" rx="8"/>
                            <text x="130" y="50" font-size="13" font-weight="black" fill="#047857" text-anchor="middle">🖊️ 3자 합동 현장 검측</text>
                            <text x="35" y="78" font-size="11" font-weight="bold" fill="#334155">• 통신/토목/건축 책임자 입석</text>
                            <text x="35" y="100" font-size="11" font-weight="bold" fill="#334155">• 체크리스트 12대 수칙 대조</text>
                            <text x="130" y="132" font-size="11" font-weight="black" fill="#15803d" text-anchor="middle">✔ 합동 현장 검측 통과</text>
...
# Overwrite Guideline HTML Files in WBS 9000-2-9
for f in os.listdir(target_dir):
    if f.endswith('.html'):
        fp = os.path.join(target_dir, f)
        with open(fp, 'w', encoding='utf-8') as file_obj:
            file_obj.write(gui_wbs_9000_2_9_html)
        print(f"   ✓ [WBS 9000-2-9 1:1 SVG INJECTED] -> {f}")

print("\n🎉 SUCCESSFULLY INJECTED 1:1 INTUITIVE 2D VISUAL SVG DIAGRAMS FOR ALL STEP CARDS IN WBS 9000-2-9 GUIDELINE HTMLs!")
