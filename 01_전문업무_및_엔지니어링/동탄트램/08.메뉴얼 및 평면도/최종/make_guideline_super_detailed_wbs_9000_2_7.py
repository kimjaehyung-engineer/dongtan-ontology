...
# SUPER DETAILED GUIDELINE HTML
gui_super_detailed_html = f"""<!DOCTYPE html>
<html lang="ko">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>동탄트램 통신분야 - 발주처 품질 요구사항 검토 초상세 수행지침서 (WBS 9000-2-7)</title>
...
<div class="max-w-5xl mx-auto bg-white shadow-2xl rounded-2xl border border-slate-200 overflow-hidden">
    <div class="bg-slate-900 text-white p-8 relative">
        <span class="bg-blue-600 text-white text-xs font-bold px-3 py-1 rounded-full uppercase">Dongtan Tram Super Detailed Guideline (WBS 9000-2-7)</span>
        <h1 class="text-3xl font-black mt-2">발주처 품질 요구사항 검토 초상세 실무 수행지침서</h1>
        <p class="text-blue-200 text-sm mt-1">L4 Code: 9000-2-7 | 주관: 현장 시스템팀 / 통신 감리단 | "초보자도 한눈에 아는 무엇을/어떻게/측정수치 수록"</p>
    </div>
    
    <div class="p-6 sm:p-10 space-y-10">
        <!-- 💡 친절한 개요 해설 박스 -->
        <div class="bg-blue-50 border border-blue-200 p-6 rounded-2xl text-sm text-blue-950 space-y-3">
            <h4 class="font-bold text-base flex items-center gap-2">💡 한눈에 읽는 초상세 실무 가이드 (Flexible 4-Step Architecture)</h4>
            <p class="bg-white p-4 rounded-xl border border-blue-300 font-medium text-slate-900 leading-relaxed">
                본 지침서는 발주처 품질 검토 업무를 현장 초보자 및 감리원이 한눈에 이해할 수 있도록 <strong>무엇을(WHAT), 어떻게(HOW), 무슨 측정장비로(TOOLS), 정밀 합격 수치(0.05dB / -95dBm)</strong>까지 초상세하고 쉽게 설명하였습니다.
                아래 <strong>Step별 1:1 직관적 2D Visual 그림</strong>은 <strong><span class="term-highlight" onclick="openDiagramZoom('svg_step1', 'STEP 1 ITP/ITC 사전 대조 도식')">클릭하면 대형 모달 창으로 확대</span></strong>되어 정밀 확인이 가능합니다.
            </p>
        </div>
        ...
        <div class="bg-blue-50/70 border border-blue-200 p-7 rounded-2xl shadow-md space-y-6">
            <div class="border-b border-blue-200 pb-4">
                <span class="bg-blue-600 text-white text-xs font-black px-3 py-1 rounded-full uppercase">SUPER DETAILED PROCESS</span>
                <h3 class="text-xl font-black text-blue-950 mt-2">📋 발주처 품질 검토 4단계 초상세 프로세스</h3>
            </div>
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                <div class="bg-white p-5 rounded-xl border border-slate-200 shadow-sm space-y-2">
                    ...
                    <p class="text-slate-700 text-xs">발주처 계획서(ITP) 대조 & 감리원 현장 입회(Hold Point) 항목 미리 형광펜 칠하기.</p>
                </div>
                <div class="bg-white p-5 rounded-xl border border-slate-200 shadow-sm space-y-2">
                    <span class="font-bold text-blue-700 flex items-center gap-2"><span>⚡</span> 2. 광케이블 접속 손실 0.05dB 측정</span>
                    <p class="text-slate-700 text-xs">OTDR & 광파워미터(1310/1550nm)로 72가닥 전수 광 손실 0.05dB 이하 판정.</p>
                </div>
                <div class="bg-white p-5 rounded-xl border border-slate-200 shadow-sm space-y-2">
                    <span class="font-bold text-blue-700 flex items-center gap-2"><span>📶</span> 3. LTE-R 무선국 준공 검증</span>
                    <p class="text-slate-700 text-xs">스펙트럼 분석기로 노선 전구간 이동하며 수신 전계강도 -95dBm 이상 측정.</p>
                </div>
                <div class="bg-white p-5 rounded-xl border border-slate-200 shadow-sm space-y-2">
                    <span class="font-bold text-blue-700 flex items-center gap-2"><span>🖊️</span> 4. 품질 승인서(ITC) 최종 체결</span>
                    <p class="text-slate-700 text-xs">발주처·감리원·시스템 책임자 3자 서명 날인 & 결함 시정대장 완결 등록.</p>
                </div>
            </div>
        </div>
        ...
        <div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-3">
                <div class="bg-blue-50 p-4 rounded-xl border border-blue-200 flex flex-col justify-between">
                    ...
                    <p class="text-[11px] text-blue-900 mt-1 font-medium">• 품질계획서 대조<br">• 감리 입회점 표기</p>
                </div>
        ...
        <div class="space-y-8">
            <h2 class="text-xl font-bold text-slate-900 mb-4 border-b-2 border-indigo-600 pb-2">
                <span class="text-indigo-600">2.</span> 초상세 HOW 세부 실무 가이드 & 1:1 2D 그림
            </h2>
            ...
            <div class="space-y-8 text-sm">
                ...
                <div class="bg-white p-6 rounded-2xl border border-blue-200 shadow-sm space-y-4">
                    <div class="flex items-center gap-3">
                        ...
                        <h3 class="font-bold text-base text-slate-900">발주처 품질검사 계획서(ITP/ITC) 사전 대조 & 감리 입회점 표기</h3>
                    </div>
                    <div class="bg-slate-50 p-5 rounded-xl text-xs text-slate-700 space-y-3 leading-relaxed border border-slate-200">
                        <p><strong>💡 초상세 실무 설명:</strong></p>
                        <ul class="list-disc pl-5 space-y-1.5 text-slate-800">
                            <li><strong>무엇을 하는가?:</strong> 발주처에서 받은 품질검사 계획서(ITP)와 시방서를 책상 위에 펼쳐두고 대조합니다.</li>
                            <li><strong>구체적 작업 방법:</strong> 광케이블, 4K CCTV, 스피커, LTE-R 안테나 등 자재 입고 시 공장시험 성적서(Mill Test)가 제대로 들어왔는지 서류를 체크합니다.</li>
                            <li><strong>현장 실무 팁:</strong> 감리원이 현장에 직접 나와서 눈으로 확인해야 하는 <strong>'입회 점검(Hold Point)'</strong> 항목을 서류에 노란색 형광펜으로 칠해 두어 검측 누락을 막습니다.</li>
                        </ul>
                    </div>
                    ...
                    <div class="clickable-diagram bg-slate-50 p-4 rounded-xl border border-slate-200 mt-3" onclick="openDiagramZoom('svg_step1', 'STEP 1 ITP/ITC 서류 대조 2D 시공 도식')">
                        <svg id="svg_step1" viewBox="0 0 520 180" width="100%" height="180" xmlns="http://www.w3.org/2000/svg">
                            ...
                            <g transform="translate(35, 62)">
                                ...
                                <text x="95" y="17" font-size="10" font-weight="bold" fill="#1d4ed8" text-anchor="middle">✔ 자재 성적서 100% 사전 검토</text>

                                <rect x="0" y="32" width="190" height="25" fill="#e0f2fe" stroke="#2563eb" stroke-width="1.5" rx="3"/>
                                <text x="95" y="49" font-size="10" font-weight="bold" fill="#1d4ed8" text-anchor="middle">✔ 감리 입회점(Hold Point) 형광펜 표기</text>
                            </g>
                ...
                <div class="bg-white p-6 rounded-2xl border border-indigo-200 shadow-sm space-y-4">
                    ...
                    <div class="bg-slate-50 p-5 rounded-xl text-xs text-slate-700 space-y-3 leading-relaxed border border-slate-200">
                        <p><strong>💡 초상세 실무 설명:</strong></p>
                        <ul class="list-disc pl-5 space-y-1.5 text-slate-800">
                            <li><strong>무엇을 하는가?:</strong> 메인 72가닥 광케이블을 자동 융착 접속기로 정밀하게 이은 후, 빛이 새지 않고 깨끗하게 연결되었는지 측정합니다.</li>
                            <li><strong>무슨 장비를 쓰는가?:</strong> OTDR(광파동 측정기)과 광파워미터를 광선 양끝에 꽂고 1310nm / 1550nm 두 가지 파장의 빛을 쏩니다.</li>
                            <li><strong>정밀 합격 수치:</strong> 측정기 화면 그래프 상에 1개 코어당 빛 손실이 <strong>0.05dB 이하(실제 현장 최상급 수치: 0.01 ~ 0.04dB)</strong>로 나오는지 72가닥 전수를 1:1 확인하고 결과 리포트를 프린트합니다.</li>
                        </ul>
                    </div>
                ...
                <div class="bg-white p-6 rounded-2xl border border-cyan-200 shadow-sm space-y-4">
                    ...
                    <div class="bg-slate-50 p-5 rounded-xl text-xs text-slate-700 space-y-3 leading-relaxed border border-slate-200">
                        <p><strong>💡 초상세 실무 설명:</strong></p>
                        <ul class="list-disc pl-5 space-y-1.5 text-slate-800">
                            <li><strong>무엇을 하는가?:</strong> 트램 전용 LTE-R 안테나 전파가 무전기와 관제실로 끊김 없이 터지는지 측정합니다.</li>
                            <li><strong>무슨 장비를 쓰는가?:</strong> 전파 측정 장비(스펙트럼 분석기)와 무전기를 들고 트램 노선을 이동하며 측정합니다.</li>
                            <li><strong>정밀 합격 수치:</strong> 정거장, 교차로, 터널 구간에서 수신 전계강도가 <strong>-95dBm 이상(신호 매우 양호)</strong>으로 유지되는지 검사하고 전파법에 따른 국가 공인 무선국 준공검사 합격 필증 서류를 확보합니다.</li>
                        </ul>
                    </div>
                ...
                <div class="bg-white p-6 rounded-2xl border border-emerald-200 shadow-sm space-y-4">
                    <div class="flex items-center gap-3">
                        ...
                        <h3 class="font-bold text-base text-slate-900">발주처·감리원 품질 검측 승인서(ITC) 최종 체결 & 대장 확정</h3>
                    </div>
                    <div class="bg-slate-50 p-5 rounded-xl text-xs text-slate-700 space-y-3 leading-relaxed border border-slate-200">
                        <p><strong>💡 초상세 실무 설명:</strong></p>
                        <ul class="list-disc pl-5 space-y-1.5 text-slate-800">
                            <li><strong>무엇을 하는가?:</strong> 모든 품질 검측(ITP)이 완료되면 최종 품질 검측 승인서(ITC)를 작성합니다.</li>
                            <li><strong>서명 절차:</strong> 발주처 품질 감독관, 현장 총괄 감리원, 통신 시스템 책임자 3인이 한자리에 모여 시험 성적서를 최종 확인 후 서명 도장을 찍습니다.</li>
                            <li><strong>대장 등록:</strong> 검측 중 지적된 지적 사항 결함 시정 조치 보고서를 부착하고, 최종 품질 대장에 완결 등록합니다.</li>
                        </ul>
                    </div>
...
for fn in os.listdir(gui_folder):
    if fn.endswith('.html'):
        ...
        with open(fp, 'w', encoding='utf-8') as f:
            f.write(gui_super_detailed_html)
        print(f"   ✓ [SUPER DETAILED APPLIED] Guideline -> {fn}")

print("\n🎉 SUCCESSFULLY ENHANCED WBS 9000-2-7 GUIDELINE HTMLs WITH SUPER DETAILED PRACTICAL EXPLANATIONS!")
