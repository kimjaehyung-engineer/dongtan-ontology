...
# Restructured Intuitive Master WBS 19 Guideline
gui_intuitive_html = f"""<!DOCTYPE html>
<html lang="ko">
<head>
    <meta charset="UTF-8">
...
<div class="max-w-5xl mx-auto bg-white shadow-2xl rounded-2xl border border-slate-200 overflow-hidden">
    ...
    <div class="bg-slate-900 text-white p-8 relative overflow-hidden">
        ...
        <div class="relative z-10">
            <div class="flex items-center gap-3 mb-2">
                ...
                <span class="bg-white text-slate-950 text-xs font-bold px-3 py-1 rounded-full">궤간척(Track Gauge Bar) 직관적 2D 기술 도식 매뉴얼</span>
            </div>
    ...
    <div class="p-6 sm:p-10 space-y-10">
        <!-- 💡 작업 안내 박스 (궤간척 직관적 설명 포함) -->
        <div class="bg-sky-50 border border-sky-200 p-5 rounded-xl text-xs sm:text-sm text-sky-900 shadow-sm space-y-2">
            <h4 class="font-bold text-sky-950 text-base flex items-center gap-2">
                <span>💡</span> [TCL] 콘크리트 타설 및 궤간척(Track Gauge Bar) 개념 해설
            </h4>
            <p class="leading-relaxed">
                TCL 콘크리트 타설 시 <strong><span class="term-highlight" onclick="openGlossary('tcl_strength')">강도 fck ≥ 35 MPa</span></strong>, <strong><span class="term-highlight" onclick="openGlossary('slump_control')">슬럼프 ≤ 10cm 저슬럼프 타설</span></strong>, <strong><span class="term-highlight" onclick="openGlossary('wet_curing')">7일 습윤양생</span></strong> 및 <strong><span class="term-highlight" onclick="openGlossary('groove_drain')">그루브 드레인 홈 세척</span></strong>을 수행합니다.
            </p>
            <div class="bg-white p-3 rounded-lg border border-sky-300 font-medium text-sky-950">
                🔍 <strong>'궤간척(Track Gauge Bar)'이란?</strong><br>
                양쪽 레일 위에 턱 올려놓고 <strong>"두 레일 사이의 거리(1,435mm)와 좌우 높이차(캔트)를 실시간으로 재어주는 정밀 막대자"</strong>입니다. 콘크리트 타설 시 무거운 측압 때문에 레일이 벌어지지 않도록 디스플레이 눈금을 보면서 <strong>캔트 오차(±2.0mm 이내)를 실시간 조율</strong>합니다.
            </div>
        </div>
        ...
        <!-- 2. 3단계 체계별 세부 작업 수행절차 (직관적 2D 궤간척 visual 도식 수록) -->
        <div>
                ...
                <div class="bg-white p-6 rounded-2xl border border-slate-200 shadow-md relative space-y-4">
                    ...
                    <div class="clickable-diagram bg-slate-50 p-4 rounded-xl flex justify-center items-center shadow-inner border border-amber-200" onclick="openDiagramZoom('svgStep1_Card', '[사전 준비] 타설 직전 스핀들 측량 & 레미콘 슬럼프(≤ 10cm) 현장 시험 도면')">
                        <svg id="svgStep1_Card" viewBox="0 0 520 180" width="100%" height="180" xmlns="http://www.w3.org/2000/svg">
                            ...
                            <rect x="40" y="45" width="180" height="95" fill="#ffffff" stroke="#d97706" stroke-width="1.5" rx="6"/>
                            <text x="130" y="70" font-size="12" font-weight="black" fill="#b45309" text-anchor="middle">레미콘 슬럼프 시험 콘 (Slump Cone)</text>
                            <line x1="70" y1="115" x2="190" y2="115" stroke="#dc2626" stroke-width="3.5"/>
                            <text x="130" y="105" font-size="12" font-weight="black" fill="#dc2626" text-anchor="middle">슬럼프 값 &le; 10cm 합격</text>
                            
                            <line x1="260" y1="95" x2="480" y2="95" stroke="#0284c7" stroke-width="4"/>
                            <text x="370" y="80" font-size="12" font-weight="black" fill="#0284c7" text-anchor="middle">타설 직전 스핀들 궤광 레벨 정밀 측량</text>
                        </svg>
                    </div>
                </div>

                <!-- STEP 2 CARD (★ [개정] 레일 + 궤간척 막대자 + 디지털 다이얼 계측기 직관 도식) -->
                <div class="bg-white p-6 rounded-2xl border border-slate-200 shadow-md relative space-y-4">
                    ...
                    <!-- STEP 2 2D Visual Diagram (★ 획기적 개선: 레일 + 궤간척 + 디지털 계측기) -->
                    <div class="clickable-diagram bg-slate-50 p-4 rounded-xl flex justify-center items-center shadow-inner border border-sky-200" onclick="openDiagramZoom('svgStep2_Card', '[본 시공] 궤간척(Gauge Bar) 실시간 캔트 오차(±2.0mm) 측량 & 고주파 다짐 정밀 도면')">
                        <svg id="svgStep2_Card" viewBox="0 0 520 220" width="100%" height="220" xmlns="http://www.w3.org/2000/svg">
                            ...
                            <!-- ★ 궤간척 막대자 (양쪽 레일에 척 걸쳐진 Gauge Bar) -->
...
for folder_path in [folder_with_space, folder_no_space]:
    ...
    for fname in ["[TCL] 콘크리트 타설 및 양생_수행지침.html", "19_[TCL] 콘크리트 타설 및 양생_수행지침.html", "[TCL] 콘크리트타설및양생_수행지침.html", "19_[TCL] 콘크리트타설및양생_수행지침.html"]:
        ...
        with open(fpath, 'w', encoding='utf-8') as f:
            f.write(gui_intuitive_html)
        print(f"✏️ Updated WBS 19 Guideline with Intuitive Gauge Bar Visuals: {fpath}")

print("\n🎉 SUCCESSFULLY UPDATED WBS 19 GUIDELINE WITH INTUITIVE GAUGE BAR VISUAL DIAGRAMS!")
