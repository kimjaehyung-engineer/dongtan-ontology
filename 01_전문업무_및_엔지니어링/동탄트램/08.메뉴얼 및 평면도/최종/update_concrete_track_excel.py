...
# Step 1: Update original Excel file sheet '콘크리트도상'
wb = openpyxl.load_workbook(excel_path)
sheet = wb['콘크리트도상']

excel_updated = 0
for r in range(2, sheet.max_row + 1):
    wbs_val = sheet.cell(row=r, column=4).value
    if not wbs_val:
        continue
    
    # Extract sequence number from WBS (e.g. 9000-6-1 -> 1, 9000-6-23 -> 23)
    try:
        seq_num = int(str(wbs_val).split("-")[-1])
    except ValueError:
        continue
        
    if seq_num in track_ground_truth:
        gt = track_ground_truth[seq_num]
        
        # Inject exact columns
        # Note: In sheet '콘크리트도상':
        # Column 6 (F) is Activity Name (명칭)
        # Column 8 (H) is Purpose (목적)
        # Column 10 (J) is Deliverables (산출물)
        # Column 11 (K) is Standard Summary (표준서 요약)
        # Column 15 (O) is Checklist Summary (체크리스트 요약)
        sheet.cell(row=r, column=6).value = gt["title"]
        sheet.cell(row=r, column=8).value = gt["purpose"]
        sheet.cell(row=r, column=10).value = gt["deliverable"]
        sheet.cell(row=r, column=11).value = gt["std_sum"]
        sheet.cell(row=r, column=15).value = gt["chk_sum"]
        
        excel_updated += 1
        print(f"Excel Column Fix Row {r:02d} [{wbs_val}] ➔ Injected '{gt['title']}' ground truths.")

try:
    wb.save(excel_path)
    print(f"🎉 Successfully Saved direct to Original Excel '{excel_path}' ({excel_updated} rows)!")
except PermissionError:
    wb.save(backup_excel_path)
    print(f"⚠️ Original Excel is locked. Saved to Backup: '{backup_excel_path}' ({excel_updated} rows)")
