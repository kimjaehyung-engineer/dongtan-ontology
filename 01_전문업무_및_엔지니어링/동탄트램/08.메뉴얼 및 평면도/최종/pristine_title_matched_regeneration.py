...
def generate_tailored_guideline_html(act_id, act_name, wbs, spec_data):
    step1 = spec_data.get('g_step1', f"{act_name} 관련 설계도서 검토 및 현장 조건 사전 준비 시행")
    step2 = spec_data.get('g_step2', f"{act_name} 핵심 공학 시방 수칙 준수 및 현장 본 시공/실행")
    step3 = spec_data.get('g_step3', f"{act_name} 품질 성적서 획득, 준공 산출물 완비 및 감리단/발주처 서명 완료")
...
print("Executing pristine title-corrected 1:1 Tailored regeneration...")
...
for root, dirs, files in os.walk(base_dir):
    for f in files:
        if f.endswith('.html'):
            ...
            match_act = re.search(r'^(.*?)_(수행지침|체크리스트)\.html$', f)
            if match_act:
                act_name = match_act.group(1)
                doc_type = match_act.group(2)
                
                # Derive WBS or Act ID
                ...
                if doc_type == '수행지침':
                    new_html = generate_tailored_guideline_html(act_id, act_name, wbs, spec_data)
                    with open(f_path, 'w', encoding='utf-8') as out:
                        out.write(new_html)
                    gui_count += 1
                elif doc_type == '체크리스트':
                    new_html = generate_tailored_checklist_html(act_id, act_name, wbs, spec_data)
                    with open(f_path, 'w', encoding='utf-8') as out:
                        out.write(new_html)
                    chk_count += 1

print(f"🎉 Fully regenerated {gui_count} Guideline HTML files & {chk_count} Checklist HTML files with pristine title-matched Tailored content!")
