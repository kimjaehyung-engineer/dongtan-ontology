...
for src, dst in copy_mappings.items():
    if os.path.exists(src):
        shutil.copy(src, dst)
        print(f"📦 Successfully Copied: {os.path.basename(src)} ➔ {os.path.basename(dst)}")
...
def force_write(path, text):
    ...
    with open(path, 'w', encoding='utf-8') as f:
        f.write(text)
    print(f"✏️ Written Original Master: {path}")

# Common Popup HTML & CSS
...
# =========================================================================
# WBS 11 GUIDELINE HTML (Step 3 카드 하단에 스핀들 미세조정 시뮬레이터 완벽 재삽입)
# =========================================================================
...
<div class="max-w-5xl mx-auto bg-white shadow-2xl rounded-2xl border border-slate-200 overflow-hidden">
    ...
    <div class="bg-emerald-900 text-white p-8 relative overflow-hidden">
        ...
        <div class="relative z-10">
            ...
            <p class="text-emerald-200 mt-2 text-sm sm:text-base">"누가 봐도 한눈에 이해하는 테르밋 용접 및 궤광 조립 실사 가이드"</p>
        </div>
    </div>
    ...
    <div class="p-6 sm:p-10 space-y-10">
        ...
        <div>
            ...
            <div class="bg-emerald-50/50 p-5 rounded-xl border border-emerald-100 text-sm space-y-2">
                <p><strong>🎯 수행 목적:</strong> 처음 업무를 맡은 직원도 5단계 시공 마스터 시퀀스, 궤광 조립 원리, 테르밋 용접 쇳물 조인트를 한눈에 이해하여 감리 승인 득함</p>
                <p><strong>⚙️ 협업 부서:</strong> 현장 공사팀, 공무팀, 안전팀 및 품질팀 전원 의무 참여</p>
            </div>
        </div>

        <!-- [마스터 시뮬레이터] 5대 시공 마스터 시퀀스 단계별 시뮬레이터 -->
        ...
        <div>
            <h2 class="text-xl font-bold text-slate-900 mb-6 flex items-center gap-2 border-b-2 border-emerald-600 pb-2">
                <span class="text-emerald-600">2.</span> 시공계획 수립 5단계 세부 수행 프로세스
            </h2>
            <div class="space-y-6 relative pl-6 border-l-4 border-emerald-500">
                ...
                <!-- STEP 3 (스핀들 미세조정 정밀 시뮬레이터 삽입 영역!) -->
                <div class="bg-white p-5 rounded-xl border border-slate-200 shadow-sm relative">
                    ...
                    <h3 class="text-base font-bold text-slate-900 mt-2 mb-2">스핀들 게이지 수동 조정 및 다이얼 인디케이터 0점 정합</h3>
                    <p class="text-slate-600 text-xs sm:text-sm mb-4">
                        하부 높이 조절 나사(스핀들 볼트)를 수동 회전시켜 레일의 높낮이와 캔트를 조율하며, 다이얼 게이지로 바늘 오차가 ±0.5mm 이내가 되도록 조정 후 락너트를 꽉 조입니다.
                    </p>
                    
                    <!-- [요청사항 재삽입] 스핀들 게이지 미세조정 정밀 시뮬레이터 -->
                    <div class="my-4 grid grid-cols-1 lg:grid-cols-12 gap-5 bg-slate-100 p-5 rounded-xl border border-slate-300">
                        ...
                        <div class="lg:col-span-7 bg-white p-4 rounded-xl border border-slate-200 shadow-inner flex flex-col justify-between">
                            ...
                            <div class="flex items-center justify-between mt-3 pt-3 border-t border-slate-200">
                                <div class="flex items-center gap-1.5">
                                    <button onclick="adjustSpindle(1)" class="sim-btn bg-emerald-600 text-white text-[11px] font-bold px-3 py-1.5 rounded-lg border border-emerald-700">▲ 상승 (나사 조임)</button>
                                    <button onclick="adjustSpindle(-1)" class="sim-btn bg-rose-600 text-white text-[11px] font-bold px-3 py-1.5 rounded-lg border border-rose-700">▼ 하강 (나사 풀기)</button>
                                </div>
                                <div id="spindleAlert" class="text-[11px] font-bold px-2.5 py-1 rounded bg-rose-100 text-rose-800 border border-rose-200 transition-all duration-300">
                                    오차 초과 (-3.0mm)
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="lg:col-span-5 flex flex-col justify-center space-y-3">
                            ...
                            <ol class="list-decimal pl-4 text-[11px] text-slate-600 space-y-2">
                                ...
                                <li><strong>3단계. 스핀들 볼트 수동 조정:</strong> 나사를 돌려 바늘을 ±0.5mm(0점)에 맞춥니다.</li>
                ...
                <div class="bg-white p-5 rounded-xl border border-slate-200 shadow-sm relative">
                    ...
                    <div class="bg-slate-100 p-4 rounded-xl border border-slate-200">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                            <div class="bg-white p-2 rounded-lg border border-slate-300 shadow-sm img-card">
                                <img src="./thermit_field.png" class="w-full h-40 object-cover rounded" alt="테르밋 현장 용접">
                                <p class="text-[10px] text-slate-500 mt-1.5 font-bold text-center">① 가설 궤도 테르밋 쇳물 주입</p>
                            </div>
                            <div class="bg-white p-2 rounded-lg border border-slate-300 shadow-sm img-card">
                                <img src="./thermit_crucible.png" class="w-full h-40 object-cover rounded" alt="도가니 화학 반응">
                                <p class="text-[10px] text-slate-500 mt-1.5 font-bold text-center">② 도가니 용탕 2,500도 반응</p>
                            </div>
                            <div class="bg-white p-2 rounded-lg border border-slate-300 shadow-sm img-card">
                                <img src="./thermit_diagram.png" class="w-full h-40 object-contain rounded" alt="테르밋 단면 모식도">
...
function updateSpindleVisual() {{
    ...
    const spindleAlert = document.getElementById('spindleAlert');
    
    let yTranslate = (currentSpindleStep + 30) * (-0.5);
    if (spindleBolt) spindleBolt.setAttribute('transform', `translate(0, ${{yTranslate}})`);
    
    let rotateDeg = currentSpindleStep * 20; 
    if (dialNeedle) dialNeedle.setAttribute('transform', `rotate(${{rotateDeg}})`);
    
    let pinXLength = -75 - (yTranslate);
    if (dialPin) dialPin.setAttribute('x2', pinXLength);
    if (dialPinTip) dialPinTip.setAttribute('cx', pinXLength);
    
    let valStr = (currentSpindleStep / 10).toFixed(1) + "mm";
    if (currentSpindleStep > 0) valStr = "+" + valStr;
    
    if (spindleAlert) {{
        if (Math.abs(currentSpindleStep) <= 5) {{
            spindleAlert.innerText = `🟢 합격! (오차 ${{valStr}})`;
            spindleAlert.className = "text-[11px] font-bold px-2.5 py-1 rounded bg-emerald-100 text-emerald-800 border border-emerald-200";
        }} else {{
            spindleAlert.innerText = `🔴 오차 초과 (${{valStr}})`;
            spindleAlert.className = "text-[11px] font-bold px-2.5 py-1 rounded bg-rose-100 text-rose-800 border border-rose-200 animate-pulse";
        }}
    }}
}}
...
# HTML Definitions for standard & checklist
...
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>콘크리트도상 - 시공계획 수립 체크리스트</title>
    <style>
        :root {{
            --bg-primary: #f8fafc;
            --bg-card: #ffffff;
...
<div class="container">
    <div class="header">
        <h1 class="title">시공계획 수립 체크리스트</h1>
        <span class="meta">WBS Code 9000-6-11 | 콘크리트도상 품질 검측대장</span>
    </div>
    ...
    <table>
        <thead>
            <tr style="background: #f1f5f9;">
                ...
                <th style="width: 68%;">표준서 & 수행지침 연계 필수 검측 항목 (KDS 47 30 00 시방 수립)</th>
                <th style="width: 14%;">점검 결과</th>
            </tr>
        </thead>
        <tbody>
            <tr>
                <td class="category" style="color:var(--accent-orange);">⚠️ 사전 리스크<br>(Step 1~2 착수 전)</td>
                <td>
                    <div style="margin-bottom: 10px;">
                        <span class="step-tag" style="background:#fef3c7; color:#92400e;">Step 1. 지반 다짐</span>
                        <strong>[하부 노반 지지력 검증]</strong> 상부 강화 노반의 평판재하시험 반력계수(<span class="term-highlight" onclick="openGlossary('hbs_test')">K30 &ge; 110 MN/m³</span>) 승인 확보 여부
                    </div>
                    <div style="margin-bottom: 10px;">
                        <span class="step-tag" style="background:#fef3c7; color:#92400e;">Step 1. 3D 측량</span>
                        <strong>[3D CP 측량 기준점 셋업]</strong> 3차원 측지계 정밀 CP 측량 성과점 보정 및 노선 설계 좌표 동기화 승인 여부
                    </div>
                    <div style="margin-bottom: 4px;">
                        <span class="step-tag" style="background:#e0f2fe; color:#0369a1;">Step 2. 궤광 가조립</span>
                        <strong>[궤광 뼈대 형성 및 안착]</strong> 51R1 홈레일 + 콘크리트 침목 궤광 형성 및 현장 가설치 안전계획 수립 여부
                    </div>
                </td>
                <td class="check-cell">☐ 확인완료</td>
            </tr>
            <tr>
                <td class="category" style="color:var(--accent-red);">⚡ 공사중 리스크<br>(Step 3~4 시공 중)</td>
                <td>
                    <div style="margin-bottom: 10px;">
                        <span class="step-tag" style="background:#dcfce7; color:#166534;">Step 3. 스핀들 조정</span>
                        <strong>[선형 정밀 미세조정]</strong> 스핀들 게이지 조정 통한 궤간(+3, -1mm) 및 캔트(±2.0mm) 0점(±0.5mm) 미세조정 및 락너트 체결 여부
                    </div>
                    <div style="margin-bottom: 10px;">
                        <span class="step-tag" style="background:#ffedd5; color:#9a3412;">Step 4. 테르밋 용접</span>
                        <strong>[2,500℃ 테르밋 쇳물 주입]</strong> 콘크리트 타설 전 레일 머리 위치 테르밋 용접(EN 14730) 및 비파괴 UT 굴곡 오차 ±0.2mm 확보 여부
                    </div>
                    <div style="margin-bottom: 4px;">
                        <span class="step-tag" style="background:#ffedd5; color:#9a3412;">Step 4. 열화 예방</span>
                        <strong>[콘크리트 열화 크랙 방지]</strong> 2,500℃ 초고열에 의한 콘크리트 균열 방지를 위해 콘크리트 타설 전 용접 완료 반영 여부
                    </div>
                </td>
                <td class="check-cell">☐ 확인완료</td>
            </tr>
            <tr>
                <td class="category" style="color:var(--accent-green);">✅ 공사후 리스크<br>(Step 5 완공 마감)</td>
                <td>
                    <div style="margin-bottom: 10px;">
                        <span class="step-tag" style="background:#f0fdf4; color:#15803d;">Step 5. 도상 콘크리트</span>
                        <strong>[TCL 콘크리트 타설]</strong> <span class="term-highlight" onclick="openGlossary('tcl_concrete')">TCL 콘크리트(강도 &ge; 30 MPa)</span> 연속 타설 동선 및 콜드조인트 방지 확인 여부
                    </div>
                    <div style="margin-bottom: 4px;">
                        <span class="step-tag" style="background:#f0fdf4; color:#15803d;">Step 5. 인허가 승인</span>
                        <strong>[시공계획서 감리 승인]</strong> 감리단 궤도공사 시공계획서 최종 적정 승인서 득 및 본사 대장 등록 여부
                    </div>
                </td>
                <td class="check-cell">☐ 확인완료</td>
            </tr>
...
print("\n🎉 SUCCESSFULLY RE-INSERTED MANUAL SPINDLE ADJUSTMENT SIMULATOR DIRECTLY UNDER STEP 3 CARD!")
