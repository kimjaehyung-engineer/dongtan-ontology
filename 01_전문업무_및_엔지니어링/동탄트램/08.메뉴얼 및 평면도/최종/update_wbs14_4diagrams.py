...
def force_write(path, text):
    ...
    with open(path, 'w', encoding='utf-8') as f:
        f.write(text)
    print(f"✏️ Successfully updated WBS 14 4-diagrams: {path}")
...
guideline_wbs14_4diagrams_html = f"""<!DOCTYPE html>
<html lang="ko">
<head>
    <meta charset="UTF-8">
    ...
    <div class="p-6 sm:p-10 space-y-10">
        ...
        <div class="bg-amber-50 border border-amber-200 p-5 rounded-xl text-xs sm:text-sm text-amber-900 shadow-sm">
            <h4 class="font-bold text-amber-950 text-base mb-1.5 flex items-center gap-2">
                <span>💡</span> [HBS] 기초콘크리트 타설 및 양생 4단계 1:1 실무 핵심
            </h4>
            <p class="leading-relaxed">
                강화노반 상부에 <strong><span class="term-highlight" onclick="openGlossary('hbr')">HBR 기초콘크리트</span></strong>를 타설하는 공종입니다. STEP 1부터 STEP 4까지 각 단계에 직관적으로 1:1 매칭되는 <strong>4개의 정밀 2D 기술 도식</strong>을 수록하였으며, 클릭 시 화면 전체에 <strong>초대형 고화질 뷰(`openDiagramZoom`)</strong>로 확대되어 글자가 또렷하게 표시됩니다.
            </p>
        </div>
        ...
        <div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
                ...
                <div class="flow-card bg-blue-50 p-4 rounded-xl border border-blue-200 flex flex-col justify-between space-y-3">
                    ...
                    <div class="bg-white p-2 rounded border border-blue-100 text-[10px] text-amber-800 font-bold">
                        💧 [도식 4] 연동 참조
                    </div>
                </div>
            </div>
        ...
        <div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <!-- STEP 1 -> 도식 1: 레미콘 배합 & 이격거리 4.8km 운반 통제 -->
                    ...
                    <div>
                        <div class="flex items-center justify-between mb-3 border-b border-slate-100 pb-2">
                            <h3 class="font-bold text-base text-slate-900 flex items-center gap-2">
                                ...
                                [도식 1] (STEP 1) 레미콘 이격거리(4.8km) 운반
                            </h3>
                            <span class="bg-amber-100 text-amber-800 text-[10px] font-extrabold px-2.5 py-0.5 rounded-full uppercase">이격거리 &le; 4.8km</span>
                        </div>
                        
                        <div class="clickable-diagram bg-slate-50 p-4 rounded-xl flex justify-center items-center shadow-inner border border-slate-200" onclick="openDiagramZoom('svgDiagram14_STEP1', '[도식 1] 레미콘 이격거리(4.8km) & 출하 60분 도착 통제 도면')">
                            <svg id="svgDiagram14_STEP1" viewBox="0 0 420 220" width="100%" height="220" xmlns="http://www.w3.org/2000/svg">
                                <rect x="0" y="0" width="420" height="220" fill="#f8fafc"/>
                                
                                <rect x="30" y="50" width="100" height="70" fill="#cbd5e1" stroke="#334155" stroke-width="2"/>
                                <text x="80" y="90" font-size="11" font-weight="black" fill="#0f172a" text-anchor="middle">레미콘 공장</text>

                                <line x1="130" y1="85" x2="300" y2="85" stroke="#0284c7" stroke-width="3.5" stroke-dasharray="5,3"/>
                                <text x="215" y="75" font-size="12" font-weight="black" fill="#0284c7" text-anchor="middle">운반 거리 ≤ 4.8km (시간 ≤ 60분)</text>

                                <rect x="300" y="50" width="90" height="70" fill="#fef3c7" stroke="#d97706" stroke-width="2"/>
                                <text x="345" y="90" font-size="11" font-weight="black" fill="#78350f" text-anchor="middle">동탄 트램 현장</text>
                            </svg>
                        </div>
                    </div>
                    <div class="bg-amber-50 p-3.5 rounded-xl border border-amber-100 text-xs text-amber-900 leading-relaxed">
                        <strong>🚚 STEP 1 핵심:</strong> 콘크리트 초기 경화 방지를 위해 <strong>현장 이격거리 4.8km 이내 레미콘 공장을 배정</strong>하고, 출하 60분 이내 도착을 통제합니다.
                    </div>
                </div>

                <!-- STEP 2 -> 도식 2: 거푸집 테이핑 밀봉 & 현장 슬럼프(10cm 이하) 검사 -->
                <div class="bg-white p-6 rounded-2xl border border-slate-200 shadow-md flex flex-col justify-between space-y-4">
                    <div>
                        <div class="flex items-center justify-between mb-3 border-b border-slate-100 pb-2">
                            <h3 class="font-bold text-base text-slate-900 flex items-center gap-2">
                                <span class="w-3 h-3 bg-sky-500 rounded-full inline-block"></span>
                                [도식 2] (STEP 2) 거푸집 테이핑 & 슬럼프(10cm)
                            </h3>
                            <span class="bg-sky-100 text-sky-800 text-[10px] font-extrabold px-2.5 py-0.5 rounded-full uppercase">슬럼프 &le; 10cm</span>
                        </div>
                        
                        <div class="clickable-diagram bg-slate-50 p-4 rounded-xl flex justify-center items-center shadow-inner border border-slate-200" onclick="openDiagramZoom('svgDiagram14_STEP2', '[도식 2] 거푸집 테이핑 밀봉 & 슬럼프(10cm) 시험 도면')">
                            <svg id="svgDiagram14_STEP2" viewBox="0 0 420 220" width="100%" height="220" xmlns="http://www.w3.org/2000/svg">
                                <rect x="0" y="0" width="420" height="220" fill="#f8fafc"/>
                                
                                <!-- 슬럼프 콘 시편 -->
                                <polygon points="80,160 120,160 110,80 90,80" fill="#94a3b8" stroke="#1e293b" stroke-width="1.5"/>
                                <text x="100" y="178" font-size="11" font-weight="black" fill="#0f172a" text-anchor="middle">슬럼프 콘 (30cm)</text>

                                <line x1="120" y1="80" x2="160" y2="80" stroke="#dc2626" stroke-width="2"/>
                                <line x1="120" y1="110" x2="160" y2="110" stroke="#dc2626" stroke-width="2"/>
                                <text x="210" y="100" font-size="12" font-weight="black" fill="#dc2626" text-anchor="start">슬럼프 낙하 ≤ 10cm</text>

                                <!-- 거푸집 접합 테이핑 -->
                                <rect x="290" y="70" width="12" height="90" fill="#b45309"/>
                                <circle cx="296" cy="160" r="7" fill="#ef4444"/>
                                <text x="315" y="120" font-size="11" font-weight="black" fill="#ef4444" text-anchor="start">시멘트풀 유출 방지 테이핑</text>
                            </svg>
                        </div>
                    </div>
                    <div class="bg-sky-50 p-3.5 rounded-xl border border-sky-100 text-xs text-sky-900 leading-relaxed">
                        <strong>🧪 STEP 2 핵심:</strong> 거푸집 접합면을 테이핑 처리하여 시멘트풀 유출을 차단하고, <strong>슬럼프 10cm 이하 & 공기량 4.5%</strong>를 현장에서 직접 시험합니다.
                    </div>
                </div>

                <!-- STEP 3 -> 도식 3: 연속타설 & 바이브레이터 밀실 고주파 다짐 -->
                <div class="bg-white p-6 rounded-2xl border border-slate-200 shadow-md flex flex-col justify-between space-y-4">
                    <div>
                        <div class="flex items-center justify-between mb-3 border-b border-slate-100 pb-2">
                            <h3 class="font-bold text-base text-slate-900 flex items-center gap-2">
                                <span class="w-3 h-3 bg-emerald-600 rounded-full inline-block"></span>
                                [도식 3] (STEP 3) 연속타설 & 고주파 다짐
                            </h3>
                            <span class="bg-emerald-100 text-emerald-800 text-[10px] font-extrabold px-2.5 py-0.5 rounded-full uppercase">바이브레이터 다짐</span>
                        </div>
                        
                        <div class="clickable-diagram bg-slate-50 p-4 rounded-xl flex justify-center items-center shadow-inner border border-slate-200" onclick="openDiagramZoom('svgDiagram14_STEP3', '[도식 3] 펌프카 연속 타설 & 고주파 바이브레이터 다짐 도면')">
                            <svg id="svgDiagram14_STEP3" viewBox="0 0 420 220" width="100%" height="220" xmlns="http://www.w3.org/2000/svg">
                                <rect x="0" y="0" width="420" height="220" fill="#f8fafc"/>
                                
                                <rect x="40" y="140" width="340" height="50" fill="#fef3c7" stroke="#d97706" stroke-width="2"/>
                                <text x="210" y="170" font-size="11" font-weight="bold" fill="#78350f" text-anchor="middle">HBS 강화노반 지층</text>

                                <rect x="60" y="85" width="300" height="55" fill="#cbd5e1" stroke="#475569" stroke-width="2"/>
                                <text x="210" y="118" font-size="13" font-weight="black" fill="#0f172a" text-anchor="middle">HBR 콘크리트 타설층 (오차 ±10mm)</text>

                                <!-- 펌프카 슈트 및 바이브레이터 -->
                                <line x1="140" y1="20" x2="140" y2="85" stroke="#0284c7" stroke-width="5"/>
                                <text x="140" y="15" font-size="11" font-weight="black" fill="#0284c7" text-anchor="middle">펌프카 타설</text>

                                <line x1="280" y1="40" x2="280" y2="105" stroke="#059669" stroke-width="3" stroke-dasharray="3,2"/>
                                <rect x="275" y="100" width="10" height="20" fill="#059669"/>
                                <text x="280" y="32" font-size="11" font-weight="black" fill="#047857" text-anchor="middle">고주파 다짐</text>
                            </svg>
                        </div>
                    </div>
                    <div class="bg-emerald-50 p-3.5 rounded-xl border border-emerald-100 text-xs text-emerald-900 leading-relaxed">
                        <strong>⚙️ STEP 3 핵심:</strong> 콜드조인트를 예방하기 위해 <strong>펌프카로 연속 타설</strong>을 수행하며, <strong>고주파 바이브레이터로 기포 없이 밀실 다짐</strong>합니다.
                    </div>
                </div>

                <!-- STEP 4 -> 도식 4: 7일 습윤 부직포 포설 양생 & 28일 압축강도(≥21 MPa) 검측 -->
                <div class="bg-white p-6 rounded-2xl border border-slate-200 shadow-md flex flex-col justify-between space-y-4">
                    <div>
                        <div class="flex items-center justify-between mb-3 border-b border-slate-100 pb-2">
                            <h3 class="font-bold text-base text-slate-900 flex items-center gap-2">
                                <span class="w-3 h-3 bg-blue-600 rounded-full inline-block"></span>
                                [도식 4] (STEP 4) 7일 습윤양생 & 강도 승인
                            </h3>
                            <span class="bg-blue-100 text-blue-800 text-[10px] font-extrabold px-2.5 py-0.5 rounded-full uppercase">강도 &ge; 21 MPa</span>
                        </div>
                        
                        <div class="clickable-diagram bg-slate-50 p-4 rounded-xl flex justify-center items-center shadow-inner border border-slate-200" onclick="openDiagramZoom('svgDiagram14_STEP4', '[도식 4] 7일 습윤 부직포 양생 & 28일 압축강도(≥21 MPa) 승인 도면')">
                            <svg id="svgDiagram14_STEP4" viewBox="0 0 420 220" width="100%" height="220" xmlns="http://www.w3.org/2000/svg">
                                <rect x="0" y="0" width="420" height="220" fill="#f8fafc"/>
                                ...
                                <circle cx="100" cy="90" r="3.5" fill="#38bdf8"/>
                                <circle cx="200" cy="90" r="3.5" fill="#38bdf8"/>
                                <circle cx="300" cy="90" r="3.5" fill="#38bdf8"/>
                    ...
                    <div class="bg-blue-50 p-3.5 rounded-xl border border-blue-100 text-xs text-blue-900 leading-relaxed">
                        <strong>💧 STEP 4 핵심:</strong> 습윤 부직포를 덮어 <strong>7일간 분무 양생</strong>을 시행하며, 28일 현장 몰드 시편 <strong>압축강도 &ge; 21 MPa</strong> 확보 후 최종 승인합니다.
                    </div>
                </div>
            </div>
        </div>
            ...
            <div class="space-y-4">
                <div class="bg-white p-5 rounded-xl border border-slate-200 shadow-sm flex items-start gap-4">
                    ...
                    <div>
                        ...
                        <p class="text-xs text-slate-600 mt-1 leading-relaxed">
                            현장 인근 <span class="term-highlight" onclick="openGlossary('plant_distance')">4.8km 이내 배치플랜트 레미콘 공장</span>을 지정하고, 레미콘 출하 후 현장 도착 및 타설 완료까지 <strong>60분 이내</strong>로 정밀 스케줄링 관리합니다. (상기 [도식 1] 참조)
                        </p>
                    </div>
                </div>
                ...
                <div class="bg-white p-5 rounded-xl border border-slate-200 shadow-sm flex items-start gap-4">
                    ...
                    <div>
                        ...
                        <p class="text-xs text-slate-600 mt-1 leading-relaxed">
                            시멘트풀 유출로 인한 공극을 예방하기 위해 거푸집 조인트면 테이핑을 가설하고, 현장 도착 레미콘 차량별로 <span class="term-highlight" onclick="openGlossary('slump_strength')">슬럼프(10cm 이하) 및 공기량(4.5%)</span> 시험을 즉시 수행합니다. (상기 [도식 2] 참조)
                        </p>
                    </div>
                </div>
                ...
                <div class="bg-white p-5 rounded-xl border border-slate-200 shadow-sm flex items-start gap-4">
                    ...
                    <div>
                        ...
                        <p class="text-xs text-slate-600 mt-1 leading-relaxed">
                            펌프카를 투입하여 중단 없는 연속 타설을 시행함으로써 콜드조인트를 예방하고, 고주파 꽂힘 바이브레이터를 투입하여 기포 및 미타설 공극이 없도록 밀실 다짐 조치합니다. (상기 [도식 3] 참조)
                        </p>
                    </div>
                </div>
                ...
                <div class="bg-white p-5 rounded-xl border border-slate-200 shadow-sm flex items-start gap-4">
                    ...
                    <div>
                        ...
                        <p class="text-xs text-slate-600 mt-1 leading-relaxed">
                            타설 두께 오차 <strong>&plusmn;10mm 이내</strong>를 측량 후, 타설 직후 습윤 부직포를 포설하여 <span class="term-highlight" onclick="openGlossary('curing_7days')">7일간 지속 살수 양생</span>을 시행하며, 현장 몰드 시편 28일 압축강도 <strong>&ge; 21 MPa(최소 18 MPa 이상)</strong> 확인 후 감리 승인 마감합니다. (상기 [도식 4] 참조)
                        </p>
                    </div>
                </div>
            </div>
...
force_write(path_gui, guideline_wbs14_4diagrams_html)
force_write(path_gui_alt, guideline_wbs14_4diagrams_html)

print("\n🎉 SUCCESSFULLY UPDATED WBS 14 WITH 4 STEP-BY-STEP DIAGRAMS (1:1 MATCHING)!")
