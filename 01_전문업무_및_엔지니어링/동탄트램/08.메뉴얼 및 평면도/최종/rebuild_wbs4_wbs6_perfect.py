...
# WBS Paths
wbs4_base = r"c:\Users\sskjh\antigravity\01_전문업무_및_엔지니어링\동탄트램\08.메뉴얼 및 평면도\최종\매뉴얼BODY(집행단계-첨부폴더)\콘크리트도상\4_발주전략 KOM"
...
for base in [wbs4_base, wbs6_base]:
    os.makedirs(os.path.join(base, "표준서"), exist_ok=True)
    os.makedirs(os.path.join(base, "수행지침"), exist_ok=True)
    os.makedirs(os.path.join(base, "체크리스트"), exist_ok=True)
...
const glossaryData = {
    ...
    'tcl': {
        ...
        desc: '트램 궤도 하부의 침목과 레일을 일체형으로 고정시켜 열차 하중을 균등하게 지반으로 지지해주는 핵심 무근/철근 콘크리트 구조체(28일 설계 압축강도 fck ≥ 30 MPa)입니다.'
    },
    'grooved_rail': {
...
# =========================================================================
# PART I. REBUILD AND CLEAN WBS 4 (발주전략 KOM - Focus purely on procurement/contract)
# =========================================================================
wbs4_standard = """<!DOCTYPE html>
<html lang="ko">
<head>
    <meta charset="UTF-8">
    ...
    <table>
        ...
        <tbody>
            <tr class="pre-row">
                ...
                <td>
                    <div style="margin-bottom: 8px;">• <strong>[조달 지연/홈레일]</strong> 전량 수입되는 <span class="term-highlight" onclick="openGlossary('grooved_rail')">외산 홈레일</span>의 해외 제작 리드타임 검토 및 선적 대책 수립 여부</div>
                    <div style="margin-bottom: 8px;">• <strong>[가시설/레미콘 BP]</strong> 도상 타설을 위한 인근 <span class="term-highlight" onclick="openGlossary('bp_plant')">가시공 B/P</span>의 공급량 한계 및 기온 조건별 수급성 검토 여부</div>
...
force_write(os.path.join(wbs4_base, "표준서", "발주전략 KOM_표준서.html"), wbs4_standard)
force_write(os.path.join(wbs4_base, "수행지침", "발주전략 KOM_수행지침.html"), wbs4_guideline)
force_write(os.path.join(wbs4_base, "체크리스트", "발주전략 KOM_체크리스트.html"), wbs4_checklist)


# =========================================================================
# PART II. WRITE WBS 6 (콘크리트 타설방법_계획 검토 - Incorporate traffic control details)
# =========================================================================
            ...
            <div class="overflow-x-auto">
                <table class="w-full text-xs sm:text-sm text-left border-collapse border border-slate-200 rounded-lg">
                    ...
                    <tbody class="divide-y divide-slate-200 bg-white">
                        ...
                        <tr>
                            <td class="p-3 font-bold bg-slate-50 border text-center">도로 노면 청소</td>
                            <td class="p-3 border text-center">타설 완료 즉시 개방 조건</td>
                            <td class="p-3 border font-semibold text-slate-700">• 도상 콘크리트 타설 후 도로 노면 고압 세척 청소 및 교통 차단 장비 즉시 수거 (개방 지연 시간 0분 준수)</td>
                        </tr>
                    </tbody>
                </table>
            </div>
...
# =========================================================================
# PART III. COPIER AUTOMATION (Token saving mechanism for both WBS 4 & WBS 6)
# =========================================================================
print("\n🔄 Running fast copier to sync prefixed files for WBS 4 & 6...")

# WBS 4 Sync
shutil.copy(os.path.join(wbs4_base, "표준서", "발주전략 KOM_표준서.html"), os.path.join(wbs4_base, "표준서", "4_발주전략 KOM_표준서.html"))
shutil.copy(os.path.join(wbs4_base, "수행지침", "발주전략 KOM_수행지침.html"), os.path.join(wbs4_base, "수행지침", "4_발주전략 KOM_수행지침.html"))
shutil.copy(os.path.join(wbs4_base, "체크리스트", "발주전략 KOM_체크리스트.html"), os.path.join(wbs4_base, "체크리스트", "4_발주전략 KOM_체크리스트.html"))
...
print("💾 Synced WBS 4 Prefixed copies.")
print("💾 Synced WBS 6 Prefixed copies.")

print("\n🎉 SUCCESSFULLY COMPLETED ALL WBS 4 AND WBS 6 FILE MIGRATIONS AND TOKEN OPTIMIZATION!")
