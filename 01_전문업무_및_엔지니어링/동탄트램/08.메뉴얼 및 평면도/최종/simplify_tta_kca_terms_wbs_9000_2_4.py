...
const glossaryData = {
    ...
    'kec_conduit_33': {
        ...
        desc: '한국전기설비규정(KEC)에 따라 전선관 내부 케이블 단면적 점유율 33% 이하(통신 4.4%, 전기 21.3%, 신호 25.4%)를 완벽 준수하며 상/하선 ELP ø150mm×6개×2조 배치 및 지하 매설 상부 프리캐스트 콘크리트 보호판을 매설하는 수칙입니다.'
    },
    'shared_pole_cctv': {
...
<head>
    ...
    <title>동탄트램 통신분야 - 전기 / 신호 / 기계 / PSD / 차량 인터페이스 협의 표준서 (쉬운 용어 개편)</title>
...
<div class="max-w-5xl mx-auto bg-white shadow-2xl rounded-2xl border border-slate-200 overflow-hidden">
    <div class="bg-slate-900 text-white p-8">
        <span class="bg-blue-600 text-white text-xs font-bold px-3 py-1 rounded-full uppercase">Dongtan Tram Standard (쉬운 용어 개편)</span>
        <h1 class="text-3xl font-black mt-2">전기 / 신호 / 기계 / PSD / 차량 인터페이스 협의 표준서</h1>
        <p class="text-blue-200 text-sm mt-1">L4 Code: 9000-2-4 | 주관: 현장 시스템팀 / 통신·전기·신호·차량 협업업체</p>
    </div>
    ...
    <div class="p-8 space-y-8">
        <div class="bg-blue-50 border border-blue-200 p-6 rounded-xl">
            ...
            <p class="text-slate-700 text-sm font-medium leading-relaxed">
                동탄트램 1단계 시스템 분야 설계 인터페이스 정리 서류(12대 항목)에 준하여 전기(소요전력, 22.9kV 수전, 통합 UPS), 신호(공동관로, 전자연동장치), 통신(96Core 광통신망 통합 포설, CCTV/LTE-R Pole 공용), 차량(무가선 LTO 배터리, 30초 정차 정차역 급속충전), 기계/PSD(비상통화, 4자 배수처리) 간의 시공 단계 인터페이스 협의 회의록 및 1,2공구 종합 관리대장의 무결성을 보장함을 목적으로 함.
            </p>
        </div>

        <div>
            <h3 class="text-lg font-bold text-slate-900 mb-4 border-b-2 border-blue-600 pb-2">📜 12대 시스템 인터페이스 정밀 시방 수칙 (Methodology)</h3>
            <ul class="space-y-3">
                <li class="bg-slate-50 p-4 rounded-xl border border-slate-200 flex items-start gap-3">
                    ...
                    <span class="text-slate-800 text-sm font-medium leading-relaxed"><strong>96Core 광통신망 통합 포설 수칙:</strong> 통신 48Core + 전기 충전기 감시 2Core + 신호 12Core(8Core)를 OF-SM-96Core×2조로 통합 포설하여 케이블 코어를 확정함.</span>
                </li>
                <li class="bg-slate-50 p-4 rounded-xl border border-slate-200 flex items-start gap-3">
                    <span class="bg-blue-600 text-white text-xs font-bold px-2.5 py-1 rounded mt-0.5">수칙 2</span>
                    <span class="text-slate-800 text-sm font-medium leading-relaxed"><strong>KEC 전선관 점유율 33% & 공동관로:</strong> KEC 규정 33% 이하 준수, 상/하선 ELP ø150mm×6개×2조 관로 배치 및 상부 프리캐스트 콘크리트 보호판 매설 수칙을 적용함.</span>
                </li>
                <li class="bg-slate-50 p-4 rounded-xl border border-slate-200 flex items-start gap-3">
                    <span class="bg-blue-600 text-white text-xs font-bold px-2.5 py-1 rounded mt-0.5">수칙 3</span>
                    <span class="text-slate-800 text-sm font-medium leading-relaxed"><strong>CCTV & LTE-R Pole 공용화 수칙:</strong> 12m 강관주 Pole 및 기초대 65개소 공용화, CCTV 제어함체(10kg) 및 LTE-R 안테나(RRU) 공용 지지대의 구조해석을 검측함.</span>
                </li>
                <li class="bg-slate-50 p-4 rounded-xl border border-slate-200 flex items-start gap-3">
                    <span class="bg-blue-600 text-white text-xs font-bold px-2.5 py-1 rounded mt-0.5">수칙 4</span>
                    <span class="text-slate-800 text-sm font-medium leading-relaxed"><strong>통합 UPS (일반 10kVA / 연동 20kVA):</strong> 일반역 10kVA, 연동역 15~20kVA 통합 UPS 채택, 신호 TPD 및 돌발유고장치(500VA) 전원을 통신 UPS에 통합 수용함.</span>
                </li>
                <li class="bg-slate-50 p-4 rounded-xl border border-slate-200 flex items-start gap-3">
                    <span class="bg-blue-600 text-white text-xs font-bold px-2.5 py-1 rounded mt-0.5">수칙 5</span>
                    <span class="text-slate-800 text-sm font-medium leading-relaxed"><strong>차량 무가선 & 30초 정차 급속충전:</strong> LTO 배터리 + 슈퍼캐패시터 차량, 30초 정차 중 5초 상승/20초 충전/5초 하강 집전장치 인터페이스 및 차상 무정전 전원(UPS) 결속 수칙을 반영함.</span>
                </li>
                <li class="bg-slate-50 p-4 rounded-xl border border-slate-200 flex items-start gap-3">
                    <span class="bg-blue-600 text-white text-xs font-bold px-2.5 py-1 rounded mt-0.5">수칙 6</span>
                    <span class="text-slate-800 text-sm font-medium leading-relaxed"><strong>무선통신 호환성 & 전파 합격증 (TTA/KCA):</strong> 무선통신 장비가 서로 잘 터지고 해킹당하지 않는지 국가 인증(TTA)을 수신하고 최종 전파 합격증(KCA) 서류를 확인·체결함.</span>
                </li>
            </ul>
        </div>
        ...
        <div class="bg-emerald-50 border border-emerald-200 p-6 rounded-xl">
            ...
            <p class="text-emerald-900 text-sm font-bold">PDF 12대 규격 반영 인터페이스 회의록, 96Core 광배선도, 통합 UPS 배전반 산출서, 5자 서명 체결 승인서</p>
        </div>
    </div>
</div>
</body>
...
# 2. GUIDELINE HTML (Simplified TTA/KCA terms in STEP 5 HOW)
gui_html = f"""<!DOCTYPE html>
<html lang="ko">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>동탄트램 통신분야 - 전기 / 신호 / 기계 / PSD / 차량 인터페이스 협의 수행지침서 (쉬운 용어 개편)</title>
    <script src="https://cdn.tailwindcss.com"></script>
    <link href="https://fonts.googleapis.com/css2?family=Noto+Sans+KR:wght@300;400;500;700;900&display=swap" rel="stylesheet">
    <style>
...
<div class="max-w-5xl mx-auto bg-white shadow-2xl rounded-2xl border border-slate-200 overflow-hidden">
    <div class="bg-slate-900 text-white p-8 relative">
        <span class="bg-blue-600 text-white text-xs font-bold px-3 py-1 rounded-full uppercase">Dongtan Tram Detailed Guideline (쉬운 용어 개편)</span>
    ...
    <div class="p-6 sm:p-10 space-y-10">
        ...
        <div class="bg-blue-50 border border-blue-200 p-6 rounded-2xl text-sm text-blue-950 space-y-3">
            ...
            <p class="bg-white p-4 rounded-xl border border-blue-300 font-medium text-slate-900 leading-relaxed">
                제공된 동탄트램 1단계 시스템 분야 설계 인터페이스 도서(12대 핵심 항목)에 준하여 현장 엔지니어가 작업 단계별로 <strong>어떻게(HOW) 96Core 광통신망을 포설하고, KEC 33% 이하 점유율로 ELP ø150mm 관로를 검측하며, CCTV/LTE-R 12m Pole 공용화 및 통합 UPS 10~20kVA 배전</strong>을 수행해야 하는지 <strong>Step별 1:1 직관적 2D Visual 기술 도식(그림)</strong>을 함께 수록하였습니다.
        ...
        <div class="bg-blue-50/70 border border-blue-200 p-7 rounded-2xl shadow-md space-y-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                ...
                <div class="bg-white p-5 rounded-xl border border-slate-200 shadow-sm space-y-2">
                    ...
                    <p class="text-slate-700 text-xs">KEC 점유율 33% 이하 준수 (통신 4.4%, 전기 21.3%, 신호 25.4%).</p>
                </div>
                <div class="bg-white p-5 rounded-xl border border-slate-200 shadow-sm space-y-2">
        ...
        <!-- 🔥 2. 초정밀 HOW 세부 실무 가이드 (STEP 5 HOW TTA/KCA 용어 쉬운 설명 개편) -->
        <div class="space-y-8">
            ...
            <div class="space-y-8 text-sm">
                ...
                <div class="bg-white p-6 rounded-2xl border border-blue-200 shadow-sm space-y-4">
                    ...
                    <ul class="list-disc pl-5 text-slate-700 space-y-1.5 text-xs font-medium leading-relaxed">
                        <li><strong>KEC 점유율(≤33%) 계산식 검증:</strong> 한국전기설비규정에 의거 전선관 내부 케이블 총 단면적 합계가 내경 단면적의 33% 이하(상선 통신 4.4%, 전기 21.3%, 신호 25.4%)인지 공학 계산식을 대조합니다.</li>
                    ...
                    <div class="clickable-diagram bg-slate-50 p-4 rounded-xl border border-slate-200 mt-3" onclick="openDiagramZoom('svg_step1', 'STEP 1 KEC 33% 점유율 시각화 & 96Core 광케이블/ELP ø150mm 2D 시공 도식')">
                        <svg id="svg_step1" viewBox="0 0 520 200" width="100%" height="200" xmlns="http://www.w3.org/2000/svg">
                            <rect x="0" y="0" width="520" height="200" fill="#f8fafc"/>
                            <rect x="15" y="15" width="225" height="30" fill="#e2e8f0" stroke="#64748b" stroke-width="2" rx="4"/>
                            <text x="127" y="35" font-size="12" font-weight="black" fill="#334155" text-anchor="middle">📦 프리캐스트 콘크리트 보호판</text>
                            
                            <g transform="translate(15, 55)">
                                <circle cx="22" cy="25" r="18" fill="#ffffff" stroke="#2563eb" stroke-width="2"/>
                                <path d="M 4 25 A 18 18 0 0 0 22 43 L 22 25 Z" fill="#2563eb" opacity="0.6"/>
                                <circle cx="15" cy="30" r="5" fill="#1e3a8a"/>

                                <circle cx="62" cy="25" r="18" fill="#ffffff" stroke="#2563eb" stroke-width="2"/>
                                <path d="M 44 25 A 18 18 0 0 0 62 43 L 62 25 Z" fill="#2563eb" opacity="0.6"/>
                                <circle cx="55" cy="30" r="5" fill="#1e3a8a"/>

                                <circle cx="102" cy="25" r="18" fill="#ffffff" stroke="#2563eb" stroke-width="2"/>
                                <path d="M 84 25 A 18 18 0 0 0 102 43 L 102 25 Z" fill="#2563eb" opacity="0.6"/>
                                <circle cx="95" cy="30" r="5" fill="#1e3a8a"/>

                                <circle cx="142" cy="25" r="18" fill="#ffffff" stroke="#2563eb" stroke-width="2"/>
                                <path d="M 124 25 A 18 18 0 0 0 142 43 L 142 25 Z" fill="#10b981" opacity="0.6"/>
                                <circle cx="135" cy="30" r="5" fill="#047857"/>

                                <circle cx="182" cy="25" r="18" fill="#ffffff" stroke="#2563eb" stroke-width="2"/>
                                <path d="M 164 25 A 18 18 0 0 0 182 43 L 182 25 Z" fill="#f59e0b" opacity="0.6"/>
                                <circle cx="175" cy="30" r="5" fill="#b45309"/>

                                <text x="102" y="65" font-size="11" font-weight="black" fill="#1e3a8a" text-anchor="middle">⚡ ELP ø150mm×6개 (KEC 점유율 ≤ 33% 렌더링)</text>
                                <text x="102" y="82" font-size="10" font-weight="bold" fill="#0369a1" text-anchor="middle">🔵 케이블 채움 33% (여유율 > 67% 방열 확보)</text>
                            </g>

                            <rect x="255" y="15" width="250" height="170" fill="#ffffff" stroke="#0284c7" stroke-width="2" rx="8"/>
                            <text x="380" y="42" font-size="13" font-weight="black" fill="#0369a1" text-anchor="middle">🌐 OF-SM-96Core×2조 통합 포설</text>
                            <text x="270" y="68" font-size="11" font-weight="bold" fill="#334155">• 통신 전송망: 48Core (점유율 4.4%)</text>
                            <text x="270" y="90" font-size="11" font-weight="bold" fill="#334155">• 전기 충전 원격감시: 4Core (21.3%)</text>
                            <text x="270" y="112" font-size="11" font-weight="bold" fill="#334155">• 신호 전자연동장치: 16Core (25.4%)</text>
                            <text x="270" y="134" font-size="11" font-weight="bold" fill="#0284c7">• 예비 코어(20% 이상): 28Core 확보</text>
                            <text x="270" y="156" font-size="10" font-weight="bold" fill="#15803d">✔ KEC 규정 점유율 33% 이하 검증 완료</text>
                            <text x="270" y="174" font-size="10" font-weight="bold" fill="#64748b">└ 케이블 발열 차단 및 추후 증설 용이</text>
                        </svg>
                    </div>
                </div>
                ...
                <!-- STEP 5 HOW + Visual (TTA/KCA 용어 쉬운 설명 전면 개편) -->
        ...
        <div class="space-y-6">
            ...
            <div class="clickable-diagram bg-slate-50 p-5 rounded-2xl border border-slate-200 shadow-inner" onclick="openDiagramZoom('svg_r4', '[WBS 9000-2-4] PDF 12대 시스템 인터페이스 2D Visual 도식')">
                <svg id="svg_r4" viewBox="0 0 550 190" width="100%" height="190" xmlns="http://www.w3.org/2000/svg">
                    ...
                    <text x="40" y="116" font-size="11" font-weight="bold" fill="#334155">• KEC 점유율 ≤ 33% (통신 4.4%)</text>
                    <text x="40" y="139" font-size="11" font-weight="bold" fill="#1d4ed8">• 프리캐스트 보호판 지하 매설</text>
...
# CHECKLIST HTML (Simplified TTA/KCA terms in items 7 & 8)
chk_html = f"""<!DOCTYPE html>
<html lang="ko">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>전기 / 신호 / 기계 / PSD / 차량 인터페이스 협의 마스터 체크리스트 (쉬운 용어 개편)</title>
    <script src="https://cdn.tailwindcss.com"></script>
    <link href="https://fonts.googleapis.com/css2?family=Noto+Sans+KR:wght@300;400;500;700;900&display=swap" rel="stylesheet">
    <style>
...
<div class="max-w-5xl mx-auto space-y-6">
    ...
    <div style="display:none;">
        <svg id="svg_chk_step1" viewBox="0 0 520 190" width="100%" height="190" xmlns="http://www.w3.org/2000/svg">
            ...
            <g transform="translate(15, 55)">
                ...
                <circle cx="102" cy="25" r="18" fill="#ffffff" stroke="#2563eb" stroke-width="2"/>
                <path d="M 84 25 A 18 18 0 0 0 102 43 L 102 25 Z" fill="#2563eb" opacity="0.6"/>
                <text x="102" y="65" font-size="11" font-weight="bold" fill="#1e3a8a" text-anchor="middle">⚡ ELP ø150mm (KEC 점유율 ≤ 33%)</text>
            </g>
            <rect x="255" y="15" width="250" height="160" fill="#ffffff" stroke="#0284c7" stroke-width="2" rx="8"/>
            <text x="380" y="42" font-size="13" font-weight="black" fill="#0369a1" text-anchor="middle">🌐 OF-SM-96Core×2조 통합 포설</text>
            <text x="270" y="70" font-size="11" font-weight="bold" fill="#334155">• 통신(4.4%) + 전기(21.3%) + 신호(25.4%)</text>
        ...
        <svg id="svg_chk_step2" viewBox="0 0 520 180" width="100%" height="180" xmlns="http://www.w3.org/2000/svg">
            ...
            <text x="260" y="125" font-size="12" font-weight="black" fill="#0369a1" text-anchor="middle">🔋 이격거리 ≥300mm & 무선통신 호환성·전파 합격증 (TTA/KCA) 확인</text>
            <text x="260" y="148" font-size="11" font-weight="bold" fill="#64748b" text-anchor="middle">전파법 적합 등록 서류 완료</text>
        </svg>
    ...
    <div class="bg-white border border-slate-200 rounded-2xl shadow-xl overflow-hidden">
        <table class="w-full border-collapse">
            ...
            <tbody class="divide-y divide-slate-200">
                ...
                <tr class="hover:bg-slate-50 transition-colors">
                    <td class="p-4 border-b border-slate-200 text-sm font-medium text-slate-800 leading-relaxed"><span class="inline-block bg-blue-100 text-blue-800 font-bold px-2 py-0.5 rounded text-xs mr-2">7. 통신 호환성</span><strong class="text-slate-900">[TTA 인증]</strong> 무선통신 장비 간 끊김 없이 잘 통하고 해킹 안 되는 국가 공인 인증(TTA) 서류를 수신<strong class="text-blue-600 font-bold">하였는가?</strong></td>
                </tr>
                <tr class="hover:bg-slate-50 transition-colors">
                    <td class="p-4 border-b border-slate-200 text-sm font-medium text-slate-800 leading-relaxed"><span class="inline-block bg-blue-100 text-blue-800 font-bold px-2 py-0.5 rounded text-xs mr-2">8. 전파 합격증</span><strong class="text-slate-900">[KCA 필증]</strong> 전파법 기준에 맞는 최종 전파 합격증(KCA 필증) 수수료 반영 서류를 확인<strong class="text-blue-600 font-bold">하였는가?</strong></td>
                </tr>
        ...
        for fn in os.listdir(sp):
            if fn.endswith('.html'):
                with open(os.path.join(sp, fn), 'w', encoding='utf-8') as f_out:
                    f_out.write(content)
                print(f"   ✓ [SIMPLIFIED TTA/KCA TERMS] {s_n} -> {fn}")

print("\n🎉 SUCCESSFULLY SIMPLIFIED ALL TTA/KCA WIRELESS COMMUNICATION TERMS IN WBS 9000-2-4 HTMLs!")
