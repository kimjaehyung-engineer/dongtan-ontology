...
path_wbs16_gui = os.path.join(base_dir, "16_[PST] 전단앵커설치 및 충전재 주입", "수행지침", "[PST] 전단앵커설치 및 충전재 주입_수행지침.html")
path_wbs16_gui_alt = os.path.join(base_dir, "16_[PST] 전단앵커설치 및 충전재 주입", "수행지침", "16_[PST] 전단앵커설치 및 충전재 주입_수행지침.html")

path_wbs11_gui = os.path.join(base_dir, "11_시공계획 수립", "수행지침", "시공계획 수립_수행지침.html")
path_wbs11_gui_alt = os.path.join(base_dir, "11_시공계획 수립", "수행지침", "11_시공계획 수립_수행지침.html")
...
def force_write(path, text):
    ...
    with open(path, 'w', encoding='utf-8') as f:
        f.write(text)
    print(f"✏️ Successfully updated file: {path}")
...
# -------------------------------------------------------------------------
# 1. CLEAN WBS 16 GUIDELINE (REMOVING LOWER REF DIAGRAM)
# -------------------------------------------------------------------------
guideline_wbs16_cleaned_html = """<!DOCTYPE html>
<html lang="ko">
<head>
    <meta charset="UTF-8">
    ...
    <div class="p-6 sm:p-10 space-y-10">
        ...
        <div class="bg-amber-50 border border-amber-200 p-5 rounded-xl text-xs sm:text-sm text-amber-900 shadow-sm">
            ...
            <p class="leading-relaxed">
                HBS 강화노반 및 TCL 기초층과 상부 PST 패널을 일체화하는 <strong><span class="term-highlight" onclick="openGlossary('shear_anchor')">전단앵커 공종</span></strong>입니다. STEP 1부터 STEP 4까지 각 단계에 직관적으로 1:1 매칭되는 <strong>4개의 정밀 2D 기술 도식</strong>을 수록하였으며, 클릭 시 화면 전체에 <strong>초대형 고화질 뷰(`openDiagramZoom`)</strong>로 확대되어 작은 글씨도 시원하게 읽힙니다.
            </p>
        </div>
...
# -------------------------------------------------------------------------
# 2. WBS 11 GUIDELINE (INSERTING REF SIMULATION DIAGRAM AT THE VERY TOP)
# -------------------------------------------------------------------------
guideline_wbs11_with_ref_diagram_html = """<!DOCTYPE html>
<html lang="ko">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
...
<div class="max-w-5xl mx-auto bg-white shadow-2xl rounded-2xl border border-slate-200 overflow-hidden">
    ...
    <div class="bg-slate-900 text-white p-8 relative overflow-hidden">
        ...
        <div class="relative z-10">
            ...
            <p class="text-emerald-200 mt-2 text-sm sm:text-base">"동탄트램 궤도 4단계 수직 지층 구조 메커니즘 & 콘크리트도상 마스터 프로세스"</p>
        </div>
    </div>
    ...
    <div class="p-6 sm:p-10 space-y-10">
        ...
        <div class="border-b-2 border-slate-200 pb-8">
            ...
            <div class="bg-white p-6 rounded-2xl border border-indigo-100 shadow-lg space-y-6">
                ...
                <div class="clickable-diagram bg-slate-50 p-6 rounded-xl flex justify-center items-center shadow-inner border border-indigo-200" onclick="openDiagramZoom('svgDiagramRef_Layers_WBS11', '[마스터 개념] 동탄트램 궤도 4단계 수직 지층 샌드위치 구조도')">
                    <svg id="svgDiagramRef_Layers_WBS11" viewBox="0 0 500 320" width="100%" height="320" xmlns="http://www.w3.org/2000/svg">
        ...
        <!-- 2. 시공계획 수립 4단계 프로세스 체계도 -->
        <div>
            <h2 class="text-xl font-bold text-slate-900 mb-6 flex items-center gap-2 border-b-2 border-emerald-600 pb-2">
                <span class="text-emerald-600">2.</span> 콘크리트도상 시공계획 수립 4단계 마스터 프로세스 (Flow Architecture)
            </h2>
            ...
            <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
                <div class="flow-card bg-amber-50 p-4 rounded-xl border border-amber-200 flex flex-col justify-between space-y-3">
                    <div>
                        <span class="bg-amber-500 text-white text-[10px] font-black px-2 py-0.5 rounded">STEP 1</span>
                        <h4 class="font-bold text-slate-900 text-xs mt-2">설계 도서 & 선후행 검토</h4>
                        <p class="text-[11px] text-slate-600 mt-1 leading-snug">
                            토공/건축 선행 공종 인도 레벨 검측 및 캔트/확폭 설계 치수 검토
                        </p>
                    </div>
                    <div class="bg-white p-2 rounded border border-amber-100 text-[10px] text-amber-800 font-bold">
                        📋 핵심: 설계 도서 및 레벨 확인
                ...
                <div class="flow-card bg-sky-50 p-4 rounded-xl border border-sky-200 flex flex-col justify-between space-y-3">
                    <div>
                        <span class="bg-sky-500 text-white text-[10px] font-black px-2 py-0.5 rounded">STEP 2</span>
                        <h4 class="font-bold text-slate-900 text-xs mt-2">인터페이스 & 자재 계획</h4>
                        <p class="text-[11px] text-slate-600 mt-1 leading-snug">
                            전력/신호 매설관로 인터페이스 협의 & 레일/PST/그라우트 수급 수립
                        </p>
                    </div>
                    <div class="bg-white p-2 rounded border border-sky-100 text-[10px] text-sky-800 font-bold">
                        ⚙️ 핵심: 시스템 인터페이스 협의
                ...
                <div class="flow-card bg-emerald-50 p-4 rounded-xl border border-emerald-200 flex flex-col justify-between space-y-3">
                    <div>
                        <span class="bg-emerald-600 text-white text-[10px] font-black px-2 py-0.5 rounded">STEP 3</span>
                        <h4 class="font-bold text-slate-900 text-xs mt-2">장비 수급 & 3D 측량 셋팅</h4>
                        <p class="text-[11px] text-slate-600 mt-1 leading-snug">
                            궤광 템플릿, 궤도 잭, 펌프카 배치 및 광파 토탈스테이션 3D 좌표 셋팅
                        </p>
                    </div>
                    <div class="bg-white p-2 rounded border border-emerald-100 text-[10px] text-emerald-800 font-bold">
                        📐 핵심: 광파 3D 측량계 세팅
                ...
                <div class="flow-card bg-blue-50 p-4 rounded-xl border border-blue-200 flex flex-col justify-between space-y-3">
                    <div>
                        <span class="bg-blue-600 text-white text-[10px] font-black px-2 py-0.5 rounded">STEP 4</span>
                        <h4 class="font-bold text-slate-900 text-xs mt-2">안전·품질 시공계획서 승인</h4>
                        <p class="text-[11px] text-slate-600 mt-1 leading-snug">
                            위험성평가, 품질시험계획서 수립 및 감리단 입회 시공계획서 최종 승인
                        </p>
                    </div>
                    <div class="bg-white p-2 rounded border border-blue-100 text-[10px] text-blue-800 font-bold">
                        ✅ 핵심: 감리단 최종 착공 승인
...
# Force write
force_write(path_wbs16_gui, guideline_wbs16_cleaned_html)
force_write(path_wbs16_gui_alt, guideline_wbs16_cleaned_html)

force_write(path_wbs11_gui, guideline_wbs11_with_ref_diagram_html)
force_write(path_wbs11_gui_alt, guideline_wbs11_with_ref_diagram_html)

print("\n🎉 SUCCESSFULLY RELOCATED REFERENCE SIMULATION DIAGRAM TO WBS 11 GUIDELINE FRONT!")
