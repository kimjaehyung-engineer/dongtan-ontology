...
const glossaryData = {
    'integrated_core_96c': {
        title: '🌐 96Core 광 통신망 통합 포설 (13.02억원 절감)',
        desc: '통신 48Core + 전기 충전기 원격감시 2Core + 신호 전자연동 12Core(8Core)를 OF-SM-96Core×2조로 통합 포설하여 개별 포설 대비 공사비 13.02억원을 절감하고 무결성을 확보하는 설계 수칙입니다.'
    },
    'kec_conduit_33': {
    ...
    'shared_pole_cctv': {
        title: '📷 교차로 CCTV & LTE-R Pole 공용화 (9.7억원 절감)',
        desc: 'CCTV용 12m 강관주 Pole 및 기초대 65개소를 공용화(9.7억원 절감)하고, CCTV 제어함체(10kg) 및 LTE-R 안테나(RRU) 공용 지지대의 구조해석 검측을 완료하는 지침입니다.'
    },
    'integrated_ups_10_20k': {
        title: '🔋 통합 UPS 전원 배정 (일반역 10kVA / 연동역 20kVA)',
        desc: '일반역사(25개소) 10kVA, 연동역사(7개소) 15~20kVA 통합 UPS 채택으로 최대 11.59억원을 절감하고 신호 TPD 및 돌발유고장치(500VA)에 24시간 무정전 전원을 수용하는 수칙입니다.'
    }
};
...
<head>
    ...
    <title>동탄트램 통신분야 - 전기 / 신호 / 기계 / PSD / 차량 인터페이스 협의 표준서 (PDF 12대 규격 수록)</title>
...
<div class="max-w-5xl mx-auto bg-white shadow-2xl rounded-2xl border border-slate-200 overflow-hidden">
    <div class="bg-slate-900 text-white p-8">
        <span class="bg-blue-600 text-white text-xs font-bold px-3 py-1 rounded-full uppercase">Dongtan Tram Standard (PDF 12대 규격 수록)</span>
        ...
        <div>
            ...
            <ul class="space-y-3">
                <li class="bg-slate-50 p-4 rounded-xl border border-slate-200 flex items-start gap-3">
                    ...
                    <span class="text-slate-800 text-sm font-medium leading-relaxed"><strong>96Core 광통신망 통합 포설 (13.02억 절감):</strong> 통신 48Core + 전기 충전기 감시 2Core + 신호 12Core(8Core)를 OF-SM-96Core×2조로 통합 포설하여 예산 13.02억원을 절감하고 코어를 확정함.</span>
                </li>
                <li class="bg-slate-50 p-4 rounded-xl border border-slate-200 flex items-start gap-3">
                ...
                <li class="bg-slate-50 p-4 rounded-xl border border-slate-200 flex items-start gap-3">
                    ...
                    <span class="text-slate-800 text-sm font-medium leading-relaxed"><strong>CCTV & LTE-R Pole 공용화 (9.7억 절감):</strong> 12m 강관주 Pole 및 기초대 65개소 공용화(9.7억 절감), CCTV 제어함체(10kg) 및 LTE-R 안테나(RRU) 공용 지지대의 구조해석을 검측함.</span>
                </li>
                <li class="bg-slate-50 p-4 rounded-xl border border-slate-200 flex items-start gap-3">
                    <span class="bg-blue-600 text-white text-xs font-bold px-2.5 py-1 rounded mt-0.5">수칙 4</span>
                    <span class="text-slate-800 text-sm font-medium leading-relaxed"><strong>통합 UPS (일반 10kVA / 연동 20kVA):</strong> 일반역 10kVA, 연동역 15~20kVA 통합 UPS 채택(최대 11.59억 절감), 신호 TPD 및 돌발유고장치(500VA) 전원을 통신 UPS에 통합 수용함.</span>
                </li>
                <li class="bg-slate-50 p-4 rounded-xl border border-slate-200 flex items-start gap-3">
                ...
                <li class="bg-slate-50 p-4 rounded-xl border border-slate-200 flex items-start gap-3">
                    ...
                    <span class="text-slate-800 text-sm font-medium leading-relaxed"><strong>TTA 보안성 검증 & 선로전환기 배수:</strong> TTA 상호운용성/보안성 검증 및 KCA 전파사용료 반영, 선로전환기(노반 배수 파이프) & 차축검지기(궤도/신호/토목 4자 배수) 연동 수칙을 체결함.</span>
                </li>
            </ul>
        </div>
...
# 2. GUIDELINE HTML (PDF 12대 규격 HOW 세부 절차 강화)
gui_html = f"""<!DOCTYPE html>
<html lang="ko">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>동탄트램 통신분야 - 전기 / 신호 / 기계 / PSD / 차량 인터페이스 협의 수행지침서 (PDF 12대 규격 반영)</title>
    <script src="https://cdn.tailwindcss.com"></script>
    <link href="https://fonts.googleapis.com/css2?family=Noto+Sans+KR:wght@300;400;500;700;900&display=swap" rel="stylesheet">
    <style>
...
<div class="max-w-5xl mx-auto bg-white shadow-2xl rounded-2xl border border-slate-200 overflow-hidden">
    <div class="bg-slate-900 text-white p-8 relative">
        <span class="bg-blue-600 text-white text-xs font-bold px-3 py-1 rounded-full uppercase">Dongtan Tram Detailed Guideline (PDF 12대 규격 반영)</span>
        <h1 class="text-3xl font-black mt-2">전기 / 신호 / 기계 / PSD / 차량 인터페이스 초정밀 수행지침서</h1>
        <p class="text-blue-200 text-sm mt-1">L4 Code: 9000-2-4 | 주관: 현장 시스템팀 / 통신·전기·신호·차량 협업업체 | "어떻게(HOW) 연동하는가 초정밀 가이드"</p>
    </div>
    ...
    <div class="p-6 sm:p-10 space-y-10">
        ...
        <div class="bg-blue-50 border border-blue-200 p-6 rounded-2xl text-sm text-blue-950 space-y-3">
            ...
            <p class="bg-white p-4 rounded-xl border border-blue-300 font-medium text-slate-900 leading-relaxed">
                제공된 동탄트램 1단계 시스템 분야 설계 인터페이스 도서(12대 핵심 항목)에 준하여 현장 엔지니어가 작업 단계별로 <strong>어떻게(HOW) 96Core 광통신망을 포설하고, ELP ø150mm 관로를 검측하며, CCTV/LTE-R 12m Pole 공용화 및 통합 UPS 10~20kVA 배전</strong>을 수행해야 하는지 구체적 기술 수치와 행동 수칙을 제공합니다. 
                특히 <strong><span class="term-highlight" onclick="openGlossary('integrated_core_96c')">96Core 광통신망 통합</span></strong>, <strong><span class="term-highlight" onclick="openGlossary('kec_conduit_33')">KEC 33% & ELP ø150mm</span></strong>, <strong><span class="term-highlight" onclick="openGlossary('shared_pole_cctv')">CCTV/LTE-R Pole 공용</span></strong> 및 <strong><span class="term-highlight" onclick="openGlossary('integrated_ups_10_20k')">통합 UPS 배전</span></strong> 수칙을 완벽 수록하였습니다.
            </p>
        </div>
        ...
        <div class="bg-blue-50/70 border border-blue-200 p-7 rounded-2xl shadow-md space-y-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                <div class="bg-white p-5 rounded-xl border border-slate-200 shadow-sm space-y-2">
                    <span class="font-bold text-blue-700 flex items-center gap-2"><span>🌐</span> 1. 96Core 광통신망 (13.02억 절감)</span>
                ...
                <div class="bg-white p-5 rounded-xl border border-slate-200 shadow-sm space-y-2">
                    <span class="font-bold text-blue-700 flex items-center gap-2"><span>📷</span> 3. CCTV & LTE-R Pole (9.7억 절감)</span>
                ...
                <div class="bg-white p-5 rounded-xl border border-slate-200 shadow-sm space-y-2">
                    ...
                    <p class="text-slate-700 text-xs">일반역 10kVA, 연동역 20kVA 통합 UPS 채택 (최대 11.59억 절감).</p>
                </div>
            </div>
        </div>
        ...
        <!-- 🔥 2. 초정밀 HOW(어떻게 수행하는가) 세부 실무 가이드 (PDF 12대 규격 수록) -->
        <div class="space-y-6">
            <h2 class="text-xl font-bold text-slate-900 mb-4 border-b-2 border-indigo-600 pb-2">
                <span class="text-indigo-600">2.</span> 초정밀 HOW(어떻게 수행하는가) 세부 실무 가이드 (PDF 12대 규격 수록)
            </h2>
            ...
            <div class="space-y-4 text-sm">
                ...
                <div class="bg-white p-6 rounded-2xl border border-indigo-200 shadow-sm space-y-3">
                    ...
                    <ul class="list-disc pl-5 text-slate-700 space-y-1.5 text-xs font-medium leading-relaxed">
                        <li><strong>12m 강관주 Pole 65개소 위치 측량:</strong> 교차로 감시 CCTV용 12m 강관주 및 기초대 65개소 위치를 현장 측량하고 독립 Pole 설치 대비 공사비 9.7억원을 절감하도록 설계합니다.</li>
                ...
                <div class="bg-white p-6 rounded-2xl border border-teal-200 shadow-sm space-y-3">
                    ...
                    <ul class="list-disc pl-5 text-slate-700 space-y-1.5 text-xs font-medium leading-relaxed">
                        <li><strong>역사별 통합 UPS 용량 검측:</strong> 일반역사(25개소) 10kVA, 연동역사(7개소) 15~20kVA 통합 UPS 분전반을 통신 기계실에 배치하고 개별 UPS 대비 최대 11.59억원 절감을 확인합니다.</li>
        ...
        <div class="space-y-6">
            ...
            <div class="clickable-diagram bg-slate-50 p-5 rounded-2xl border border-slate-200 shadow-inner" onclick="openDiagramZoom('svg_r4', '[WBS 9000-2-4] PDF 12대 시스템 인터페이스 2D Visual 도식')">
                <svg id="svg_r4" viewBox="0 0 550 180" width="100%" height="180" xmlns="http://www.w3.org/2000/svg">
                    ...
                    <text x="40" y="75" font-size="11" font-weight="bold" fill="#334155">• OF-SM-96Core×2조 (13.02억 절감)</text>
                    ...
                    <text x="325" y="75" font-size="11" font-weight="bold" fill="#334155">• 12m 강관주 65개 공용 (9.7억 절감)</text>
                    <text x="325" y="98" font-size="11" font-weight="bold" fill="#334155">• 통합 UPS 10~20kVA (11.59억 절감)</text>
...
# 3. CHECKLIST HTML (3-Column Premium Master Template - PDF 12대 규격 수록)
chk_html = f"""<!DOCTYPE html>
<html lang="ko">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>전기 / 신호 / 기계 / PSD / 차량 인터페이스 협의 마스터 체크리스트 (PDF 12대 규격)</title>
    <script src="https://cdn.tailwindcss.com"></script>
    <link href="https://fonts.googleapis.com/css2?family=Noto+Sans+KR:wght@300;400;500;700;900&display=swap" rel="stylesheet">
    <style>
...
<div class="max-w-5xl mx-auto space-y-6">
    ...
    <div class="bg-blue-50/80 border border-blue-200 rounded-2xl p-6 shadow-sm space-y-2">
        ...
        <p class="text-xs text-blue-900 leading-relaxed font-medium">
            본 체크리스트는 시스템 인터페이스 정리 서류(12대 규격)의 96Core 광망, KEC 점유율, 12m Pole 공용, 통합 UPS 수칙을 12개 정밀 점검 항목으로 구성하였으며, 모든 항목의 문장 끝은 예외 없이 질문형 어미(<strong class="text-blue-700">~하였는가?</strong>)로 100% 정형화되었습니다.
        </p>
    </div>
        ...
        for fn in os.listdir(sp):
            if fn.endswith('.html'):
                with open(os.path.join(sp, fn), 'w', encoding='utf-8') as f_out:
                    f_out.write(content)
                print(f"   ✓ [PDF SPECS UPGRADED WBS 9000-2-4] {s_n} -> {fn}")

print("\n🎉 SUCCESSFULLY UPGRADED WBS 9000-2-4 HTML FILES WITH 12 PDF SYSTEM INTERFACE SPECS!")
