      ...
      <div className="p-4 space-y-3 text-xs">
        ...
        <div className="grid grid-cols-1 md:grid-cols-5 gap-2.5">
          {diag.metrics.map((m, idx) => {
            const rawPct = Math.round(m.actualRatio * 100);
            const isOptimal = m.status === 'OPTIMAL';
            const isOverload = m.status === 'OVERLOAD';
            const isCriticalNG = m.status === 'CRITICAL_NG' || m.actualRatio > 1.0;
            const excessPct = Math.max(0, rawPct - 100);
              ...
              >
                <div className="flex justify-between items-center">
                  ...
                  <span className={`text-[10px] font-mono font-bold px-1.5 py-0.2 rounded ${
                    ...
                    {isOptimal ? '✓ 골든존' : isCriticalNG ? `✕ 위험 NG (+${excessPct}%)` : isOverload ? '⚠ 하중집중' : '○ 과다설계'}
                  </span>
                </div>
                ...
                <div className="flex items-baseline justify-between font-mono">
                  ...
                  <div className="flex items-baseline gap-1">
                    <strong className={`text-sm ${
                      isOptimal ? 'text-emerald-700' : isCriticalNG ? 'text-rose-700 font-black' : isOverload ? 'text-amber-700' : 'text-slate-700'
                    }`}>
                      {rawPct}%
                    </strong>
                    {isCriticalNG && (
                      <span className="text-[10px] font-extrabold text-rose-600 bg-rose-100 px-1 py-0.2 rounded border border-rose-300">
                        (+{excessPct}% 초과)
                      </span>
                    )}
                  </div>
                ...
                <div className="space-y-0.5">
                  <div className="w-full bg-slate-200 h-2 rounded-full overflow-hidden relative">
                    ...
                    <div
                      className={`h-full rounded-full transition-all duration-500 z-10 relative ${
                        isOptimal ? 'bg-emerald-600' : isCriticalNG ? 'bg-rose-600' : isOverload ? 'bg-amber-500' : 'bg-blue-500'
                      }`}
                  ...
                  <div className="flex justify-between text-[9px] text-slate-400 font-mono">
                    ...
                    <span className={isCriticalNG ? 'text-rose-600 font-black' : ''}>100% {isCriticalNG && `(초과: ${rawPct}%)`}</span>
                  </div>
                </div>

                <p className={`text-[10px] font-sans leading-tight ${isCriticalNG ? 'text-rose-700 font-bold' : 'text-slate-600'}`}>
                  {m.statusText}
                </p>
              </div>
            );
