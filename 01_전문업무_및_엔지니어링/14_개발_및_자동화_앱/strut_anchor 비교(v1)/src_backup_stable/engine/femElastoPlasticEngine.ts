  ...
  public solveStages(stages: ExcavationStage[]): StageResult[] {
    ...
    for (const stage of stages) {
      ...
      for (let i = 0; i < stage.activeSupports.length; i++) {
        ...
        const sf = sup.allowableCapacity / Math.max(1, axialForce);
        const strutStressRatio = Number((axialForce / Math.max(1, sup.allowableCapacity)).toFixed(2));
        ...
        supResults.push({
          ...
          allowableForce: sup.allowableCapacity,
...
export class AlternativeEvaluator {
  public static generate4Alternatives(inputs: ProjectInputs): AlternativeSpec[] {
    ...
    // 지보단수 결정 (대략 2.5m~3.0m 간격)
    const numTiers = Math.max(2, Math.min(6, Math.floor(excavationDepth / 2.8)));
    const tierSpacings: number[] = [];
    for (let i = 0; i < numTiers; i++) {
      tierSpacings.push(Number((1.5 + i * ((excavationDepth - 2.5) / Math.max(1, numTiers - 1))).toFixed(1)));
    }
    ...
    // 사용자 지정 앵커 각도 (기본 45° 고각)
    const userAnchorAngle = inputs.supports.find(s => s.type === 'GROUND_ANCHOR')?.angle || 45;

    // 2. 대안 2: All Anchor (고각 45°~70° 적용)
    ...
    const hybridSups: SupportStage[] = tierSpacings.map((depth, idx) => {
      ...
      return {
        ...
        depth,
        angle,
        horizSpacing: isUpper ? 2.0 : userStrutSpacing,
  ...
  private static evaluateSingleAlt(
    ...
    for (let i = 0; i < supports.length; i++) {
      ...
      const nextExc = (i + 1 < supports.length) ? Math.min(excavationDepth, supports[i + 1].depth + 0.6) : excavationDepth;
      stages.push({
        stage: i + 2,
    ...
    const engine = new FEMElastoPlasticEngine(soils, wall, 0.1);
