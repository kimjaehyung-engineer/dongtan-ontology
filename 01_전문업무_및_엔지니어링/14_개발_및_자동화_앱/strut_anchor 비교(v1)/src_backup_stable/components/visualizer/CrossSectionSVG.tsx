          ...
          <svg width={svgWidth} height={svgHeight} className="select-none font-sans text-xs">
            ...
            {inputs.deckingConfig?.useDecking !== false && (
              <g>
                {/* 중간말뚝 H-Pile 본체 */}
                <rect
                  x={wallLeftX + (excWidth * scaleX) / 2 - 4}
                  y={groundY}
                  width={8}
                  height={(inputs.deckingConfig?.kingPostTotalLength || totalLength) * scaleY}
                  fill="#64748b"
                  stroke="#334155"
                  strokeWidth="1.5"
                  rx="1"
                ...
                <g transform={`translate(${wallLeftX + (excWidth * scaleX) / 2 + 10}, ${groundY + 15})`}>
                  <rect x="0" y="0" width="175" height="16" rx="2" fill="#ffffff" fillOpacity="0.9" stroke="#cbd5e1" strokeWidth="0.8" />
                  <text x="4" y="12" fill="#334155" fontSize="9.5" fontWeight="bold">
                    중간말뚝 ({inputs.deckingConfig?.kingPostSpec || 'H-300x300'} @ {inputs.deckingConfig?.kingPostSpacing || 3.5}m)
                  </text>
                </g>
              </g>
            )}
