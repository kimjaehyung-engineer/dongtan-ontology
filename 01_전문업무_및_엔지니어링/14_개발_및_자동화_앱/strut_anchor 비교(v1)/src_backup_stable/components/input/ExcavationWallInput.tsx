...
import { Construction, Box, Waves, ArrowDownUp, Ruler, ShieldAlert, Cpu, CheckCircle2, Layers, Sliders, Truck, Anchor, Sparkles, AlertTriangle } from 'lucide-react';
  ...
  const handleBatchSupportChange = (field: 'spec' | 'spacing' | 'wale' | 'anchorAngle' | 'strandCount', value: any) => {
    ...
    const updatedSups = inputs.supports.map((s, idx) => {
      ...
      if (field === 'anchorAngle') {
        ...
        return { ...s, horizSpacing: parseFloat(value) || (isAnchor ? 2.0 : 3.5) };
      } else if (field === 'wale') {
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
          ...
          <div className="p-3.5 space-y-2.5 text-xs">
            ...
            <div>
              <div className="flex justify-between items-center mb-1">
                ...
                <span className="text-[11px] text-slate-400 font-mono">표준: 1.5m ~ 2.0m</span>
              </div>
              <div className="flex items-center gap-1.5">
                {[1.5, 1.8, 2.0, 2.5].map((sp) => (
                  <button
                    key={sp}
                ...
                <input
                  ...
                  onChange={(e) => handleHPileSpacingChange(parseFloat(e.target.value) || 1.8)}
        ...
        <div className="eng-panel">
          ...
          <div className="p-3.5 space-y-2.5 text-xs">
            {!showTierDetail ? (
              <>
                {/* 1. 어스앵커 모드인 경우 앵커 각도 및 강연선 선택 표시 */}
                {isAnchorMode ? (
                  <>
                    <div>
                    ...
                    <div>
                      <div className="flex justify-between items-center mb-1">
                        ...
                        <span className="text-[11px] text-slate-400 font-mono">표준: 1.5m ~ 2.5m</span>
                      </div>
                      <div className="flex items-center gap-1.5">
                        {[1.5, 1.8, 2.0, 2.5].map((sp) => (
                          <button
                    ...
                    <div className="bg-blue-50/60 p-2 rounded border border-blue-200 space-y-1.5">
                      ...
                      <div className="grid grid-cols-2 gap-1.5">
                        ...
                        <select
                          ...
                          className="w-full bg-white border border-slate-300 rounded px-1.5 py-1 text-[11px] font-mono font-bold text-slate-800 focus:border-blue-500 focus:outline-none"
                        >
                          {[2.5, 3.0, 3.5, 4.0, 5.0].map(sp => (
                            <option key={sp} value={sp}>버팀보 @ {sp}m</option>
                          ))}
                        </select>
                      </div>
                    </div>
                  ...
                  /* 3. 일반 버팀보 모드인 경우 버팀보 규격 및 간격 표시 */
                  <>
                    <div>
                    ...
                    <div>
                      <div className="flex justify-between items-center mb-1">
                        ...
                        <span className="text-[11px] text-slate-400 font-mono">표준: 3.0m ~ 4.5m</span>
                      </div>
                      <div className="flex items-center gap-1.5">
                        {[2.5, 3.0, 3.5, 4.0, 5.0].map((sp) => (
                          <button
