...
file_v1 = files[0]
print("Fixing V1 HTML file:", file_v1)

with open(file_v1, 'r', encoding='utf-8') as f:
    content = f.read()

# 1. Fix showTooltip to use innerHTML instead of textContent
old_tooltip_func = """function showTooltip(e, text) {
  tip.textContent = text;
  tip.classList.add("show");
  positionTooltip(e);
}"""

new_tooltip_func = """function showTooltip(e, text) {
  tip.innerHTML = text;
  tip.classList.add("show");
  positionTooltip(e);
}"""

if 'tip.textContent = text;' in content:
    content = content.replace(old_tooltip_func, new_tooltip_func)
    # Also check single line or alternative pattern
    content = content.replace('tip.textContent = text;', 'tip.innerHTML = text;')
    print("Fixed tooltip innerHTML rendering!")

# 2. Update renderIntersections JS function to remove ugly double-terminal circles
# and render a single clean intersection node dot + staggered label badge
...
js_rendering_logic_fixed = """// --- 교차로 노선 구간 정밀 매칭 & 지시선/슬라이딩 패널 렌더링 ---
...
start_js_pos = content.find("// --- 교차로 노선 구간")
if start_js_pos == -1:
    start_js_pos = content.find("// --- 교차로 정보")
end_js_pos = content.find("function selectIntersection(item) {", start_js_pos)

if start_js_pos != -1 and end_js_pos != -1:
    content = content[:start_js_pos] + js_rendering_logic_fixed + "\n\n" + content[end_js_pos:]
    print("Updated JS rendering logic successfully!")

with open(file_v1, 'w', encoding='utf-8') as f:
    f.write(content)

print("Finished fixing tooltips & intersection nodes rendering in V1 HTML!")
