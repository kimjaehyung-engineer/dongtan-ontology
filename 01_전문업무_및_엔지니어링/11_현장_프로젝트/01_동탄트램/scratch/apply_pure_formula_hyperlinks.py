# -*- coding: utf-8 -*-
"""
동탄트램 8대 공종 전체 엑셀(v8.xlsx, v8.xlsm) 순수 =HYPERLINK() 표준 수식 100% 무결점 주입기
(Excel 자체 표준 함수 =HYPERLINK("상대경로", "라벨") 사용, XML 충돌 객체 완전 배제)
"""
...
# Kill any stuck EXCEL processes before writing
...
for target_file in [XLSX_PATH, XLSM_PATH]:
    ...
    for sheet_name, folder_name in DISCIPLINE_MAP.items():
        ...
        # 1. 시트 내 표준서, 수행지침, 체크리스트 열 위치 동적 탐색
        col_std = None
        col_guide = None
        col_chk = None
        ...
        print(f"[{sheet_name}] ({folder_name}) -> 열 위치: 표준서={col_std}, 수행지침={col_guide}, 체크리스트={col_chk}")

        # 2. 각 행별로 정확한 =HYPERLINK() 수식 주입 (XML 관계 객체는 제거)
        linked_count = 0
        for r in range(2, ws.max_row + 1):
            ...
            if t_idx < len(subdirs):
                ...
                if std_f and os.path.exists(std_f):
                    ...
                    cell_std.value = f'=HYPERLINK("{rel_path}", "👉 [클릭] 표준서 열기 📄")'
                ...
                if guide_f and os.path.exists(guide_f):
                    ...
                    cell_guide.value = f'=HYPERLINK("{rel_path}", "👉 [클릭] 수행지침 열기 📄")'
                ...
                if chk_f and os.path.exists(chk_f):
                    ...
                    cell_chk.value = f'=HYPERLINK("{rel_path}", "👉 [클릭] 체크리스트 열기 📄")'
        ...
        print(f"  ✓ {sheet_name} 총 {linked_count}개 행 순수 =HYPERLINK() 수식 주입 완료")

    wb.save(target_file)
    print(f"✓ {os.path.basename(target_file)} 저장 완료!")

print("\n==================================================")
print("8대 공종 전체 순수 =HYPERLINK() 표준 수식 100% 적용 완료!")
print("==================================================")
