# -*- coding: utf-8 -*-
"""
동탄트램 11.철도종합시운전 28개 액티비티 엑셀 데이터 완벽 입력 및 하이퍼링크 주입 스크립트
(통신/기계 시트 22개 열 완벽 준용 + 동적 절대경로 수식 100% 바인딩)
"""
...
# D-Day 및 주관 부서 매핑
DDAYS = [
    "D-60", "D-50", "D-40", "D-35", "D-30", "D-30", "D-25", "D-20",
    "D+0", "D+30", "D+60", "D+80", "D+120", "D+130", "D+175", "D+180",
    "D+185", "D+190", "D+195", "D+205", "D+210", "D+215", "D+220", "D+225",
    "D+230", "D+235", "D+238", "D+240"
]

DEPTS = [
    "개통운영팀", "개통운영팀", "시스템팀", "시스템팀", "시스템팀", "개통운영팀", "개통운영팀", "시공사업단",
    "개통운영팀", "개통운영팀", "시공사업단", "안전품질팀", "시공사업단", "시공사업단", "시공사업단", "시공사업단",
    "개통운영팀", "개통운영팀", "개통운영팀", "시공사업단", "개통운영팀", "화성도시공사", "개통운영팀", "시공사업단",
    "시공사업단", "화성도시공사", "시공사업단", "시공사업단"
]
...
for target_file in TARGET_FILES:
    ...
    # 철도종합시운전 시트 가져오기 또는 생성
    if "철도종합시운전" in wb.sheetnames:
        ws = wb["철도종합시운전"]
        # 기존 데이터 클리어
    ...
    for idx, item in enumerate(ALL_TASKS, 1):
        ...
        # 선행/후행
        pred = f"9000-9-{idx-1}" if idx > 1 else "-"
        succ = f"9000-9-{idx+1}" if idx < len(ALL_TASKS) else "-"
        dday = DDAYS[idx-1]
        dept = DEPTS[idx-1]

        # 요약문 및 상세 방법 생성
        method_detail = f"1) {steps[0][0]}: {steps[0][1]} ({steps[0][2][0]})\n2) {steps[1][0]}: {steps[1][1]} ({steps[1][2][0]})\n3) {steps[2][0]}: {steps[2][1]} ({steps[2][2][0]})"
        std_sum = f"1) {task_title} ({wbs_code}) 업무 달성을 위해 철도안전법 제38조 및 국토부 시행지침을 100% 충족한다.\n2) {subtitle}을 엄격히 완수하여 국토교통부/TS 승인을 획득한다."
        ...
        if is_inside_attach:
            rel_std = f"{COMMISH_DIR_NAME}\\{folder_name}\\표준서\\{folder_name}_표준서.html"
            rel_guide = f"{COMMISH_DIR_NAME}\\{folder_name}\\수행지침\\{folder_name}_수행지침.html"
            rel_chk = f"{COMMISH_DIR_NAME}\\{folder_name}\\체크리스트\\{folder_name}_체크리스트.html"
        else:
            rel_std = f"매뉴얼BODY(집행단계-첨부폴더)v8\\{COMMISH_DIR_NAME}\\{folder_name}\\표준서\\{folder_name}_표준서.html"
            rel_guide = f"매뉴얼BODY(집행단계-첨부폴더)v8\\{COMMISH_DIR_NAME}\\{folder_name}\\수행지침\\{folder_name}_수행지침.html"
            rel_chk = f"매뉴얼BODY(집행단계-첨부폴더)v8\\{COMMISH_DIR_NAME}\\{folder_name}\\체크리스트\\{folder_name}_체크리스트.html"

        row_values = [
            (1, "9000", font_data_bold, align_center),
            (2, "9000-9", font_data_bold, align_center),
            (3, "철도종합시험운행", font_data_bold, align_center),
            ...
            (9, dept, font_data, align_center),
            (10, subtitle, font_data, align_left),
            (11, method_detail, font_data, align_left),
            (12, f"{task_title} 결과보고서, 회의록 및 TS 공인 필증", font_data, align_left),
            ...
            (19, "철도종합시험운행 시행지침 준수", font_data, align_center),
            (20, "● 철도안전법 제38조, 동법 시행규칙 제75조 및 국토부 철도종합시험운행 시행지침 100% 준수", font_data, align_left),
            (21, "1) 👉 [더블클릭] 체크리스트 열기 📄", font_data_bold, align_center),
            (22, "한국교통안전공단(TS), 국가철도공단 및 철도종합시운전 기술자문단 상시 운영", font_data, align_left)
        ]
    ...
    wb.save(target_file)
    print(f"  ✓ {os.path.basename(target_file)} 철도종합시운전 시트 28개 액티비티 100% 저장 완료!")

# 전용 하이퍼링크 목록 엑셀 파일에도 철도종합시운전 시트 추가
...
if os.path.exists(NAV_XLSX):
    ...
    if "철도종합시운전" in wb_nav.sheetnames:
        wb_nav.remove(wb_nav["철도종합시운전"])
    ws_nav = wb_nav.create_sheet("철도종합시운전")
    ws_nav.views.sheetView[0].showGridLines = True

    # 타이틀
    ws_nav.merge_cells("A1:E1")
    ws_nav["A1"].value = "동탄도시철도(트램) 철도종합시운전 엔지니어링 매뉴얼 링크 대장"
    ...
    print(f"  ✓ 동탄트램_공종별_매뉴얼_하이퍼링크_목록.xlsx 내 [철도종합시운전] 시트 추가 완료!")

print(f"\n==================================================")
print(f"전체 대상 엑셀 파일 철도종합시운전 시트 28개 액티비티 연동 완료!")
print(f"==================================================")
