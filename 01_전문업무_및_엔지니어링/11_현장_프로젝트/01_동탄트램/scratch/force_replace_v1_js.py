...
# Find start of intersectionData
start_pos = content.find("const intersectionData =")
end_pos = content.find("function renderInteractiveElements() {", start_pos)
...
function renderIntersections() {
  ...
  intersectionData.forEach((item, idx) => {
    ...
    // Stagger distance: alternate +16px / -16px / +26px / -26px
...
if start_pos != -1 and end_pos != -1:
    content = content[:start_pos] + js_rendering_logic + "\n\n" + content[end_pos:]
    print("Replaced JS rendering block successfully!")

with open(file_v1, 'w', encoding='utf-8') as f:
    f.write(content)

print("Saved updated V1 HTML successfully!")
