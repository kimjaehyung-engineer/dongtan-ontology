...
<head>
  ...
  <title>동탄트램 Time-Chainage & 예정공정표 대시보드 (513개 실데이터 연동)</title>
...
<body>
  ...
  <header class="app-header">
    <div class="brand-title">
      ...
      <span class="brand-tag">㈜천우씨엠 기술제안 513개 Activity 연동</span>
    </div>
    <div class="header-controls">
      ...
      <button class="btn-action" onclick="alert('엑셀 513개 데이터 파싱 완료 상태입니다.')">📁 데이터 검증</button>
  ...
  <div class="app-layout">
    ...
    <main class="main-view">
      <div class="chart-box">
        <div class="chart-topbar">
          <div class="chart-title-text">
            📍 Time-Chainage 실무 다이어그램 (STA 0km ~ 21km / 2027.07 ~ 2031.12)
          </div>
      ...
      <div class="summary-bar">
        ...
        <div class="stat-pill">1공구 구간: <strong>301역 ~ 201역 (7.8km)</strong></div>
        <div class="stat-pill">2공구 구간: <strong>201역 ~ S01/114역 (26.6km)</strong></div>
  ...
  <script>
    ...
    function renderSVG(filtered) {
      ...
      STATIONS.forEach((st) => {
        ...
        html += `<text x="${x}" y="${height - padB + 20}" fill="#475569" font-size="10" font-weight="500" text-anchor="end" transform="rotate(-45, ${x}, ${height - padB + 20})">${st.name}</text>`;
      });
      ...
      filtered.forEach((act, idx) => {
        ...
        // 위치 분배 매핑 (1공구 0~7.8km, 2공구 4.2~20.5km)
        ...
        html += `<line x1="${x1}" y1="${y1}" x2="${x2}" y2="${y2}" stroke="${color}" stroke-width="${strokeW}" stroke-linecap="round" class="act-line" data-acode="${act.acode}" data-ades="${act.ades}" data-es="${act.es}" data-ef="${act.ef}" data-ed="${act.ed}" data-zone="${act.zone}" style="cursor:pointer; opacity:${isSelected ? 1 : 085}; transition:all 0.15s;"/>`;
      });
      ...
      document.querySelectorAll('.act-line').forEach(el => {
        el.addEventListener('mouseenter', (e) => {
          ...
          tooltip.innerHTML = `<div class="tooltip-title">[${e.target.dataset.zone}] ${e.target.dataset.ades}</div><div>ACODE: <b>${e.target.dataset.acode}</b></div><div>기간: ${e.target.dataset.es} ~ ${e.target.dataset.ef} (<b>${e.target.dataset.ed}일</b>)</div>`;
          el.style.opacity = '1';
        });
    ...
    function openModal(act) {
      ...
      body.innerHTML = `
        ...
        <div style="background:#f1f5f9; padding:12px; border-radius:8px; border:1px solid #e2e8f0;">
          <div style="font-weight:700; font-size:0.8rem; color:#475569; margin-bottom:4px;">📊 ㈜천우씨엠 공기 산출 근거 공식 수치</div>
          <div style="font-size:0.78rem; color:#64748b; line-height:1.4;">
            본 액티비티는 동탄도시철도 기본설계 기술제안 기준 <b>일일 시공속도 및 인력/장비 투입 공수 산출식</b>에 의거하여 총 <b>${act.ed}일</b>의 공기가 소요됩니다.
          </div>
        </div>
      `;
...
print("Successfully replaced JSON string and generated final dashboard!")
