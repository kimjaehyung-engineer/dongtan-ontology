...
json_dur_all_path = r'c:\Users\sskjh\antigravity\01_전문업무_및_엔지니어링\11_현장_프로젝트\01_동탄트램\scratch\chunwoo_exact_all.json'
...
with open(json_dur_all_path, 'r', encoding='utf-8') as f:
    exact_durations = json.load(f)

def clean_key(s):
    return re.sub(r'[^0-9a-zA-Z가-힣]', '', str(s))

# Build lookup map with clean keys
lookup = {}
for k, v in exact_durations.items():
    ck = clean_key(k)
    lookup[ck] = v
...
for item in splits_data:
    ...
    ck = clean_key(sec_name)
    matched = None
    if ck in lookup:
        matched = lookup[ck]
    else:
        for k2, v2 in lookup.items():
            if ck in k2 or k2 in ck:
                matched = v2
                break
    
    if matched and matched.get('totalDays', 0) > 0:
        item['novanDays'] = matched.get('novanDays', 20)
        item['gwedoDays'] = matched.get('gwedoDays', 30)
        item['pojangDays'] = matched.get('pojangDays', 10)
        item['totalDays'] = matched.get('totalDays', item['novanDays'] + item['gwedoDays'] + item['pojangDays'])
    else:
        item['novanDays'] = 20
        item['gwedoDays'] = 30
        item['pojangDays'] = 10
...
<head>
  ...
  <title>동탄트램 Time-Chainage 대시보드 (중복 반영 그룹 총 소요공기 산정)</title>
  ...
  <style>
    ...
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .legend-bar {
      ...
      border: 1.5px solid #94a3b8; border-radius: 8px; padding: 8px 14px;
      font-size: 0.8rem; font-weight: 600; box-shadow: var(--shadow-sm); margin-top: 6px; flex-wrap: wrap;
    }
    .work-filter-btn {
      display: inline-flex; align-items: center; gap: 6px; padding: 5px 12px;
      border-radius: 6px; font-size: 0.8rem; font-weight: 700; cursor: pointer;
      user-select: none; transition: all 0.15s; border: 1.5px solid transparent; background: #ffffff;
    }
    
    .filter-novan { color: #ea580c; border-color: #fdba74; }
    .filter-novan.active { background: #ea580c; color: #ffffff; border-color: #c2410c; }

    .filter-gwedo { color: #16a34a; border-color: #86efac; }
    .filter-gwedo.active { background: #16a34a; color: #ffffff; border-color: #15803d; }

    .filter-pojang { color: #0284c7; border-color: #7dd3fc; }
    .filter-pojang.active { background: #0284c7; color: #ffffff; border-color: #0369a1; }

    .btn-work-all {
      background: #e2e8f0; color: #334155; border: 1px solid #cbd5e1;
      padding: 4px 10px; border-radius: 6px; font-size: 0.76rem; font-weight: 700; cursor: pointer;
    }
    .btn-work-all:hover { background: #cbd5e1; color: #0f172a; }
    ...
    .tooltip-card {
      ...
      line-height: 1.4; backdrop-filter: blur(4px); max-width: 360px;
    }
...
<body>
  ...
  <header class="app-header">
    <div class="brand-title">
      ...
      <span class="brand-tag" id="stat-filter-label">중복 반영 그룹 실소요공기 산정 산식 적용</span>
    </div>
  ...
  <div class="app-layout">
    ...
    <main class="main-view">
        ...
        <div class="chart-topbar">
          <div style="display:flex; flex-direction:column; gap:4px; width:100%;">
            <div class="chart-title-text" id="chart-title-text">
              📍 Time-Chainage 2D 다이어그램 (작업조 중복시공 반영 그룹 실공기 산정)
            </div>

            <!-- 공종별 개별 선택/토글 필터 바 -->
            <div class="legend-bar">
              <span style="color:#1e3a8a; font-weight:700;">🎯 공종별 개별 선택 맵 필터:</span>
              
              <div class="work-filter-btn filter-novan active" id="btn-filter-novan" onclick="toggleWorkType('노반')">
                <input type="checkbox" id="chk-novan" checked onclick="event.stopPropagation(); toggleWorkType('노반');" />
                <span>🟠 노반 공사</span>
              </div>

              <div class="work-filter-btn filter-gwedo active" id="btn-filter-gwedo" onclick="toggleWorkType('궤도')">
                <input type="checkbox" id="chk-gwedo" checked onclick="event.stopPropagation(); toggleWorkType('궤도');" />
                <span>🟢 궤도 공사</span>
              </div>

              <div class="work-filter-btn filter-pojang active" id="btn-filter-pojang" onclick="toggleWorkType('포장')">
                <input type="checkbox" id="chk-pojang" checked onclick="event.stopPropagation(); toggleWorkType('포장');" />
                <span>🟦 포장 공사</span>
              </div>

              <div style="margin-left: 8px; display:flex; gap:4px;">
                <button class="btn-work-all" onclick="selectAllWorkTypes(true)">전체 선택</button>
                <button class="btn-work-all" onclick="selectAllWorkTypes(false)">전체 해제</button>
              </div>

              <span style="font-size:0.72rem; color:#64748b; margin-left:auto;">※ 시스템 공정은 미산정으로 제외</span>
            </div>
          </div>
      ...
      <div class="summary-bar">
        ...
        <div class="stat-pill">X축 거리범위: <strong id="stat-range-label">STA 0.0km ~ 20.5km</strong></div>
        <div class="stat-pill">Y축 연도범위: <strong id="stat-yrange-label" style="color:#d97706;">2027.5년 ~ 2028.6년</strong></div>
        <div class="stat-pill">선택 표출 공종: <strong id="stat-work-types-label">노반(🟠) + 궤도(🟢) + 포장(🟦)</strong></div>
      </div>
    </main>
  ...
  <script>
    ...
    // 그룹 내 작업조 중복 및 선후행 중복(Overlap 45~50%)을 반영한 주경로(Critical Path) 실소요공기 계산 함수
    function calcGroupOverlapDays(items) {
      ...
      // 2개 이상 구간의 경우 작업조 2~3조 병렬 및 선후행 중복(Overlap 45%) 산식
    ...
    function renderSectionTree() {
      ...
      if (fltLblEl) fltLblEl.innerText = selectedSplitKeys.size > 0 ? `${selectedSplitKeys.size}개 선택됨` : '중복 반영 그룹 실소요공기 산정 산식 적용';
        ...
        g.items.forEach(sec => {
          ...
          itemEl.innerHTML = `
            ...
            <div class="sub-sec-info">
              <span style="color:${visibleWorkTypes.has('노반') ? '#ea580c' : '#94a3b8'}; font-weight:${visibleWorkTypes.has('노반') ? '700' : '400'};">🟠노반:${nD}일</span> | 
              <span style="color:${visibleWorkTypes.has('궤도') ? '#16a34a' : '#94a3b8'}; font-weight:${visibleWorkTypes.has('궤도') ? '700' : '400'};">🟢궤도:${gD}일</span> | 
              <span style="color:${visibleWorkTypes.has('포장') ? '#0284c7' : '#94a3b8'}; font-weight:${visibleWorkTypes.has('포장') ? '700' : '400'};">🟦포장:${pD}일</span>
            </div>
          `;
          itemEl.onclick = () => {
            selectedSubSec = sec;
        ...
        targetSplitsToDraw.forEach((g, gIdx) => {
          ...
          g.items.forEach((item, itemIdx) => {
            ...
            if (visibleWorkTypes.has('노반')) {
              ...
              html += `<line x1="${x1}" y1="${ny1}" x2="${xMidNovan}" y2="${ny2}" stroke="#ea580c" stroke-width="${sw}" stroke-linecap="round" class="split-line" data-grp="${item.splitGroup}" data-sec="${item.sectionName}" data-cat="노반 공사" data-nd="${nD}" data-gd="${gD}" data-pd="${pD}" data-tot="${totD}" data-grpot="${grpOverlapDays}" data-dist="${item.distM || 0}" data-sy="${secStartYr.toFixed(1)}" data-ey="${novanEndYr.toFixed(1)}" style="cursor:pointer; filter: drop-shadow(0px 2px 4px rgba(0,0,0,0.25));"/>`;
            ...
            if (visibleWorkTypes.has('궤도')) {
              ...
              html += `<line x1="${xMidNovan}" y1="${gy1}" x2="${xMidGwedo}" y2="${gy2}" stroke="#16a34a" stroke-width="${sw}" stroke-linecap="round" class="split-line" data-grp="${item.splitGroup}" data-sec="${item.sectionName}" data-cat="궤도 공사" data-nd="${nD}" data-gd="${gD}" data-pd="${pD}" data-tot="${totD}" data-grpot="${grpOverlapDays}" data-dist="${item.distM || 0}" data-sy="${novanEndYr.toFixed(1)}" data-ey="${gwedoEndYr.toFixed(1)}" style="cursor:pointer; filter: drop-shadow(0px 2px 4px rgba(0,0,0,0.25));"/>`;
              hasDrawnAnyLine = true;
            }
            ...
            if (visibleWorkTypes.has('포장')) {
              ...
              html += `<line x1="${xMidGwedo}" y1="${py1}" x2="${x2}" y2="${py2}" stroke="#0284c7" stroke-width="${sw}" stroke-linecap="round" class="split-line" data-grp="${item.splitGroup}" data-sec="${item.sectionName}" data-cat="포장 공사" data-nd="${nD}" data-gd="${gD}" data-pd="${pD}" data-tot="${totD}" data-grpot="${grpOverlapDays}" data-dist="${item.distM || 0}" data-sy="${gwedoEndYr.toFixed(1)}" data-ey="${pojangEndYr.toFixed(1)}" style="cursor:pointer; filter: drop-shadow(0px 2px 4px rgba(0,0,0,0.25));"/>`;
              hasDrawnAnyLine = true;
            }
      ...
      body.innerHTML = `
        ...
        <div style="background:#eff6ff; padding:14px; border-radius:8px; border:1px solid #bfdbfe; display:flex; flex-direction:column; gap:10px;">
          <div style="font-weight:700; font-size:0.88rem; color:#1e3a8a; display:flex; justify-content:space-between; align-items:center;">
            <span>📊 ㈜천우씨엠 중복시공 반영 소요공기 분석</span>
          ...
          <div style="display:grid; grid-template-columns: 1fr 1fr 1fr; gap:8px; font-size:0.82rem; text-align:center;">
            <div style="background:${visibleWorkTypes.has('노반') ? '#fff7ed' : '#f1f5f9'}; padding:8px; border-radius:6px; border:1.5px solid ${visibleWorkTypes.has('노반') ? '#ea580c' : '#cbd5e1'}; opacity:${visibleWorkTypes.has('노반') ? '1' : '0.5'};">
              <span style="color:#ea580c; font-weight:700;">1단계: 🟠 노반 공사</span><br/>
            ...
            <div style="background:${visibleWorkTypes.has('궤도') ? '#f0fdf4' : '#f1f5f9'}; padding:8px; border-radius:6px; border:1.5px solid ${visibleWorkTypes.has('궤도') ? '#16a34a' : '#cbd5e1'}; opacity:${visibleWorkTypes.has('궤도') ? '1' : '0.5'};">
              <span style="color:#16a34a; font-weight:700;">2단계: 🟢 궤도 공사</span><br/>
            ...
            <div style="background:${visibleWorkTypes.has('포장') ? '#f0f9ff' : '#f1f5f9'}; padding:8px; border-radius:6px; border:1.5px solid ${visibleWorkTypes.has('포장') ? '#0284c7' : '#cbd5e1'}; opacity:${visibleWorkTypes.has('포장') ? '1' : '0.5'};">
              <span style="color:#0284c7; font-weight:700;">3단계: 🟦 포장 공사</span><br/>
          ...
          <div style="background:#ffffff; padding:10px; border-radius:6px; border:1px solid #cbd5e1; font-size:0.78rem; line-height:1.5; color:#334155;">
            • <b>단순 직렬 합산 공기</b>: <strong>${realGrpSum}일</strong> (구간 내 항목을 순차적으로 1개씩 다 더함)<br/>
            • ⚡ <b>중복시공 반영 주경로(CP) 실공기</b>: <strong style="color:#1e40af; font-size:0.85rem;">${realGrpOverlap}일</strong> (작업조 2~3조 동시 시공 및 선후행 공종 중복 시공 반영)
          </div>
        </div>
      `;
...
print("✓ Successfully implemented Group Overlap Duration Calculation Engine!")
