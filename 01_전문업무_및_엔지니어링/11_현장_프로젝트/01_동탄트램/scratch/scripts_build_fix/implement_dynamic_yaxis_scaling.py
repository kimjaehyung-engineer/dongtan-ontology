...
<head>
  ...
  <title>동탄트램 Time-Chainage 대시보드 (공정선 범위 맞춤 동적 Y축 스케일링 뷰)</title>
...
<body>
  ...
  <header class="app-header">
    <div class="brand-title">
      ...
      <span class="brand-tag" id="stat-filter-label">공정선 범위 맞춤 동적 Y축 스케일링 뷰</span>
    </div>
        ...
        <div class="chart-topbar">
          <div style="display:flex; flex-direction:column; gap:4px; width:100%;">
            <div class="chart-title-text" id="chart-title-text">
              📍 Time-Chainage 2D 다이어그램 (동적 Y축 연도 범위 자동 맞춤)
            </div>
  ...
  <script>
    ...
    function renderSectionTree() {
      ...
      if (fltLblEl) fltLblEl.innerText = selectedSplitKeys.size > 0 ? `${selectedSplitKeys.size}개 선택됨` : '공정선 범위 맞춤 동적 Y축 스케일링 뷰';
        ...
        g.items.forEach(sec => {
          ...
          itemEl.innerHTML = `
            <div class="sub-sec-head">
              <span>${secName}</span>
              <span style="font-size:0.7rem; color:#1e40af; font-weight:700;">선택합 ${subTot}일</span>
            </div>
    ...
    // 3열: 공정선 실제 연도 범위에 맞춘 동적 Y축 스케일링 (Dynamic Y-Range Auto-Fit) SVG 렌더링 엔진
    function renderSVG() {
      ...
      // 2. 동적 Y축(연도) 범위 자동 산정 (Dynamic Y-Axis Auto-Fit)
      ...
      // 공정선이 Y축 상하 85% 영역을 시원하게 차지하도록 타이트한 마진(0.12년) 부여
      ...
      // 1. Y축 연도 타이트 격자선 (스케일 범위에 맞춰 0.1년 ~ 0.25년 간격 자동 생성)
        ...
        targetSplitsToDraw.forEach((g, gIdx) => {
          g.items.forEach((item, itemIdx) => {
            ...
            if (visibleWorkTypes.has('노반')) {
              ...
              html += `<line x1="${x1}" y1="${ny1}" x2="${xMidNovan}" y2="${ny2}" stroke="#ea580c" stroke-width="${sw}" stroke-linecap="round" class="split-line" data-grp="${item.splitGroup}" data-sec="${item.sectionName}" data-cat="노반 공사" data-nd="${nD}" data-gd="${gD}" data-pd="${pD}" data-tot="${totD}" data-dist="${item.distM || 0}" data-sy="${secStartYr.toFixed(1)}" data-ey="${novanEndYr.toFixed(1)}" style="cursor:pointer; filter: drop-shadow(0px 2px 4px rgba(0,0,0,0.25));"/>`;
            ...
            if (visibleWorkTypes.has('궤도')) {
              ...
              html += `<line x1="${xMidNovan}" y1="${gy1}" x2="${xMidGwedo}" y2="${gy2}" stroke="#16a34a" stroke-width="${sw}" stroke-linecap="round" class="split-line" data-grp="${item.splitGroup}" data-sec="${item.sectionName}" data-cat="궤도 공사" data-nd="${nD}" data-gd="${gD}" data-pd="${pD}" data-tot="${totD}" data-dist="${item.distM || 0}" data-sy="${novanEndYr.toFixed(1)}" data-ey="${gwedoEndYr.toFixed(1)}" style="cursor:pointer; filter: drop-shadow(0px 2px 4px rgba(0,0,0,0.25));"/>`;
              hasDrawnAnyLine = true;
            }
            ...
            if (visibleWorkTypes.has('포장')) {
              ...
              html += `<line x1="${xMidGwedo}" y1="${py1}" x2="${x2}" y2="${py2}" stroke="#0284c7" stroke-width="${sw}" stroke-linecap="round" class="split-line" data-grp="${item.splitGroup}" data-sec="${item.sectionName}" data-cat="포장 공사" data-nd="${nD}" data-gd="${gD}" data-pd="${pD}" data-tot="${totD}" data-dist="${item.distM || 0}" data-sy="${gwedoEndYr.toFixed(1)}" data-ey="${pojangEndYr.toFixed(1)}" style="cursor:pointer; filter: drop-shadow(0px 2px 4px rgba(0,0,0,0.25));"/>`;
              hasDrawnAnyLine = true;
            }
            ...
            if (hasDrawnAnyLine) {
              ...
              html += `<rect x="-65" y="-12" width="130" height="22" fill="#ffffff" stroke="#1e3a8a" stroke-width="1.8" rx="5" opacity="0.96"/>`;
              html += `<text x="0" y="3" fill="#0f172a" font-size="10" font-weight="700" text-anchor="middle">${item.sectionName}</text>`;
              html += `</g>`;
              labelIdx++;
            }
          });
        ...
        el.addEventListener('mouseenter', (e) => {
          ...
          tooltip.innerHTML = `
            ...
            <div style="margin-top:4px; padding-top:4px; border-top:1px solid #38bdf840; color:#38bdf8;">
              ...
              - 총합 소요공기: <b>${e.target.dataset.tot}일</b>
            </div>
          `;
        });
    ...
    function openModalForSection(sec) {
      ...
      body.innerHTML = `
        ...
        <div class="detail-grid">
          ...
          <div class="detail-item"><div class="detail-label">총 산출 소요일수</div><div class="detail-value" style="color:#1e40af; font-size:1.05rem;">총 ${totD} 일</div></div>
        </div>
        ...
        <div style="background:#eff6ff; padding:14px; border-radius:8px; border:1px solid #bfdbfe; display:flex; flex-direction:column; gap:8px;">
          <div style="font-weight:700; font-size:0.85rem; color:#1e3a8a;">📊 ㈜천우씨엠 3대 핵심 공종별 개별 소요일수 카드</div>
          ...
          <div style="font-size:0.73rem; color:#64748b; margin-top:2px;">
            * 세로축(Y축) 연도 범위가 화면에 표출된 공정선 실제 기간에 맞춰 동적으로 자동 맞춤됩니다.
          </div>
        </div>
      `;
...
print("✓ Successfully implemented Dynamic Y-Axis Scale Auto-Fit Engine!")
