...
<head>
  ...
  <title>동탄트램 Time-Chainage 대시보드 (공종별 개별 선택 필터링 맵 뷰)</title>
  ...
  <style>
    ...
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .filter-novan.active { background: #ea580c; color: #ffffff; border-color: #c2410c; shadow: var(--shadow-sm); }

    .filter-gwedo { color: #16a34a; border-color: #86efac; }
    .filter-gwedo.active { background: #16a34a; color: #ffffff; border-color: #15803d; shadow: var(--shadow-sm); }

    .filter-pojang { color: #0284c7; border-color: #7dd3fc; }
    .filter-pojang.active { background: #0284c7; color: #ffffff; border-color: #0369a1; shadow: var(--shadow-sm); }
...
<body>
  ...
  <header class="app-header">
    <div class="brand-title">
      ...
      <span class="brand-tag" id="stat-filter-label">공종별 개별 토글 선택 맵 뷰</span>
    </div>
  ...
  <script>
    ...
    // 공종별 개별 선택 필터 집합 (기본: 노반, 궤도, 포장 모두 활성화)
    ...
    function renderSectionTree() {
      ...
      if (fltLblEl) fltLblEl.innerText = selectedSplitKeys.size > 0 ? `${selectedSplitKeys.size}개 선택됨` : '공종별 개별 토글 선택 맵 뷰';
    ...
    // 3열: 공종별 개별 선택 토글 지원 2D SVG 다이어그램 렌더링 엔진
    function renderSVG() {
      ...
      // 3. 공종별 개별 토글 선택 (노반, 궤도, 포장) SVG 공정선 표출
      ...
      body.innerHTML = `
        ...
        <div style="background:#eff6ff; padding:14px; border-radius:8px; border:1px solid #bfdbfe; display:flex; flex-direction:column; gap:8px;">
          ...
          <div style="font-size:0.73rem; color:#64748b; margin-top:2px;">
            * 차트 상단의 [🟠노반 공사], [🟢궤도 공사], [🟦포장 공사] 토글 버튼을 클릭하여 개별 공종선만 선별적으로 감상하실 수 있습니다.
          </div>
        </div>
      `;
...
print("✓ Successfully added Work Type Individual Filter Toggle Feature!")
