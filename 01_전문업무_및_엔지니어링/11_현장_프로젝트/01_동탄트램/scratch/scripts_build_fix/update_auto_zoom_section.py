...
<head>
  ...
  <title>동탄트램 Time-Chainage 대시보드 (선택 구간 자동 줌인 확대 뷰어)</title>
...
<body>
  ...
  <header class="app-header">
    <div class="brand-title">
      ...
      <span class="brand-tag">선택 시공구간 100% 화면 가득 줌인(Zoom-In) 특화</span>
    </div>
    <div class="header-controls">
      ...
      <button class="btn-action" onclick="resetZoom()">🌐 전체 뷰 초기화</button>
      ...
      <div class="chart-box">
        <div class="chart-topbar">
          <div class="chart-title-text" id="chart-title-text">
            📍 Time-Chainage 2D 다이어그램 (선택 구간 화면 100% 줌인)
          </div>
  ...
  <script>
    ...
    // 3열: 선택된 시공구간 100% 화면가득 줌인(Zoom-In) SVG 차트 엔진
    function renderSVG() {
      const rect = svg.getBoundingClientRect();
      const width = (rect.width || 900) * zoomLevel;
      const height = rect.height || 550;
      
      const padL = 75, padR = 50, padT = 55, padB = 70;
      const graphW = width - padL - padR;
      const graphH = height - padT - padB;

      const groups = getGroupedSplits();

      // 선택 구간에 따른 동적 X축 (minKm ~ maxKm) 줌인 좌표 계산
      let minKm = 0.0, maxKm = 21.0;
      
      if (selectedSubSec) {
        minKm = Math.max(0.0, (selectedSubSec.startM / 1000.0) - 0.2);
        maxKm = Math.min(21.0, (selectedSubSec.endM / 1000.0) + 0.2);
        ...
        const padding = Math.max(0.3, (realMax - realMin) * 0.15);
        minKm = Math.max(0.0, realMin - padding);
        maxKm = Math.min(21.0, realMax + padding);
      }
      ...
      for (let yr = 2027.5; yr <= 2032; yr += 0.5) {
        ...
        html += `<text x="${padL - 12}" y="${y + 4}" fill="#64748b" font-size="11" font-weight="600" text-anchor="end">${yr.toFixed(1)}년</text>`;
      }

      // X축 정거장 가이드선 (현재 줌인 좌표 범위 내 정거장만 표출)
      STATIONS.forEach((st) => {
      ...
      if (currentViewMode === 'SPLITS') {
        ...
        groups.forEach((g, gIdx) => {
          g.items.forEach((item, itemIdx) => {
            ...
            const x2 = getX(endKm > startKm ? endKm : startKm + 0.4);
            ...
            // 줌인된 시공구간 배경 반투명 밴드
            html += `<rect x="${Math.min(x1, x2)}" y="${padT}" width="${Math.abs(x2 - x1)}" height="${graphH}" fill="${color}" opacity="${isSelected ? 0.35 : 0.15}"/>`;
            
            // 굵고 선명한 공정선
            html += `<line x1="${x1}" y1="${y1}" x2="${x2}" y2="${y2}" stroke="${color}" stroke-width="${isSelected ? 8 : 5}" stroke-linecap="round" class="split-line" data-grp="${item.splitGroup}" data-sec="${item.sectionName}" data-dist="${item.distM}" data-sy="${startYr.toFixed(1)}" data-ey="${endYr.toFixed(1)}" style="cursor:pointer; opacity:1; filter: drop-shadow(0px 2px 4px rgba(0,0,0,0.15));"/>`;
            
            // 텍스트 라벨 지그재그 교차 배치로 겹침 방지 (Staggered Labels)
            const midX = (x1 + x2) / 2;
            const midY = (y1 + y2) / 2;
            const yOffset = (labelIndex % 2 === 0) ? -14 : 18;

            html += `<g transform="translate(${midX}, ${midY + yOffset})">`;
            html += `<rect x="-55" y="-12" width="110" height="20" fill="#ffffff" stroke="${color}" stroke-width="1.2" rx="4" opacity="0.95"/>`;
            html += `<text x="0" y="2" fill="#0f172a" font-size="11" font-weight="700" text-anchor="middle">${item.sectionName}</text>`;
            html += `</g>`;
...
print("Successfully updated Auto-Zoom Focus Section Time-Chainage Dashboard HTML!")
