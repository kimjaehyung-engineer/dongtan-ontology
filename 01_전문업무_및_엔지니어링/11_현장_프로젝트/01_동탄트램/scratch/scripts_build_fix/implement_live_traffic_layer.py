...
# Update renderNaverMapView implementation to feature live traffic layer & UTIC CCTV links
...
new_func_code = """function renderNaverMapView(item) {
  ...
  // 1. Naver Live Traffic Layer Enabled URL (traffic=true)
  const naverTrafficUrl = `https://map.naver.com/v5/search/${encodeURIComponent(searchQuery)}?c=${lng},${lat},16,0,0,0,dh&traffic=true`;
  
  // 2. UTIC National Traffic Information Center Live Traffic & CCTV URL
  const uticUrl = `https://www.utic.go.kr/map/map.do?searchKeyword=${encodeURIComponent(searchQuery)}`;
  
  // 3. Kakao Map Search URL
  const kakaoUrl = `https://map.kakao.com/link/search/${encodeURIComponent(searchQuery)}`;
  ...
  if (actionBtns) {
    actionBtns.innerHTML = `
      <a href="${naverTrafficUrl}" target="_blank" style="background: #03c75a; color: #ffffff; text-decoration: none; padding: 0.55rem 1.1rem; border-radius: 8px; font-size: 0.85rem; font-weight: 800; display: flex; align-items: center; gap: 0.4rem; box-shadow: 0 3px 8px rgba(3,199,90,0.3); transition: transform 0.15s ease;">
        <span>🚦</span> 네이버 실시간 교통상황 보기 (혼잡도 ON)
      </a>
      <a href="${uticUrl}" target="_blank" style="background: #0284c7; color: #ffffff; text-decoration: none; padding: 0.55rem 1.1rem; border-radius: 8px; font-size: 0.85rem; font-weight: 800; display: flex; align-items: center; gap: 0.4rem; box-shadow: 0 3px 8px rgba(2,132,199,0.3); transition: transform 0.15s ease;">
        <span>📡</span> 국가교통정보센터(UTIC) 4K CCTV/소통정보
      </a>
      <a href="${kakaoUrl}" target="_blank" style="background: #fee500; color: #191919; text-decoration: none; padding: 0.55rem 1.1rem; border-radius: 8px; font-size: 0.85rem; font-weight: 800; display: flex; align-items: center; gap: 0.4rem; box-shadow: 0 3px 8px rgba(254,229,0,0.3); transition: transform 0.15s ease;">
        <span>💬</span> 카카오 맵
      </a>
    `;
  }
...
if re.search(old_func_pattern, content):
    ...
    print("Updated renderNaverMapView with Live Traffic Layer & UTIC CCTV integration!")

with open(f_path, 'w', encoding='utf-8') as f:
    f.write(content)

print("Finished applying Live Traffic Layer to V1 HTML!")
