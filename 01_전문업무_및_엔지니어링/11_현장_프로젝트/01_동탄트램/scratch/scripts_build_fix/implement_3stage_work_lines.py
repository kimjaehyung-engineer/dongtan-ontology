...
# Pre-index chunwoo durations with clean key
chunwoo_map = {}
for k, v in chunwoo_durations.items():
    ck = clean_key(k)
    chunwoo_map[ck] = v

# Bind durations with fuzzy matching
for item in splits_data:
    ...
    matched_dur = None
    if ck in chunwoo_map:
        matched_dur = chunwoo_map[ck]
    else:
        # Try partial match
        for k2, v2 in chunwoo_map.items():
            if ck in k2 or k2 in ck:
                matched_dur = v2
                break
    
    if matched_dur and (matched_dur.get('novanDays', 0) > 0 or matched_dur.get('gwedoDays', 0) > 0):
        item['novanDays'] = matched_dur.get('novanDays', 25)
        item['gwedoDays'] = matched_dur.get('gwedoDays', 35)
        item['pojangDays'] = matched_dur.get('pojangDays', 12)
    else:
        # Default realistic durations
        is_stn = '정거장' in sec_name or '교' in sec_name
        item['novanDays'] = 30 if is_stn else 20
        item['gwedoDays'] = 40 if is_stn else 30
        item['pojangDays'] = 15 if is_stn else 10

    item['totalDays'] = item['novanDays'] + item['gwedoDays'] + item['pojangDays']
...
<head>
  ...
  <title>동탄트램 Time-Chainage 대시보드 (3대 공종 노반/궤도/포장 연속 라인 맵핑)</title>
...
<body>
  ...
  <header class="app-header">
    <div class="brand-title">
      ...
      <span class="brand-tag" id="stat-filter-label">3대 공종(노반🟠 ➔ 궤도🟢 ➔ 포장🟦) 연속 공정선 표출</span>
    </div>
        ...
        <div class="chart-topbar">
          <div style="display:flex; flex-direction:column; gap:4px; width:100%;">
            ...
            <div class="legend-bar">
              <span style="color:#1e3a8a; font-weight:700;">🏷️ ㈜천우씨엠 3대 공종 범례:</span>
              <div class="legend-item"><span class="legend-badge badge-sub"></span> <span><strong>🟠 노반 공사</strong> (토공/구조물)</span></div>
  ...
  <script>
    ...
    function renderSectionTree() {
      ...
      if (fltLblEl) fltLblEl.innerText = selectedSplitKeys.size > 0 ? `${selectedSplitKeys.size}개 선택됨` : '3대 공종(노반🟠 ➔ 궤도🟢 ➔ 포장🟦) 연속 공정선 표출';
        ...
        g.items.forEach(sec => {
          ...
          itemEl.innerHTML = `
            ...
            <div class="sub-sec-info">
              <span style="color:#ea580c; font-weight:700;">🟠노반:${nD}일</span> | 
              <span style="color:#16a34a; font-weight:700;">🟢궤도:${gD}일</span> | 
              <span style="color:#0284c7; font-weight:700;">🟦포장:${pD}일</span>
            </div>
          `;
          itemEl.onclick = () => {
            selectedSubSec = sec;
    ...
    // 3열: 3대 핵심 공종(노반🟠 ➔ 궤도🟢 ➔ 포장🟦) 연속 공정선 SVG 다이어그램 렌더링 엔진
    function renderSVG() {
      ...
      // 3. ㈜천우씨엠 3대 공종 (노반🟠 ➔ 궤도🟢 ➔ 포장🟦) 3연속 대각 공정선 표출
      if (currentViewMode === 'SPLITS') {
        ...
        targetSplitsToDraw.forEach((g, gIdx) => {
          g.items.forEach((item, itemIdx) => {
            ...
            // 3개 공종별 시간 분할 비율 (Novan -> Gwedo -> Pojang)
            ...
            html += `<text x="0" y="3" fill="#0f172a" font-size="10" font-weight="700" text-anchor="middle">${item.sectionName} (총 ${totD}일)</text>`;
            html += `</g>`;
      ...
      document.querySelectorAll('.split-line').forEach(el => {
        el.addEventListener('mouseenter', (e) => {
          ...
          tooltip.innerHTML = `
            ...
            <div>선택 공종: <b style="color:#38bdf8;">${e.target.dataset.cat}</b> | 연장: <b>${e.target.dataset.dist}m</b></div>
            <div style="margin-top:4px; padding-top:4px; border-top:1px solid #38bdf840; color:#38bdf8;">
              <b>📊 ㈜천우씨엠 공기산출 내역 (총 ${e.target.dataset.tot}일):</b><br/>
    ...
    function openModalForSection(sec) {
      ...
      body.innerHTML = `
        ...
        <div class="detail-grid">
          ...
          <div class="detail-item"><div class="detail-label">공종 구성</div><div class="detail-value">노반🟠 + 궤도🟢 + 포장🟦</div></div>
        ...
        <div style="background:#eff6ff; padding:14px; border-radius:8px; border:1px solid #bfdbfe; display:flex; flex-direction:column; gap:8px;">
          ...
          <div style="font-size:0.73rem; color:#64748b; margin-top:2px;">
            * 차트 다이어그램 상에서 🟠노반 ➔ 🟢궤도 ➔ 🟦포장 순서로 대각선 3개가 연결되어 직관적으로 표현됩니다.
          </div>
        </div>
      `;
...
print("✓ Successfully implemented 3-Stage Work Sequence (Novan -> Gwedo -> Pojang) Line Engine!")
