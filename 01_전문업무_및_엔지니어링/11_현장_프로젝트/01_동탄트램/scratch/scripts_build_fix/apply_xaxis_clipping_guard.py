...
def classify_work(text):
    t = text.lower()
    if '상부강화' in t or '강화노반' in t or '상부노반' in t:
        return '상부강화노반', '#dc2626' # Red
    elif '궤도' in t or '레일' in t or '도상' in t or '용접' in t:
        return '궤도', '#16a34a' # Green
    elif '시스템' in t or '전력' in t or '신호' in t or '통신' in t or '변전' in t or '급전' in t:
        return '시스템', '#9333ea' # Purple
    elif '노반' in t or '다짐' in t or '구조물' in t:
        return '노반', '#ea580c' # Orange
    elif '토공' in t or '지반' in t or '굴착' in t or '쌓기' in t or '깎기' in t:
        return '토공', '#b45309' # Amber / Brown
    else:
        if '일반' in t:
            return '노반', '#ea580c'
        else:
...
for item in splits_data:
    ...
    w_cat, w_col = classify_work(item['sectionName'] + ' ' + item['splitGroup'])
...
for act in activities_data:
    ...
    w_cat, w_col = classify_work(act_ades)
...
<head>
  ...
  <title>동탄트램 Time-Chainage 대시보드 (X축 범위 이탈 방지 무결성 뷰어)</title>
  ...
  <style>
    ...
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .badge-earth { background: #b45309; border: 1px solid #78350f; }
    .badge-sub { background: #ea580c; border: 1px solid #c2410c; }
    .badge-top { background: #dc2626; border: 1px solid #b91c1c; }
    .badge-track { background: #16a34a; border: 1px solid #15803d; }
    .badge-sys { background: #9333ea; border: 1px solid #7e22ce; }

    .svg-wrapper {
      flex: 1; width: 100%; height: 100%; position: relative; min-height: 420px; height: 420px; background: #ffffff;
      border: 1.8px solid #94a3b8; border-radius: 6px; overflow: hidden;
    }
    svg { width: 100%; height: 100%; display: block; min-height: 420px; }
...
<body>
  ...
  <header class="app-header">
    <div class="brand-title">
      ...
      <span class="brand-tag">X축 클리핑 영역 이탈 방지 & 5대 세부공종</span>
    </div>
  ...
  <div class="app-layout">
    ...
    <main class="main-view">
        ...
        <div class="chart-topbar">
          <div style="display:flex; flex-direction:column; gap:4px; width:100%;">
            ...
            <div class="legend-bar">
              <span style="color:#1e3a8a; font-weight:700;">🏷️ 5대 공종 범례:</span>
              <div class="legend-item"><span class="legend-badge badge-earth"></span> <span><strong>🟤 토공</strong> (지반/굴착/흙쌓기)</span></div>
              <div class="legend-item"><span class="legend-badge badge-sub"></span> <span><strong>🟠 노반</strong> (구조물/다짐)</span></div>
              <div class="legend-item"><span class="legend-badge badge-top"></span> <span><strong>🔴 상부강화노반</strong> (상부노반)</span></div>
              <div class="legend-item"><span class="legend-badge badge-track"></span> <span><strong>🟢 궤도</strong> (레일/도상/용접)</span></div>
              <div class="legend-item"><span class="legend-badge badge-sys"></span> <span><strong>🟣 시스템</strong> (전력/신호/통신)</span></div>
            </div>
          </div>
      ...
      <div class="summary-bar">
        ...
        <div class="stat-pill">범례: 
          <strong style="color:#b45309;">🟤 토공</strong> | 
          <strong style="color:#ea580c;">🟠 노반</strong> | 
          <strong style="color:#dc2626;">🔴 상부강화노반</strong> | 
          <strong style="color:#16a34a;">🟢 궤도</strong> | 
          <strong style="color:#9333ea;">🟣 시스템</strong>
        </div>
  ...
  <script>
        ...
        g.items.forEach(sec => {
          ...
          itemEl.innerHTML = `
            <div class="sub-sec-head">
              ...
              <span style="font-size:0.7rem; color:${sec.workColor || '#dc2626'}; font-weight:700;">${sec.workCategory || '노반'}</span>
            </div>
            <div class="sub-sec-info">STA ${sec.startM}m ~ ${sec.endM}m (연장 ${sec.distM}m)</div>
          `;
          itemEl.onclick = () => {
            selectedSubSec = sec;
    ...
    // 3열: X축 클리핑 영역 이탈 철통 방어 2D SVG Engine
    function renderSVG() {
      ...
      // 3. 5대 세부 공종별 컬러 코딩 라인 표출 (X축 범위 안착 가드 100% 적용!)
      if (currentViewMode === 'SPLITS') {
        ...
        targetSplitsToDraw.forEach((g, gIdx) => {
          g.items.forEach((item, itemIdx) => {
            ...
            // ★ [핵심 수술] X축 화면 범위(minKm ~ maxKm) 안쪽에 들어오는 항목만 안전 표출!
            if (endKm < minKm - 0.8 || startKm > maxKm + 0.8) return;
            ...
            const color = item.workColor || '#dc2626';

            // 시공구간 반투명 배경 밴드
            html += `<rect x="${Math.min(x1, x2)}" y="${padT}" width="${Math.max(5, Math.abs(x2 - x1))}" height="${graphH}" fill="${color}" opacity="${isSelected ? 0.35 : 0.14}"/>`;
            
            // 공종 전용 색상 공정선
            html += `<line x1="${x1}" y1="${y1}" x2="${x2}" y2="${y2}" stroke="${color}" stroke-width="${isSelected ? 9 : 6}" stroke-linecap="round" class="split-line" data-grp="${item.splitGroup}" data-sec="${item.sectionName}" data-cat="${item.workCategory || '노반'}" data-dist="${item.distM}" data-sy="${startYr.toFixed(1)}" data-ey="${endYr.toFixed(1)}" style="cursor:pointer; opacity:1; filter: drop-shadow(0px 2px 4px rgba(0,0,0,0.2));"/>`;
            ...
            html += `<rect x="-55" y="-12" width="110" height="20" fill="#ffffff" stroke="${color}" stroke-width="1.6" rx="4" opacity="0.95"/>`;
      ...
      document.querySelectorAll('.split-line').forEach(el => {
        el.addEventListener('mouseenter', (e) => {
          ...
          tooltip.innerHTML = `<div class="tooltip-title">[${e.target.dataset.grp}] ${e.target.dataset.sec}</div><div>세부공종: <b>${e.target.dataset.cat || '노반'}</b></div><div>연장: <b>${e.target.dataset.dist}m</b></div><div>예정공기: ${e.target.dataset.sy}년 ~ ${e.target.dataset.ey}년</div>`;
        });
    ...
    function openModalForSection(sec) {
      ...
      body.innerHTML = `
        ...
        <div class="detail-grid">
          ...
          <div class="detail-item"><div class="detail-label">세부 공종 구분</div><div class="detail-value" style="color:${sec.workColor || '#dc2626'};">${sec.workCategory || '노반'}</div></div>
        ...
        <div style="background:#f1f5f9; padding:12px; border-radius:8px; border:1px solid #e2e8f0;">
          <div style="font-weight:700; font-size:0.8rem; color:#475569; margin-bottom:4px;">📊 ㈜천우씨엠 공종별 산출 근거</div>
          <div style="font-size:0.78rem; color:#64748b; line-height:1.4;">
            본 구간은 <b>${sec.workCategory || '노반'} 공종</b>의 세부 시공구간으로, 총 <b>${sec.distM}m</b> 연장에 걸쳐 공정계획에 따라 안전하게 시공됩니다.
          </div>
        </div>
      `;
...
print("✓ Successfully applied X-axis clipping guards & undefined property fallbacks!")
