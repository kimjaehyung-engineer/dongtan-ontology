...
for item in splits_data:
    ...
    item['sectionName'] = str(item.get('sectionName', '')).replace('\n', ' ').replace('  ', ' ').strip()
...
<head>
  ...
  <title>동탄트램 Time-Chainage 대시보드 (Null Pointer 방어 무결성 완제)</title>
...
<body>
  ...
  <header class="app-header">
    <div class="brand-title">
      ...
      <span class="brand-tag" id="stat-filter-label">28개 전체 시공구간 100% 표출</span>
    </div>
  ...
  <script>
    ...
    function renderSectionTree() {
      ...
      if (fltLblEl) fltLblEl.innerText = selectedSplitKeys.size > 0 ? `${selectedSplitKeys.size}개 선택됨` : '28개 전체 시공구간 100% 표출';
      ...
      groups.forEach(g => {
        ...
        g.items.forEach(sec => {
          ...
          itemEl.innerHTML = `
            ...
            <div class="sub-sec-info">STA ${sec.startM || 0}m ~ ${sec.endM || 0}m (연장 ${sec.distM || 0}m)</div>
          `;
          itemEl.onclick = () => {
            selectedSubSec = sec;
    ...
    // 3열: 철통 무결성 방어선 2D SVG Time-Chainage 엔진
    function renderSVG() {
      ...
      if (currentViewMode === 'SPLITS') {
        ...
        targetSplitsToDraw.forEach((g, gIdx) => {
          g.items.forEach((item, itemIdx) => {
            ...
            // 공종 전용 색상 공정선
            html += `<line x1="${x1}" y1="${y1}" x2="${x2}" y2="${y2}" stroke="${color}" stroke-width="${isSelected ? 9 : 6}" stroke-linecap="round" class="split-line" data-grp="${grpName}" data-sec="${secName}" data-cat="${item.workCategory || '노반'}" data-dist="${item.distM || 0}" data-sy="${startYr.toFixed(1)}" data-ey="${endYr.toFixed(1)}" style="cursor:pointer; opacity:1; filter: drop-shadow(0px 2px 4px rgba(0,0,0,0.2));"/>`;
            ...
            html += `<text x="0" y="2" fill="#0f172a" font-size="10.5" font-weight="700" text-anchor="middle">${secName}</text>`;
            html += `</g>`;
      ...
      body.innerHTML = `
        ...
        <div style="background:#f1f5f9; padding:12px; border-radius:8px; border:1px solid #e2e8f0;">
          ...
          <div style="font-size:0.78rem; color:#64748b; line-height:1.4;">
            본 구간은 <b>${sec.workCategory || '노반'} 공종</b>의 세부 시공구간으로, 총 <b>${sec.distM || 0}m</b> 연장에 걸쳐 공정계획에 따라 안전하게 시공됩니다.
          </div>
        </div>
      `;
...
print("✓ Successfully injected Null Pointer Safety Guard & Bulletproof Error Handling!")
