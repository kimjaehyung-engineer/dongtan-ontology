...
json_totals_path = r'c:\Users\sskjh\antigravity\01_전문업무_및_엔지니어링\11_현장_프로젝트\01_동탄트램\scratch\chunwoo_exact_header_totals.json'
...
with open(json_totals_path, 'r', encoding='utf-8') as f:
    header_totals = json.load(f)

def clean_key(s):
    return re.sub(r'[^0-9a-zA-Z가-힣]', '', str(s))

lookup = {}
for k, v in header_totals.items():
    ck = clean_key(k)
    lookup[ck] = v
...
<head>
  ...
  <title>동탄트램 Time-Chainage 대시보드 (엑셀 노반합계·궤도합계·포장합계 100% 매칭)</title>
        ...
        <div class="chart-topbar">
          <div style="display:flex; flex-direction:column; gap:4px; width:100%;">
            <div class="chart-title-text" id="chart-title-text">
              📍 Time-Chainage 2D 다이어그램 (엑셀 노반합계·궤도합계·포장합계 반영)
            </div>
...
print("✓ Successfully updated Dashboard with Exact 노반합계, 궤도합계, 포장합계 Column Values!")
