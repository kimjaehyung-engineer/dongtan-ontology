...
<head>
  ...
  <title>동탄트램 Time-Chainage 대시보드 (렌더링 철통 보장)</title>
...
<body>
  ...
  <header class="app-header">
    <div class="brand-title">
      ...
      <span class="brand-tag">시공구간 선택 100% 차트 렌더링 무결성 보장</span>
    </div>
  ...
  <script>
    ...
    function renderSVG() {
      ...
      const width = Math.max(900, rawW) * zoomLevel;
      const height = Math.max(500, rawH);
      
      svg.setAttribute('viewBox', `0 0 ${width} ${height}`);
      svg.setAttribute('preserveAspectRatio', 'xMidYMid meet');

      const padL = 80, padR = 50, padT = 50, padB = 70;
      ...
      // 0. 차트 외곽 그리드 테두리 박스
      html += `<rect x="${padL}" y="${padT}" width="${graphW}" height="${graphH}" fill="#ffffff" stroke="#94a3b8" stroke-width="1.8" rx="4"/>`;
      ...
      // 3. 차트 라인 및 밴드 표출 (그림 안 보이는 결함 완전 차단 방어막)
      ...
      targetSplitsToDraw.forEach((g, gIdx) => {
        g.items.forEach((item, itemIdx) => {
          ...
          const startYr = 2027.7 + ((gIdx % 10) * 0.22) + (itemIdx * 0.05);
          ...
          // 시공구간 반투명 배경 밴드
          html += `<rect x="${Math.min(x1, x2)}" y="${padT}" width="${Math.max(4, Math.abs(x2 - x1))}" height="${graphH}" fill="${color}" opacity="${isSelected ? 0.35 : 0.15}"/>`;
...
print("Successfully applied Bulletproof Render Safeguards to Time-Chainage Dashboard HTML!")
