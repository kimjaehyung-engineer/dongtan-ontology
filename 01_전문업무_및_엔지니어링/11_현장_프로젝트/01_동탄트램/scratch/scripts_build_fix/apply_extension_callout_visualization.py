...
new_render = """function renderConstructionSections() {
  ...
  constructionSections.forEach((sec, idx) => {
    ...
    // Ensure normal vector consistently points downwards into open space
    if (ny < 0) {
      nx = -nx;
      ny = -ny;
    }

    // Alternating offset height to avoid overlapping dimension brackets
    ...
    const lineColor = is1 ? "#ea580c" : "#2563eb";
    ...
    startNode.setAttribute("r", "2.2"); startNode.setAttribute("fill", nodeColor);
    startNode.setAttribute("stroke", "#ffffff"); startNode.setAttribute("stroke-width", "0.6");
    ...
    endNode.setAttribute("r", "2.2"); endNode.setAttribute("fill", nodeColor);
    endNode.setAttribute("stroke", "#ffffff"); endNode.setAttribute("stroke-width", "0.6");

    // 2. Vertical Extension Leader Lines
    ...
    leaderLineStart.setAttribute("stroke", lineColor); leaderLineStart.setAttribute("stroke-width", "1");
    leaderLineStart.setAttribute("stroke-dasharray", "3, 2");
    leaderLineStart.setAttribute("opacity", "0.85");
    ...
    leaderLineEnd.setAttribute("stroke", lineColor); leaderLineEnd.setAttribute("stroke-width", "1");
    leaderLineEnd.setAttribute("stroke-dasharray", "3, 2");
    leaderLineEnd.setAttribute("opacity", "0.85");

    // 3. Dimension Bracket Line
    ...
    dimLine.setAttribute("stroke", lineColor); dimLine.setAttribute("stroke-width", "1.5");
    dimLine.style.cursor = "pointer";

    // Start/End Ticks
    const tickStart = document.createElementNS("http://www.w3.org/2000/svg", "line");
    tickStart.setAttribute("x1", startExt.x - nx * 3); tickStart.setAttribute("y1", startExt.y - ny * 3);
    tickStart.setAttribute("x2", startExt.x + nx * 3); tickStart.setAttribute("y2", startExt.y + ny * 3);
    tickStart.setAttribute("stroke", lineColor); tickStart.setAttribute("stroke-width", "1.5");

    const tickEnd = document.createElementNS("http://www.w3.org/2000/svg", "line");
    tickEnd.setAttribute("x1", endExt.x - nx * 3); tickEnd.setAttribute("y1", endExt.y - ny * 3);
    tickEnd.setAttribute("x2", endExt.x + nx * 3); tickEnd.setAttribute("y2", endExt.y + ny * 3);
    tickEnd.setAttribute("stroke", lineColor); tickEnd.setAttribute("stroke-width", "1.5");
    ...
    rect.setAttribute("fill", badgeBg); rect.setAttribute("stroke", nodeColor);
...
if old_render:
    ...
    print("Replaced renderConstructionSections with extension leader callout visualization")
...
new_select = """function selectConstructionSection(sec) {
  ...
  // Focus map on the extension bracket midpoint
  ...
  // 1. Neon Glowing Extension Leader Lines & Dimension Bracket
  ...
  lStart.setAttribute("stroke", glowColor); lStart.setAttribute("stroke-width", "2.5");
  lStart.setAttribute("class", "flashing-route-line");
  ...
  lEnd.setAttribute("stroke", glowColor); lEnd.setAttribute("stroke-width", "2.5");
  lEnd.setAttribute("class", "flashing-route-line");
  ...
  dimHighlight.setAttribute("stroke", glowColor); dimHighlight.setAttribute("stroke-width", "4");
  dimHighlight.setAttribute("class", "flashing-route-line");
  ...
  if (matchedIntersections.length > 0) {
    matchedIntersections.forEach(item => {
      ...
      trackLine.setAttribute("stroke-width", "5.5");
...
if old_select:
    ...
    print("Replaced selectConstructionSection with extension leader callout selection")

with open(path, 'w', encoding='utf-8') as f:
    f.write(text)

print("Applied extension leader callout visualization successfully!")
