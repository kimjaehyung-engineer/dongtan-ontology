...
<head>
  ...
  <title>동탄트램 Time-Chainage 대시보드 (원하는 시공구간 토글 선택)</title>
  ...
  <style>
    ...
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    /* 시공구간 필터 드롭다운 & 토글 툴바 */
    .split-toggle-select {
      background: #eff6ff; border: 1.5px solid #3b82f6; color: #1e40af;
      padding: 5px 10px; border-radius: 6px; font-size: 0.8rem; font-weight: 700;
      outline: none; cursor: pointer; transition: all 0.15s;
    }
    .split-toggle-select:hover { background: #dbeafe; }
    ...
    /* 상단 토글 바 (2차 Quick Filter Bar) */
    .quick-filter-bar {
      height: 38px; background: #f1f5f9; border-bottom: 1px solid var(--border);
      display: flex; align-items: center; padding: 0 1.25rem; gap: 8px; overflow-x: auto;
      white-space: nowrap; scrollbar-width: thin;
    }
    .quick-filter-title { font-size: 0.76rem; font-weight: 700; color: #475569; display: flex; align-items: center; gap: 4px; }
    .btn-quick-tag {
      font-size: 0.72rem; padding: 3px 9px; border-radius: 9999px; border: 1px solid var(--border-dark);
      background: #ffffff; color: var(--text-sub); font-weight: 600; cursor: pointer; transition: all 0.15s;
    }
    .btn-quick-tag:hover { border-color: var(--primary); color: var(--primary); }
    .btn-quick-tag.active { background: #2563eb; color: #ffffff; border-color: #2563eb; }

    /* 메인 2열 레이아웃 */
    .app-layout {
      height: calc(100vh - 98px); display: flex; width: 100vw;
    }
    ...
    .group-header {
      background: #f1f5f9; padding: 10px 12px; font-size: 0.86rem; font-weight: 700;
...
<body>
  ...
  <header class="app-header">
    <div class="brand-title">
      ...
      <span class="brand-tag">시공구간 토글 선택 특화</span>
    </div>
    <div class="header-controls">
      <button class="btn-zone active" id="btn-zone-all" onclick="setZoneFilter('ALL')">전체 공구</button>
      ...
      <!-- 상단 시공구간 필터 드롭다운 -->
      <select class="split-toggle-select" id="split-select-dropdown" onchange="onSplitDropdownChange(this.value)">
        <option value="ALL">🔍 원하시는 시공구간 선택 (전체 보기)</option>
      </select>
  ...
  <!-- 상단 2차 퀵 필터 태그 바 -->
  <div class="quick-filter-bar" id="quick-filter-bar">
    <span class="quick-filter-title">⚡ 퀵 시공구간 토글:</span>
    <button class="btn-quick-tag active" id="tag-all" onclick="setQuickCategory('ALL')">전체 보기</button>
    <button class="btn-quick-tag" id="tag-general" onclick="setQuickCategory('일반부지')">🏗️ 일반부지 구간만</button>
    <button class="btn-quick-tag" id="tag-tram" onclick="setQuickCategory('트램부지')">🚈 트램부지 구간만</button>
    <div style="width: 1px; height: 14px; background: var(--border-dark); margin: 0 4px;"></div>
  </div>
  ...
  <div class="app-layout">
    ...
    <aside class="sidebar">
      <div class="sidebar-header">
        <div class="sidebar-title">
          <span>📋 시공구간 분할 계층 목차</span>
          <div class="tree-toolbar">
            <button class="btn-tree-tool" onclick="expandAllGroups()">📂 전체 펼치기</button>
            <button class="btn-tree-tool" onclick="collapseAllGroups()">📁 전체 접기</button>
          </div>
        </div>
        <input type="text" class="search-input" id="search-input" placeholder="시공구간 명칭 또는 세부구간 검색..." oninput="onSearchChange(this.value)" />
      </div>
    ...
    <main class="main-view">
      <div class="chart-box">
        <div class="chart-topbar">
          <div class="chart-title-text" id="chart-title-text">
            📍 Time-Chainage 2D 다이어그램 (선택 시공구간 클로즈업 뷰)
          </div>
      ...
      <div class="summary-bar">
        ...
        <div class="stat-pill">세부구간 항목: <strong id="stat-subsec-count">123개</strong></div>
        <div class="stat-pill">필터 상태: <strong id="stat-filter-label">전체 시공구간</strong></div>
  ...
  <script>
    ...
    let currentZone = 'ALL';
    let currentSplitFilter = 'ALL'; // Selected split group name
    let currentCategoryFilter = 'ALL'; // 'ALL', '일반부지', '트램부지'
    ...
    const dropdown = document.getElementById('split-select-dropdown');
    ...
    // 상단 시공구간 필터 드롭다운 옵션 동적 생성
    function populateSplitDropdown() {
      dropdown.innerHTML = '<option value="ALL">🔍 원하시는 시공구간 선택 (전체 보기)</option>';
      
      const grpSet = new Set();
      SECTION_SPLITS.forEach(item => {
        if (currentZone !== 'ALL' && item.zone !== currentZone) return;
        grpSet.add(`${item.zone} | ${item.splitGroup}`);
      });

      Array.from(grpSet).sort().forEach(gStr => {
        const parts = gStr.split(' | ');
        const z = parts[0];
        const gName = parts[1];
        const opt = document.createElement('option');
        opt.value = `${z}_${gName}`;
        opt.innerText = `📍 [${z}] ${gName}`;
    ...
    function populateQuickTags() {
      const existingTags = quickBar.querySelectorAll('.btn-group-tag');
      existingTags.forEach(t => t.remove());

      const grpSet = new Set();
      SECTION_SPLITS.forEach(item => {
        if (currentZone !== 'ALL' && item.zone !== currentZone) return;
        if (currentCategoryFilter !== 'ALL' && !item.splitGroup.includes(currentCategoryFilter)) return;
        grpSet.add(`${item.zone}_${item.splitGroup}`);
      });

      Array.from(grpSet).sort().forEach(gKey => {
        const parts = gKey.split('_');
        const z = parts[0];
        const gName = parts[1];

        const btn = document.createElement('button');
        btn.className = `btn-quick-tag btn-group-tag ${currentSplitFilter === gKey ? 'active' : ''}`;
        btn.innerText = `[${z}] ${gName}`;
        btn.onclick = () => {
          if (currentSplitFilter === gKey) {
            currentSplitFilter = 'ALL';
          } else {
            currentSplitFilter = gKey;
            expandedGroups.add(gKey);
          }
          dropdown.value = currentSplitFilter;
          renderAll();
        };
    ...
    function getGroupedSplits() {
      ...
      SECTION_SPLITS.forEach(item => {
        ...
        if (currentSplitFilter !== 'ALL' && gKey !== currentSplitFilter) return;
        if (currentCategoryFilter !== 'ALL' && !item.splitGroup.includes(currentCategoryFilter)) return;
    ...
    function renderSectionTree() {
      ...
      document.getElementById('stat-filter-label').innerText = currentSplitFilter !== 'ALL' ? currentSplitFilter.replace('_', ' ') : (currentCategoryFilter !== 'ALL' ? currentCategoryFilter + '만' : '전체 시공구간');

      groups.forEach(g => {
        const gKey = `${g.zone}_${g.groupName}`;
        const isExpanded = expandedGroups.has(gKey) || currentSplitFilter === gKey;

        const card = document.createElement('div');
        card.className = `split-group-card ${isExpanded ? 'expanded' : ''} ${currentSplitFilter === gKey ? 'active' : ''}`;
    ...
    function renderSVG() {
      ...
      const padL = 70, padR = 40, padT = 45, padB = 65;
      ...
      for (let yr = 2027.5; yr <= 2032; yr += 0.5) {
        ...
        html += `<text x="${padL - 12}" y="${y + 4}" fill="#64748b" font-size="11" font-weight="500" text-anchor="end">${yr.toFixed(1)}년</text>`;
      }
      ...
      STATIONS.forEach((st) => {
        ...
        html += `<text x="${x}" y="${height - padB + 20}" fill="#475569" font-size="10.5" font-weight="500" text-anchor="end" transform="rotate(-40, ${x}, ${height - padB + 20})">${st.name}</text>`;
      });
      ...
      if (currentViewMode === 'SPLITS') {
        ...
        groups.forEach((g, gIdx) => {
          g.items.forEach((item, itemIdx) => {
            ...
            const x2 = getX(endKm > startKm ? endKm : startKm + 0.5);
            ...
            const color = item.splitGroup.includes('일반') ? '#ef4444' : '#16a34a';

            html += `<rect x="${Math.min(x1, x2)}" y="${padT}" width="${Math.abs(x2 - x1)}" height="${graphH}" fill="${color}" opacity="${isSelected ? 0.3 : 0.12}"/>`;
            html += `<line x1="${x1}" y1="${y1}" x2="${x2}" y2="${y2}" stroke="${color}" stroke-width="${isSelected ? 6 : 4}" stroke-linecap="round" class="split-line" data-grp="${item.splitGroup}" data-sec="${item.sectionName}" data-dist="${item.distM}" data-sy="${startYr.toFixed(1)}" data-ey="${endYr.toFixed(1)}" style="cursor:pointer; opacity:${isSelected ? 1 : 0.9};"/>`;
            html += `<text x="${(x1 + x2)/2}" y="${(y1 + y2)/2 - 8}" fill="#1e3a8a" font-size="10" font-weight="700" text-anchor="middle">${item.sectionName}</text>`;
          });
        });
      } else {
        RAW_ACTIVITIES.forEach((act, idx) => {
          ...
          const y2 = getY(endY);
          html += `<line x1="${x1}" y1="${y1}" x2="${x2}" y2="${y2}" stroke="#2563eb" stroke-width="2.5" stroke-linecap="round" opacity="0.8"/>`;
        });
      }
    ...
    function onSplitDropdownChange(val) {
      currentSplitFilter = val;
      if (val !== 'ALL') {
        expandedGroups.add(val);
      }
      renderAll();
    }
    ...
    function setQuickCategory(cat) {
      ...
      document.getElementById('tag-general').classList.remove('active');
      document.getElementById('tag-tram').classList.remove('active');
      
      if (cat === 'ALL') document.getElementById('tag-all').classList.add('active');
      if (cat === '일반부지') document.getElementById('tag-general').classList.add('active');
      if (cat === '트램부지') document.getElementById('tag-tram').classList.add('active');
...
print("Successfully updated Top Bar Toggle Controls for Section Split Filtering!")
