...
<head>
  ...
  <title>동탄트램 Time-Chainage 대시보드 (5대 세부공종 완벽 구분)</title>
...
<body>
  ...
  <header class="app-header">
    <div class="brand-title">
      ...
      <span class="brand-tag">5대 전문 공종 세분화 구분</span>
    </div>
      ...
      <div class="chart-box">
        <div class="chart-topbar">
          <div style="display:flex; flex-direction:column; gap:4px; width:100%;">
            ...
            <!-- 5대 전문 공종 색상 범례 (5 Work Categories Color Legend) -->
  ...
  <script>
        ...
        g.items.forEach(sec => {
          ...
          itemEl.innerHTML = `
            <div class="sub-sec-head">
              ...
              <span style="font-size:0.7rem; color:${sec.workColor}; font-weight:700;">${sec.workCategory}</span>
            </div>
    ...
    // 3열: 5대 공종 세분화 컬러 바인딩 2D SVG Engine
    function renderSVG() {
      ...
      if (currentViewMode === 'SPLITS') {
        ...
        targetSplitsToDraw.forEach((g, gIdx) => {
          g.items.forEach((item, itemIdx) => {
            ...
            // 공종 전용 색상 공정선
            html += `<line x1="${x1}" y1="${y1}" x2="${x2}" y2="${y2}" stroke="${color}" stroke-width="${isSelected ? 9 : 6}" stroke-linecap="round" class="split-line" data-grp="${item.splitGroup}" data-sec="${item.sectionName}" data-cat="${item.workCategory}" data-dist="${item.distM}" data-sy="${startYr.toFixed(1)}" data-ey="${endYr.toFixed(1)}" style="cursor:pointer; opacity:1; filter: drop-shadow(0px 2px 4px rgba(0,0,0,0.2));"/>`;
      ...
      document.querySelectorAll('.split-line').forEach(el => {
        el.addEventListener('mouseenter', (e) => {
          ...
          tooltip.innerHTML = `<div class="tooltip-title">[${e.target.dataset.grp}] ${e.target.dataset.sec}</div><div>세부공종: <b>${e.target.dataset.cat}</b></div><div>연장: <b>${e.target.dataset.dist}m</b></div><div>예정공기: ${e.target.dataset.sy}년 ~ ${e.target.dataset.ey}년</div>`;
        });
    ...
    function openModalForSection(sec) {
      ...
      body.innerHTML = `
        ...
        <div class="detail-grid">
          ...
          <div class="detail-item"><div class="detail-label">세부 공종 구분</div><div class="detail-value" style="color:${sec.workColor};">${sec.workCategory}</div></div>
        ...
        <div style="background:#f1f5f9; padding:12px; border-radius:8px; border:1px solid #e2e8f0;">
          ...
          <div style="font-size:0.78rem; color:#64748b; line-height:1.4;">
            본 구간은 <b>${sec.workCategory} 공종</b>의 세부 시공구간으로, 총 <b>${sec.distM}m</b> 연장에 걸쳐 공정계획에 따라 안전하게 시공됩니다.
          </div>
        </div>
      `;
...
print("✓ Successfully implemented 5-Work-Category Color Classification & Legend System!")
