# -*- coding: utf-8 -*-
"""
동탄트램 11.철도종합시운전 28개 액티비티 총 84개 HTML 파일 일괄 생성 엔진
(표준서 28개 + 수행지침 28개(1:1 SVG 도식 및 모달) + 체크리스트 28개)
"""
...
from commish_part1 import COMMISH_TASKS_PART1
from commish_part2 import COMMISH_TASKS_PART2
from commish_custom_svgs_part1 import COMMISH_SVGS_PART1
from commish_custom_svgs_part2 import COMMISH_SVGS_PART2

ALL_TASKS = COMMISH_TASKS_PART1 + COMMISH_TASKS_PART2
ALL_SVGS = {**COMMISH_SVGS_PART1, **COMMISH_SVGS_PART2}

BASE_DIR = os.path.abspath(r"08.메뉴얼 및 평면도\최종\02_메뉴얼 공종프로섹스(집행단계)\매뉴얼BODY(집행단계-첨부폴더)v8\11.철도종합시운전")
os.makedirs(BASE_DIR, exist_ok=True)

print(f"철도종합시운전 총 {len(ALL_TASKS)}개 액티비티 HTML 생성 시작...")
...
<body>
  <div class="container">
    <div class="header">
      <span class="badge">{wbs_code} | 동탄트램 철도종합시운전 표준서</span>
    ...
    <div class="section-title">3. 표준 기술 시방 및 검측 요구조건 (철도안전법 / 시행지침)</div>
    <table class="table">
      <thead>
        <tr>
          <th style="width:20%;">검측 항목</th>
          <th style="width:40%;">시방 기준 및 품질 요구조건</th>
          <th style="width:20%; text-align:center;">적용 시기</th>
    ...
    for s_idx, (stitle, ssub, sdetails) in enumerate(steps, 1):
        ...
        <div style="background:#ffffff; border:1px solid #e2e8f0; border-radius:8px; padding:18px; margin-bottom:16px; box-shadow:0 2px 6px rgba(0,0,0,0.03);">
          <div style="display:flex; align-items:center; gap:10px; margin-bottom:8px;">
            <span style="background:#0284c7; color:#ffffff; font-size:12px; font-weight:800; padding:3px 8px; border-radius:4px;">{stitle}</span>
            <span style="font-size:15px; font-weight:700; color:#0f172a;">{ssub}</span>
          </div>
...
<body>
  <div class="container">
    <div class="header">
      <span class="badge">{wbs_code} | 동탄트램 철도종합시운전 수행지침</span>
    ...
    <div class="section-title">3. 핵심 엔지니어링 및 법령 용어 해설</div>
    <div style="margin-top:12px;">
      {term_boxes}
    </div>

    <div class="section-title">4. 시운전 속도별 비상제동거리 및 안전 판정기 (Interactive Calculator)</div>
    <div style="background:#f8fafc; border:1px solid #cbd5e1; border-radius:8px; padding:16px; margin-top:12px;">
      <div style="font-size:13px; font-weight:700; color:#0f172a; margin-bottom:10px;">트램 주행 속도별 비상제동 정지거리 계산 및 적합성 판정</div>
      <div style="display:flex; gap:12px; flex-wrap:wrap; align-items:center;">
        <label style="font-size:12px; color:#334155;">주행 속도 (km/h): <input type="number" id="testSpeed" value="70" step="5" style="width:70px; padding:4px 8px; border:1px solid #cbd5e1; border-radius:4px;"></label>
        <label style="font-size:12px; color:#334155;">실측 제동거리 (m): <input type="number" id="testDist" value="142" style="width:70px; padding:4px 8px; border:1px solid #cbd5e1; border-radius:4px;"></label>
        <button onclick="calculateBrake()" style="background:#0284c7; color:#fff; border:none; padding:6px 14px; border-radius:4px; font-size:12px; font-weight:700; cursor:pointer;">적합성 판정</button>
  ...
  <script>
    ...
    function calculateBrake() {{
      const v = parseFloat(document.getElementById('testSpeed').value);
      const d = parseFloat(document.getElementById('testDist').value);
      const res = document.getElementById('calcResult');
      // 법정 비상제동 한계거리 식: S_max = v^2 / (2 * 1.3 * 9.81 * 0.1) approx 160m at 70km/h
      let limit = 160;
      if (v <= 30) limit = 40;
      else if (v <= 50) limit = 90;
      else limit = 160;

      if (d <= limit) {{
        res.innerHTML = "✔ [합격] 법정 허용 제동거리 이내 (속도 " + v + "km/h 기준 " + limit + "m 이하 만족)";
        res.style.color = "#15803d";
      }} else {{
        res.innerHTML = "❌ [불합격] 허용 제동거리 초과 (기준: " + limit + "m 이하)";
...
<body>
  <div class="container">
    <div class="header">
      <span class="badge">{wbs_code} | 동탄트램 철도종합시운전 체크리스트</span>
      <h1 class="title">{task_title} 시운전 검측 체크리스트</h1>
      <p class="subtitle">{subtitle}</p>
    </div>

    <div class="section-title">1. 필수 검측 체크리스트 (철도안전법 / 시행지침)</div>
    <table class="table">
      <thead>
        <tr>
          <th style="width:8%; text-align:center;">No.</th>
          <th style="width:22%;">검측 항목</th>
          <th style="width:45%;">점검 내용 및 기준</th>
    ...
    <div style="background:#f8fafc; border:1px solid #e2e8f0; border-radius:8px; padding:16px; margin-top:12px; display:flex; justify-content:space-between; align-items:center;">
      <div>
        ...
        <div style="font-size:12px; color:#64748b; margin-top:4px;">철도안전법 및 국토부 시행지침 100% 충족 확인</div>
      </div>
...
# -------------------------------------------------------------
# 4. 28개 액티비티 폴더 생성 및 84개 파일 작성 루프
# -------------------------------------------------------------
created_count = 0
...
for item in ALL_TASKS:
    ...
    # 폴더 구조: 11.철도종합시운전/{folder_name}/표준서, 수행지침, 체크리스트
...
print(f"\n=======================================================")
print(f"동탄트램 11.철도종합시운전 28개 액티비티 총 {created_count}개 HTML 파일 생성 완료!")
print(f"=======================================================")
