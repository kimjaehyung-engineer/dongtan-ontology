...
<head>
  ...
  <title>동탄트램 Time-Chainage 대시보드 (시공구간 분할 아코디언 트리)</title>
...
<body>
  ...
  <header class="app-header">
    <div class="brand-title">
      ...
      <span class="brand-tag">시공구간 분할 아코디언 트리 & 맵 표출 특화</span>
    </div>
    <div class="header-controls">
      ...
      <button class="btn-viewmode active" id="btn-view-splits" onclick="setViewMode('SPLITS')">🗺️ 시공구간 대표 표출 뷰</button>
  ...
  <div class="app-layout">
    ...
    <main class="main-view">
      <div class="chart-box">
        <div class="chart-topbar">
          <div class="chart-title-text" id="chart-title-text">
            📍 Time-Chainage 2D 다이어그램 (시공구간 분할 대표 뷰)
          </div>
      ...
      <div class="summary-bar">
        <div class="stat-pill">시공구간 분할: <strong id="stat-group-count">28개 구간</strong></div>
        <div class="stat-pill">세부구간 항목: <strong id="stat-subsec-count">123개</strong></div>
        <div class="stat-pill">착공예정: <strong>2027년 07월 01일</strong></div>
  ...
  <script>
    ...
    let currentZone = 'ALL';
    let currentViewMode = 'SPLITS'; // 'SPLITS' or 'ACTS'
    let searchQuery = '';
    let zoomLevel = 1.0;

    let expandedGroups = new Set(['1공구_일반부지 1구간', '2공구_일반부지 1구간']);
    ...
    function renderSectionTree() {
      ...
      groups.forEach(g => {
        ...
        card.className = `split-group-card ${isExpanded ? 'expanded' : ''}`;
      ...
      if (currentViewMode === 'SPLITS') {
        ...
        groups.forEach((g, gIdx) => {
          g.items.forEach((item, itemIdx) => {
            ...
            // 대표 구간 배경 밴드 및 라인
            html += `<rect x="${Math.min(x1, x2)}" y="${padT}" width="${Math.abs(x2 - x1)}" height="${graphH}" fill="${color}" opacity="${isSelected ? 0.25 : 0.08}"/>`;
            html += `<line x1="${x1}" y1="${y1}" x2="${x2}" y2="${y2}" stroke="${color}" stroke-width="${isSelected ? 6 : 3.5}" stroke-linecap="round" class="split-line" data-grp="${item.splitGroup}" data-sec="${item.sectionName}" data-dist="${item.distM}" data-sy="${startYr.toFixed(1)}" data-ey="${endYr.toFixed(1)}" style="cursor:pointer; opacity:${isSelected ? 1 : 0.85};"/>`;
...
print("Successfully generated Section-Split Accordion Tree Time-Chainage Dashboard HTML!")
